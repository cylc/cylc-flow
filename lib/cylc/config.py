...
###            #print "OK:", itask.id
    ...
    def generate_triggers( self, lexpression, lnames, right, cycler, asyncid_pattern, suicide ):
        ...
        if not re.search( '\|', lexpression ):
            ...
            for label in ctrig:
                ...
                if right in self.cycling_tasks and \
                    (cname[label] in self['scheduling']['special tasks']['start-up'] or \
                        ...
                        self.taskdefs[right].add_startup_trigger( trigger, cycler, suicide )
                    ...
                    self.taskdefs[right].add_trigger( trigger, cycler, suicide )
        else:
            countx = 0
            for label in ctrig:
            ...
            if right in self.cycling_tasks and countx == len( cname.keys() ):
                self.taskdefs[right].add_startup_conditional_trigger( ctrig, expr, cycler, suicide )
            else:
                self.taskdefs[right].add_conditional_trigger( ctrig, expr, cycler, suicide )
    ...
    def get_graph( self, start_ctime, stop, colored=True, raw=False,
        ...
        # Get initial cycle time for the suite (all cyclers)
        global_initial_ctime = cycon( self.cyclers ).initial_adjust_up( start_ctime )

        for e in self.edges:
            # Get initial cycle time for this cycler
            ctime = e.cyclr.initial_adjust_up( start_ctime )

            while int(ctime.get()) <= int(stop):
                # Loop over cycles generated by this cycler

                if int(ctime.get()) == int(global_initial_ctime):
                    initial_cycle = False
                else:
                    initial_cycle = True
    ...
    def close_families( self, nlid, nrid ):
        ...
        if nlid:
            one, two = nlid.split()
            lname = one.getstr()
            ltag = two.getstr(formatted=True)
            nl = nlid.getstr(formatted=True)
        if nrid:
            one, two = nrid.split()
            rname = one.getstr()
            rtag = two.getstr(formatted=True)
            nr = nrid.getstr(formatted=True)
