...
class config( object ):
    ...
    def process_graph_line( self, line, section, seq, offset_seq_map,
        ...
        for i in [0] + range( 1, len(tasks)-1 ):
            ...
            if self.validation:
                # Detect self-edges, the simplest kind of graph cycle (a => a),
                # which stalls suites.  These aren't found by the cyclic check
                # because they are eliminated from the graph (by (py)graphviz?).
                # They can easily be generated by FAMILY <=> member,
                #   E.g. "FAM:succeed-all => f" (where f is a member of FAM)
                self_dep_nodes = []
                for right_node in right_nodes:
                    for left_node in left_nodes:
                        if right_node in left_node:
                            # E.g. "f in f:succeed".
                            # (avoids self-suicide "f => !f")
                            self_dep_nodes.append(right_node)
                if len(self_dep_nodes):
                    print >> sys.stderr, 'Self-edges found in "%s":' % orig_line
                    for node in self_dep_nodes:
                        print >> sys.stderr, "  %s => %s" % (node, node)
                    raise SuiteConfigError, "ERROR: self-edges detected"
