...
def generate_req_string( foo ):
    ...
    for req in foo:
        ...
        # replace '$(CURRENT_REFERENCE_TIME)' or '$(CURRENT_REFERENCE_TIME - XX )'
        m = re.search( '\$\(CURRENT_REFERENCE_TIME\s*-\s*(\d+)\s*\)', req )
        if m:
            req = re.sub( '\$\(CURRENT_REFERENCE_TIME.*\)', '\' + reference_time.decrement( ref_time, ' + m.group(1) + ') + \'', req )
        else:
            req = re.sub( '\$\(CURRENT_REFERENCE_TIME\)', '\' + ref_time + \'', req )
        ...
        if timed:
            strng = strng + indent + indent_unit + '[ ' + time + ', ' + req  + ' ]'
        else:
            strng = strng + indent + indent_unit + req 
...
\n''')
    ...
    #    print header
        ...
        if 'DELAYED_DEATH' in parsed_def.keys():
            ...
            if delayed_death == 'False' or delayed_death == 'false' or delayed_death == 'No' or delayed_death == 'no':
                quick_death = 'False'
