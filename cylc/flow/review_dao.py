...
class CylcReviewDAO(object):
    ...
    def get_suite_broadcast_states(self, user_name, suite_name):
        ...
        stmt = CylcWorkflowDAO.pre_select_broadcast_states(
            self._db_init(user_name, suite_name), order="ASC"
        )[0]
        broadcast_states = []
    ...
    def get_suite_broadcast_events(self, user_name, suite_name):
        ...
        stmt = CylcWorkflowDAO.pre_select_broadcast_events(
            self._db_init(user_name, suite_name), order="DESC"
        )[0]
        broadcast_events = []
        for row in self._db_exec(user_name, suite_name, stmt):
    ...
    def set_is_cylc8(user_name, suite_name):
        ...
        suite_dir = os.path.join(
            CylcReviewService._get_user_home(user_name), "cylc-run", suite_name
        )
        ...
        for cycle in relevant_targzip_log_cycles:
            ...
            for member in tar.getmembers():
                ...
                if not member.isfile():
                    continue
                try:
                    cycle_str, name, submit_num_str = member.name.split(
                        "/", 4
                    )[1:4]
    ...
    def get_suite_cycles_summary(
        ...
        * T2 is the time when last update time of (a task in) the cycle
        ...
        of_n_entries = 0
        stmt = (
            "SELECT COUNT(DISTINCT cycle) FROM task_states WHERE "
            + "submit_num > 0"
        )
        try:
            for row in self._db_exec(user_name, suite_name, stmt):
        ...
        check_query = self._db_exec(
            user_name, suite_name, check_stmt, ["task_jobs"]
        )
        if check_query.fetchone() is not None:
            stmt = (
                "SELECT cycle,"
                + " sum("
                + self.JOB_STATUS_COMBOS["submitted,running"]
                + ") AS n_job_active,"
                + " sum("
                + self.JOB_STATUS_COMBOS["succeeded"]
                + ") AS n_job_success,"
                + " sum("
                + self.JOB_STATUS_COMBOS["submission-failed,failed"]
                + ") AS n_job_fail"
                + " FROM task_jobs GROUP BY cycle"
            )
        else:
            fail_events_stmt = " OR ".join(
                ["event=='%s'" % (name) for name in TASK_STATUS_GROUPS["fail"]]
            )
            stmt = (
                "SELECT cycle,"
                + " sum("
    ...
    def get_suite_state_summary(self, user_name, suite_name):
        ...
        * server: host:port of server, if available
        """
        ret = {"is_running": False, "is_failed": False, "server": None}
        ...
        port_file_path = os.path.expanduser(
            os.path.join(
                "~" + user_name, "cylc-run", suite_name, ".service", "contact"
            )
        )
        try:
            host = None
