...
def strip_null(data):
    ...
    if is_awaitable(data):
        return async_next(strip_null, data)
...
class CylcExecutionContext(ExecutionContext):
    ...
    def execute_operation(
        self, initial_result_record, root_value
        ...
        """
        result = super().execute_operation(initial_result_record, root_value)

        # Traverse the document and stop if found
        ...
        if cylc_visitor.arg_flag:
            return async_next(null_stripper, result)  # type: ignore
        return result
