...
class TaskJobManager:
    ...
    * Remove workflow contact files on job hosts.
    ...
    def submit_task_jobs(self, workflow, itasks, curve_auth,
        ...
        """
        itasks, ghost_tasks = self._ghost_submit_task_jobs(
    ...
    def _ghost_submit_task_jobs(
        ...
        Returns:
            * A list of tasks which require subsequent processing as
              if they were live mode tasks.
            * A list of tasks which require no further processing.
        """
        live_tasks: 'List[TaskProxy]' = []
        ...
        for itask in itasks:
            ...
            if rtconfig['run mode'] == 'workflow':
                run_mode = workflow_run_mode
            else:
                if rtconfig['run mode'] != workflow_run_mode:
                    LOG.info(
            ...
            if is_done:
                ...
                live_tasks.append(itask)
        return live_tasks, ghost_tasks
