...
if [[ ! -x $CYLC_DIR/bin/cylc ]]; then
    ...
    echo " % cd $CYLC_DIR"
    echo " % chmod +x bin/* util/* examples/*/scripts/*"
    return 1
fi
...
PYTHONPATH=$CYLC_DIR/src:$CYLC_DIR/src/job-submission:$CYLC_DIR/src/task-types:$CYLC_DIR/src/locking:$CYLC_DIR/src/gui:$CYLC_DIR/src/external:$PYTHONPATH

if [[ -n $CYLC_SUITE_DIR ]]; then
    # caller must be a cylc job scritp; add suite-specific paths as well
    PATH=$CYLC_SUITE_DIR/scripts:$PATH
    PYTHON_PATH=$CYLC_SUITE_DIR/configured:$PYTHONPATH
fi
