...
if [ $FAST -gt 3 ]; then
    MSG="FAST not within allowed range (0,1,2,3)"
    echo "`date -u +%Y%m%d%T%Z`; $msgout $MSG"
    cylc task-message -p CRITICAL $MSG
    cylc task-failed
    exit 1
...
while true; do
    ...
    # Note: recls can be used with a patern to list only files you want.
    # This could be used in a future version for multiple files
    # download.
