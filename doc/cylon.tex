...
\title{An Optimal Dynamic Metascheduler for Multi-Model Environmental Forecasting Operations}
...
This document describes a new dynamic metascheduler (a type of control
system) for multi-model environmental forecast operations. {\em Cylon}
is optimal in the sense that it is able to present tasks to the batch
queue scheduler at the earliest possible time, regardless of forecast
cycle, and automatically achieves maximum functional parallelism. During
catchup from delays, and throughout historical case studies or delayed
parallel trials, it runs tasks from multiple forecast cycles at once to
the full extent allowed by inter-task dependencies.  This development
was largely motivated by the need for faster catchup operation in a
complex system with little downtime between forecast cycles, but the
total lack of system-specific scheduling logic in cylon also makes it
extremely flexible, easy to use, and widely applicable.  Cylon can
control tasks across a heterogenous distributed network and is easily
interfaced to existing systems. It can stop and restart the entire
forecasting operation in any state of operation (running or failed tasks
are automatically restarted) and tasks can be added and removed
...
failures that involve omitted cycles in the driving models. Cylon is
