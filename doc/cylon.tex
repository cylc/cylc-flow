...
\begin{abstract}
    ...
    {\em Cylon} is a metascheduler\footnote{A {\em
    ...
    this paper.} for controlling complex forecasting systems that
    consist of any number of driving models, downstream models, and
    associated data collection, preprocessing, postprocessing, and
    product generation tasks. Cylon explicitly handles all dependencies
    between individual tasks, including those between tasks in 
    different forecast cycles, and consequently is not constrained by
    forecast cycle boundaries when the system's external driving data
    for one or more future cycles is available in advance. 
    \footnote{A {\em task} is any group of processes treated as a
