...
%\title{An Optimal Dynamic Metascheduler for Multi-Model Environmental Forecasting Systems}
%\title{A Next-Generation Control System for Complex Environment %Forecasting Operations}
\title{A Continuous Task Scheduler for Complex Environmental Forecasting Systems}
...
{\em Cylon} is a new task scheduling system for complex multi-model
environmental forecasting operations that uniquely, as far as the author
is aware, manages continuously interdependent task networks with no
division into global (system-wide) forecast cycles, and thus achieves
optimal scheduling in all modes of operation. A series of distinct
forecast cycles emerges naturally in normal real time operation but
during catch up operation, delayed parallel trials, or historical case
studies, tasks with many different cycle times can run simultaneously to
the full extent allowed by their dependencies. Cylon's development was
...
interfaced to existing systems.  Dependencies between tasks do not need
to be defined explicitly by the user but are resolved at run-time (each
task simply registers its own prerequisites and outputs as if it was to
run alone). Tasks can be added and removed the fly, and the system can
be stopped and restarted in any state of operation.
%(running or failed tasks are automatically restarted) 
Control of the configured task set, and failure recovery scenarios, can
be tested quickly in a simulation mode that is indistinguishable (to the
control system) from real operation. Explicit handling of forecast model
`restart' dependencies allows continued operation, with very little
human intervention, over failures that involve omitted cycles in the
driving models. Cylon is written in Python and uses the Python Remote
