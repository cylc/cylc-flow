...
\begin{abstract}
    ...
    {\em Cylon} is a metascheduler\footnote{A {\em metascheduler}
    determines the order in which tasks are sent to the job
    scheduler(s), i.e.\ it decides when dependent tasks are {\em ready}
    to run. The term also describes a single aggregate view of multiple
    distributed resource managers, which is not the topic of this
    paper.} for forecasting systems that abandons the system-wide
    forecast cycle in favour of explicit handling of intercycle
    dependencies between individual tasks.\footnote{A {\em task} is any
    group of processes treated as a single entity for scheduling
    purposes.} In a system controlled by cylon a series of distinct
    forecast cycles emerges naturally in real time operation, but in
    catch up operation, delayed parallel trials, or historical case
    studies, tasks from any number of forecast cycles can run
    simultaneously to the full extent allowed by dependencies, with the
    potential for greatly increased system throughput. Cylon is also
    extremely flexible and easy to use because each task simply
...
Existing metaschedulers suited to controlling forecasting systems, as
far as the author is aware, invariably assume/require a series of
distinct {\em forecast cycles}. This represents normal real time
operation perfectly well, but it can be a serious impediment whenever
advance availability of external driving data makes it possible, in
principle, to run tasks from future cycles before the current cycle is
finished [Section~\ref{sec:Existing}]. The more primitive existing
systems place the burden on the user to define a list of tasks to cycle
through, perhaps with some way of specifying some functional
...
system is changed. Another option is system-specific finite state
scheduling logic. Optimal job scheduling within a forecast cycle is
possible in this case but the coded logic inevitably becomes convoluted
and inflexible as system complexity increases, and handling of
intercycle dependencies is almost certainly not feasible. Finally, a
general scheduling tool for jobs with dependencies, such as SMS, is
probably the best option. but as discussed, this does not allow
successive cycles to interleave.

Notably, this occurs after delays
or downtime, when better system throughput would result in faster catch
up operation. But allowing this, in the general case, requires explicit
handling of intercycle dependencies, which dramatically increases the
apparent complexity of even the simplest systems
[Section~\ref{sec:ICD}].\footnote{A crude system-specific overlap of
consecutive ``sequential'' cycles may be feasible for specific systems,
but not in general because of intercycle dependencies.} 


Cylon uses a novel run time dependency negotiation algorithm
[Section~\ref{sec:cylon}]. A pool of task proxy objects represent, and
can interact with, external system tasks. A task proxy object knows its
prerequisites (inputs) and keeps track of its completed outputs, but is
unaware of other tasks in the system. The task pool must include
``waiting'' and finished tasks from previous, current, and future
cycles, as well as those currently running (the life cycle of a proxy
object is such that it always exists by the time it is needed, and dies
not long after it is no longer needed). The entire task pool is then
enabled to interact indiscriminately, and regardless of forecast cycle,
to negotiate dependencies. Correct task ordering thus emerges naturally
at run time even though tasks do not know who is supposed to supply
their inputs, and optimal throughput is achieved in all modes of
operation. During catch up from delays, and similarly for historical
...
processing tasks. From a job scheduling perspective a forecasting system
...
forecast fields from an external system). 
Tasks are therefore naturally grouped by {\em forecast cycle}, wherein
each task has the same {\em forecast reference time}, which is the
nominal start time of the forecast models in the group, or that 
of the associated forecast model run(s) for data processing tasks.
...
An ideal forecasting control system would queue tasks as soon as
their prerequisites are satisfied, but not before, so they can
begin executing immediately sufficient resource becomes available. 

%\footnote{A task that depends on external data can be treated as
...
If the system's external driving data are available in advance, however,
(see below) some tasks belonging to subsequent cycles may be able to
start, in principle, before the current cycle has finished. 
...
%If we are restricted to sequential cycling, the best that can be done in
%general is to eliminate the gap between cycles (a limited crude overlap
%of the single cycle job schedule may be possible for specific task sets,
%but we are interested in automatic optimal metascheduling for any
%system).
