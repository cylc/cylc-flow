...
In the documentation below, the ``configured system'' means the system
(set of defined tasks) that is accessible to cylc via the user's shell
environment. This is achieved by sourcing the cylc environment file 
(which is created by 'cylc configure-system') for that system.
...
\subsection{cylc configure-system}
...
{
\color{Magenta}
\lstinputlisting{command-usage/cylc-configure-system.txt}
}
