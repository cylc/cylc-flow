...
\begin{abstract}
    ...
    metascheduler\footnote{A metascheduler sends dependent tasks to the
    batch queue job scheduler(s) when it has determined that they are
    {\em ready}to run. We drop the ``meta'' prefix from here on,
    ...
    throughput. Catchup and real time operation are not treated
    separately: in the one running system a series of distinct forecast
    cycles will seamlessly emerge when and if its tasks catch up to real
    time operation. Cylc is easily interfaced to existing tasks and is
    ...
    downstream dependants the rest of the system can carry on unaffected
    while the problem is addressed, after which time the delayed tasks
    will catch up as quickly as possible.  Cylc's handling of forecast
    model `restart' dependencies allows continued operation, with very
    little operator intervention, over major failures that result in
    omitted forecasts in the driving models.  Ability to control the
    ...
    object oriented Python and uses {\em Pyro} (Python Remote Objects).
    to control tasks across a heterogenous distributed network.  
