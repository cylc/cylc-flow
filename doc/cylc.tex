...
\subsection{Configuring A New System}
...
Now generate a task class module and config file for the new system
(Section~\ref{sec:configure}), and customise the config file if
necessary.
\begin{lstlisting}
    cylc configure -s SYSTEM [options]
\end{lstlisting}


\subsection{Reconfiguring An Existing System}

To regenerate the system's task class module after adding, removing, or
modifying its task definition files (Section~\ref{sec:configure}).

\begin{lstlisting}
    cylc configure -s SYSTEM [options]
\end{lstlisting}

Then customise the system config file if necessary.

\subsection{Scheduling A Configured System}

To cold start or restart a configured system (Section~\ref{sec:run}):

\begin{lstlisting}
    cycl schedule -s SYSTEM [options]
\end{lstlisting}
    
\subsection{Monitoring A Running System}

There are currently several terminal based system monitors for cylc
(Section~\ref{sec:monitor}), for example: 

\begin{lstlisting}
    cylc monitor -s SYSTEM [options]
\end{lstlisting}
