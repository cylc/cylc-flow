...
\pagebreak
\section{Configuring and Running Cylc Systems}
\label{sec:usage}

\subsection{Quick Start Summary}

\begin{itemize}
    \item divide your system into {\em tasks} of known {\em type}
        (Section~\ref{sec:tasktype} with defined {\em prerequisites} and
        {\em outputs} (\ref{sec:requisites}. 

    \item write a {\em task definition file} 
        (Section~\ref{sec:taskdef}) for each task.

    \item write a single {\em user config file} 
        (Section~\ref{sec:config}) for your system.

    \item run \verb=cylc configure <system-dir>= to generate the 
        generates Python task class code and environment script 
        for your system (Section~\ref{sec:configure}).

    \item source the newly genearted cylc environment script in 
        your shell.
        
    \item test run your system in {\em dummy mode}: 
        \verb=cycl scheduler -d YYYYMMDDHH=
        (Section~\ref{sec:dummymode}). 

    \item write a {\em task script} for each task, to execute real
        external tasks (Section~\ref{sec:taskscripts}).

    \item run your system for real: \verb=cylc scheduler YYYYMMDDHH=
        (Section~\ref{sec:run}).

    \item monitor your running system(s) with \verb=cylc monitor=.
        (Section~\ref{sec:monitor})

    \item interact with / control your running system(s) with
        \verb=cylc controller=
...
\pagebreak
\subsection{Monitoring Running Systems}
...
\pagebreak
\subsection{Packaged Working Examples}
...
Configure a new system, or reconfigure an existing one after modifying
its task definition files. This automatically generates a Python 
source module that defines task classes for the system, based on its
task definition files, and creates a script \verb=cylc-env.sh= that,
when sourced in the user's shell, configures the environment so that
cylc can run the system.  
...
Start up a cylc scheduler for the configured system. Multiple scheduler
instances can run at once (e.g.\ to control a main operation and several
parallel test systems) so long as each system is configured, via its
user config file (Section~\ref{sec:config}), with a different {\em
system name} (which distinguishes objects registered by each system in
the Pyro nameserver).
