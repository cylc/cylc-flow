...
\begin{abstract}
    ...
    throughput. Under cylc delayed and real time operation are not
    treated differently: if and when a system catches up to real time
    operation a series of distinct forecast cycles will seamlessly
    emerge. Cylc is easily interfaced to existing tasks and is extremely
    flexible and easy to use. It can be stopped and restarted in
    ...
    is written in object oriented Python and uses {\em Pyro} (Python
    Remote Objects) to control tasks across heterogenous distributed
    networks.  
...
\begin{lstlisting}
    # configure the userguide-1 system
    cylc configure --system userguide-1 $CYLC_DIR/sys/examples/userguide-1
    # start scheduling in dummy mode
    cylc schedule --system userguide-1 --dummy-mode 2010081012
\end{lstlisting}
...
\begin{lstlisting}
    ...
    cylc monitor --system userguide-1 -a
\end{lstlisting}
...
\begin{lstlisting}
    ...
    cylc control --system userguide-1 --stop
\end{lstlisting}
