...
The first listing below shows the config file for the 'simple-0' example
system with all documentation stripped out, so that the configurable
items are are easy to see. Following that is a listing of the fully
documented file. 

\lstset{language=Python}
{
\color{Brown}
\lstinputlisting{system_config.py.stripped}
}

\pagebreak
And here is the fully documented version of the example system config file.
...
{
\color{Brown}
\lstinputlisting{full-template.def.stripped}
}
