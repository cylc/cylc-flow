...
with cylc. First the system must be divided into distinct {\bf tasks}
(Section~\ref{sec:tasks}) with clearly defined {\bf prerequisites} and
{\bf outputs} (Section~\ref{sec:requisites}). Then a simple {\bf task
definition file} must be written for each task
(Section~\ref{sec:taskdef}), and finally some {\bf task scripts} to run
the real system tasks may need to be written
(Section~\ref{sec:scripts}).

\label{sec:tasks})
\subsection{What is a Task?}

A task is any group of one or more processes to be treated as a single
entity for scheduling purposes, with clearly defined prerequisites and
outputs. 

For example, a task called ``weather_forecast'' might encompass
preprocessing of atmospheric observations, data assimilation,
atmospheric model forecast, and postprocessing of model output.
However, it would be better to split this into four separate tasks. Cylc
can handle large numbers of tasks and works out all dependencies itself
at run time, so there are definite advantages to fine-graining,

\begin{itemize}
    \item a system that is more modular and transparent
    \item after failures you can rerun just the process(es) that failed
    \item the same external script (e.g.\ for file retrieval) can often
        be invoked, with different arguments, for multiple tasks.
\end{itemize}

The a special {\bf task
definition file} written for each task and, if necessary, {\bf task
scripts} written to execute the external tasks. 
...
Cylc's scheduling algorithm works by matching unsatisfied prerequisites
with completed outputs (Section~\ref{sec:algorithm}).  These {\bf
prerequisites and outputs are just literal text strings}, each
representing a {\bf message} from a running task reporting that some
important (scheduling-relevant) event has occurred. For example, the
message ``file foo-2009101218.nc ready'' could be registered as an
output by the task that generates said file (and reported by it when the
event has occurred), and as a prerequisite by any tasks that require it
as input.  A task considers a registered output ``completed'' if it has
received a matching message from the external task that it represents.
Similarly, a task considers a registered prerequisite ``satisfied'' if
it has been told by any other task in the system that it has a completed
output to match the prerequisite. 

Prerequisites and outputs typically pertain to the completion of a file,
or a group of files, but tasks can also trigger off any other event that
a can conceiveably be reported: database interactions, downloaded
from a network, copying, archiving, and so on.
