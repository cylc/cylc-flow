\begin{abstract}
    ...
    task proxy objects to resolve dependencies amongst themselves, 
    within and between forecast cycles\footnote{For our
    ...
    other tasks.} so that correct scheduling emerges naturally at run
    time.  Cylc does not group tasks artificially by forecast cycle, and
    its task proxies are individually self-spawning (i.e.\ there is no
    ``global suite forecast cycle'') so that tasks from multiple
    cycles can run at once to the full extent allowed by the
    real dependencies in the suite. This matters in
    ...
    intervention, over major failures that require omitted cycles in
    ...
    line interfaces, and many advanced userspace features including job
    databases, a fast simulation mode, suite validation, dependency
    graph plotting, centralised alert hooks for failures and timeouts,
    and cryptographic suite security.
