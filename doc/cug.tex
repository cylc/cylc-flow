...
the first registration path component for suites in the central
...
database suite registration paths, with hyphens (because `.' is the registration
path delimiter). Type \lstinline=cylc db print --help= to see what the
...
paths incorporate the suite registration path (this is the default
...
Cylc commands target particular suites via {\em registration paths} in a
{\em suite database}, so that you don't need to remember and continually
refer to the actual location of the suite definition on disk. A suite
registration path is a hierarchical name akin to a directory path but
...
Note that {\em registration paths are entirely virtual}: groups do not
...
Currently you cannot explicitly indicate a group path on the command
...
exist yet unless you specify the entire target registration path. 
...
\subsection{Special Suite.rc Variables}
