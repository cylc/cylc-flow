...
but graphviz itself has not been installed properly (or if the
...
This is just a matter of unpacking the new cylc release, copying the old 
central database into the new cylc installation,\footnote{
The central database is currently associated with a particular cylc
installation - it is held in \lstinline=$CYLC_DIR/CDB=.}
and re-running \lstinline=cylc admin create-cdb= to upload the new
example suites. 
...
[tasks]
    [ [foo]]
        manual completion = True  # task-specific setting
\end{lstlisting}
