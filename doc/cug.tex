...
Suite registration creates the suite run directory (a directory with the suite
name under \lstinline=~/cylc-run/= by default), and populates it with
authentication files and a symbolic link to a suite definition directory.  Cylc
commands that parse suite definition files can take the file path or the suite
name as input. Commands that interact with running suites have to target the
suite by name.
\lstset{language=transcript}
\begin{lstlisting}
