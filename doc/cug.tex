...
Some batch systems have considerable
delays between the time when the job completes and when it writes the job logs
in its normal location. If this is the case, you can configure retry delays for
job log retrieval by setting some
\lstinline=[[[remote]]]retrieve job log retry delays=.  E.g.:
...
[runtime]
    [[hello]]
        ...
        [[[remote]]]
            ...
            retrieve job log retry delays = PT10S, PT1M, PT3M
\end{lstlisting}

Finally, if the disk space of the suite host is limited, you may want to
restrict the maximum sizes of the job log files to retrieve by setting
\lstinline@[[[remote]]]retrieve job log max size=SIZE@. The value of SIZE can
be anything that is acceptable by the \lstinline@--max-size=SIZE@ option of the
...
\lstinline=[[[events]]]mail events= to specify a list of events when
...
By default, the emails will be sent to the current user with \lstinline=from:=
set to \lstinline=notifications@$(hostname)= using an SMTP server at
\lstinline=localhost:25=. These can be configured using the settings
\lstinline=[[[events]]]mail to= (list of email addresses),
\lstinline=[[[events]]]mail from= and \lstinline=[[[events]]]mail smtp=.
...
The value of each setting should be a set of shell scripts or shell script
templates (see below).

Alternatively, task event handlers can be specified using the
\lstinline=[[[events]]]handler= and the \lstinline=[[[events]]]handler events=
settings, where the former is a set of shell scripts or shell script templates
(see below) and the latter is a list of events when these commands should be
invoked.
...
N.B. The handler commands is equivalent to
\lstinline="echo '!!!!!EVENT!!!!!' '%(event)s' '%(suite)s' '%(id)s' '%(message)s'"=.
