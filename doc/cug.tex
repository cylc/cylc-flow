...
\section{Dynamic Vs Static Cycling}

An atmospheric model (say) may need to be cycled for the following reasons:
\begin{myitemize}
    \item In operational forecasting it may need to wait on real time data
        before each new run.
    \item Job run length limits on compute hosts may dictate that single long
        runs are split into multiple shorter runs. (This also makes
        incremental post-processing easier.)
\end{myitemize}
There are several ways to do this in principle (and cylc can do them all):
\begin{myitemize}
    \item {\em Dynamic Cycling:} A minimal workflow - everything that needs to
        be repeated - is repeatedly extended as required, as the suite runs.
        This could stop at a final cycle point, or {\em it could continue
        indefinitely}.  The task names are always the same, but they have a
        different cycle point each time they run. For example, a yearly-cycling
        task called \lstinline=model= might repeat with cycle points
        \lstinline=2021=, \lstinline=2022=, etc. As explained in the
        introduction, dynamic cycling was traditionally done by repeating
        entire single-cycle workflows sequentially, but cylc does it without
        any hard cycle boundaries.
        \begin{myitemize}
            \item {\em Dynamic cycling in cylc} is configured by associating
                workflows with general ISO 8601 recurrence expressions, for any
                date-time (or integer) sequence.  Multiple cycling sequences
                can be used at once.
        \end{myitemize}
    \item {\em Static Cycling:} If the workflow is finite and not too long we
        could statically define it from start to finish as a single monolithic
        workflow. This is how a workflow engine that does not know about
        cycling would have to do it. The underlying model jobs are
        still repeated, but there is no cycling as far as the workflow engine
        is concerned: each job is launched by a different task, and each task
        is predefined from the start. For example, task
        \lstinline=model_runA= might represent the
        \lstinline=2020= run, \lstinline=model_runB= the \lstinline=2021= run,
        and so on. Or the tasks might be called \lstinline=model_1=,
        \lstinline=model_2=, etc., with the integer values mapping to the
        date-time cycle points needed by the jobs. The task names could
        even have the actual cycle point date-times hard-coded, such as
        \lstinline=model_2020=.  {\em It is not possible to continue a static
        workflow indefinitely}.
        \begin{myitemize}
            \item {\em Static cycling in cylc} can be configured by writing out
                every single task individually, or by expanding {\em suite
                parameters} to automatically generate each task (statically,
                before the scheduler starts up), or by doing the same (again,
                statically) with Jinja2 loops in the suite definition.

        \end{myitemize}

    \item {\em Mixed Dynamic/Static Cycling:} Both static and dynamic cycling
        can be combined in the same suite, although there are few good reasons
        to do so.  For example, 6-hourly cycling could be achieved with four
        static ``chunks'' within a daily dynamic cycle (c.f. a 6-hourly dynamic
        cycle).
\end{myitemize}

Dynamic cycling (which we normally just refer to as ``cycling'') is the most
powerful and flexible cycling method. It is the only good way to run large
operational suites that need to continue running indefinitely, and it has
performance advantages too (cylc doesn't need to manage and monitor {\em all}
the tasks in the entire workflow at once).  Nevertheless static cycling should
be understood, particularly for context and comparison with workflow engines
that do not know about cycling.
...
parameterized task names over lists of parameter values.  Uses include:
\begin{myitemize}
    ...
    \item splitting a model run into steps or ``chunks`` without dynamic cycling
\end{myitemize}
...
Parameter values can be lists of strings, or integer ranges (both bounds included):
\begin{lstlisting}
[cylc]
    [[parameters]]
        obs = ship, buoy, plane
        run = 1..5  # 1, 2, 3, 4, 5
\end{lstlisting}
Angle brackets denote parameterization.  For example:
\begin{lstlisting}
...
\end{lstlisting}
Note that the parameter name appears in the expanded task names for integer
parameters (e.g.\ \lstinline=model_run1= for \lstinline@run = 1@ as above),
but not for string parameters (e.g.\ \lstinline=proc_ship= for
\lstinline@obs = ship@ as above). This is to ensure the order is clear if
multiple integer parameters are used. These default expansion templates can be
overridden, however:
\begin{lstlisting}
[cylc]
    [[parameters]]
...
\begin{lstlisting}
    $CYLC_TASK_PARAM_run  # value "2"
    $CYLC_TASK_PARAM_obs  # value "ship"
\end{lstlisting}
...
\subsubsection{Parameterized Tasks In The Graph}

Using a different number of parameters on each side of an arrow in the graph
will branch or join a workflow.  For example this:
\begin{lstlisting}
    graph = "prep => model<run>"  # for run = 1, 2
\end{lstlisting}
expands to this:
\begin{lstlisting}
    prep => model_run1
    prep => model_run2
      # or equivalently:
    prep => model_run1 & model_run2
\end{lstlisting}
and this:
\begin{lstlisting}
    # for run = 1, 2 and mem = cat, dog
...
\end{lstlisting}
This is visualized by \lstinline=cylc graph= in Figure~\ref{fig-params-1}.
...
\subsubsection{Singling Out Specific Parameter Values}

Specific parameter values can be singled out in the graph and under
\lstinline=[runtime]= with this notation: \lstinline@<param_name=param_value>@.
For example, here's how to make a special task trigger off just the first of a
set of model runs:
\begin{lstlisting}
[cylc]
    [[parameters]]
...
[runtime]
    ...
    [[model<run=1>
        # special config for the first model run (if any)...
    #...
\end{lstlisting}

\subsubsection{Parameter Offsets In The Graph}

A negative offset notation \lstinline@<param_name=-1>@ is interpreted as the
previous value in the ordered list of parameter values (integer or string).
For example, to split a model run into multiple chunks with each chunk
depending on the previous one, this graph:
\begin{lstlisting}
...
Family names can be parameterized just like task names.  For instance, to
define a family for tasks that share common values of a parameter:
\begin{lstlisting}
[runtime]
    [[RUN<r>]]
    [[model<r>]]
    ...
    [[post_proc<r>]]
        inherit = RUN<r>
# result: RUN_r1 contains model_r1 and post_proc_r1, and so on.
\end{lstlisting}
...
\begin{lstlisting}
    graph = "foo => FAMILY"
\end{lstlisting}
or to trigger another task off {\em all} members succeeding (or failing etc.):
\begin{lstlisting}
    graph = "FAMILY:succeed-all => foo"
\end{lstlisting}
or to trigger another task off {\em any} members succeeding (or failing etc.):
\begin{lstlisting}
    graph = "FAMILY:succeed-any => foo"
\end{lstlisting}

If the members of \lstinline=FAMILY= were generated with parameters such as
\lstinline=member<m>=, you can just as easily trigger all members at once using
parameter notation.
\begin{lstlisting}
    graph = "foo => member<m>"
\end{lstlisting}
Similarly, to trigger another task off {\em all} members succeeding (or failing
etc.):
\begin{lstlisting}
    graph = "member<m> => foo"
\end{lstlisting}
...
For family {\em member-to-member} triggering just use parameterized members
on each side of the arrow.  For example, if
family \lstinline=OBS_GET= has members \lstinline=get<obs>= and
family \lstinline=OBS_PROC= has members \lstinline=proc<obs>= then this:
\begin{lstlisting}
...
\subsubsection{Static Chunking vs Dynamic Cycling}

Parameter expansion can be used instead of using dynamic cycling to generate
repeated task runs.  The resulting series of tasks may then be referred to as
steps or ``chunks''. The fundamental difference between these approaches is
that {\em parameter expansion is static} (parameters are expanded at start-up,
when the suite is parsed), whereas dynamic cycling extends a workflow at run
time, potentially indefinitely.

Here's a dynamic cycling workflow of two-monthly model runs for one year, with
previous-instance model dependence (e.g.\ for model restart files):
\lstset{language=suiterc}
\begin{lstlisting}
[scheduling]
    ...
    [[dependencies]]
        [[[R1]]]  # Run once, at the initial point.
            graph = "prep => model"
        [[[P2M]]]  # 2-month intervals between the initial and final points.
...
For this simple case both suites achieve the same result. Scheduling efficiency
is more or less the same (because dynamic cycling generates continuous
workflows with no global cycle boundaries in cylc - see \ref{HowCylcWorks}).
Special behaviour at the start or end of a workflow, or anywhere in between,
can be added in both cases. In general, however, dynamic cycling is more
flexible and powerful for the following reasons:
\begin{myitemize}
    \item In the dynamic case you can use ISO 8601 recurrence expressions
        to define any date-time cycle, and cylc does the difficult date-time
        arithmetic for you.
    \item The static suite must be finite in extent (and not too big) because
        every task in the entire workflow, from beginning to end, is
        predefined.  In the dynamic case it makes no difference if the final
        cycle point is far in the future, or if there is no final point at all.
    \item Dynamic cycling is also better for suite visualization and monitoring.
        \begin{myitemize}
            \item Every task in the entire static workflow is necessarily
                visible at once, which is a problem in large or long suites. In
                the dynamic case you can visualize as little or as much of it as
                you like, and at run time you'll only see cycle points that are
                currently active.  The two workflows are shown together in
                Figure~\ref{fig-eg2}.
            \item In the dynamic workflow successive instances of the same task
                have the same name and are labelled with their proper date-time
                cycle points.  In the static workflow successive instances of
                the same task have different names that incorporate the integer
                chunk index (i.e.\ the expanded parameter).
        \end{myitemize}
\end{myitemize}
...
Dynamic cycling and static chunking can both be used in the same suite. Here's
a yearly dynamic cycle with four static chunks within each cycle:
...
Note the inter-cycle trigger that connects the first chunk in each cycle to
the last chunk in the previous cycle.  Of course it would be easier to use
a three-month dynamic cycle:
\begin{lstlisting}
[scheduling]
    initial cycle point = 2020-01
    [[dependencies]]
...
There is at least one genuine use-case for mixed static and dynamic cycling,
however.  Consider a dynamic date-time cycling workflow of model runs that
can take too long to run (and get killed for exceeding a wall clock limit), or
not, depending on which of several platforms is available to run on at the
time.  This could be handled without changing the date-time cycling structure
of the whole suite, by splitting each run into a variable number of short
chunks and using more chunks on the slower platform.

Finally, there is one other feature of static chunking that can provide a small
advantage.  In dynamic cycling cylc automatically ignores any dependence on
cycle points earlier than the suite initial point (this isn't strictly
necessary but it does simplify scheduling configuration a little).  So this:
\begin{lstlisting}
    graph = "model[-P1D] => model"
\end{lstlisting}
simplifies at the suite initial cycle point to this:
\begin{lstlisting}
    graph = "model"
\end{lstlisting}
Similarly, parameter offsets are ignored if they extend beyond the start of the
list of parameter values.  So this:
\begin{lstlisting}
    graph = "model<chunk-1> => model<chunk>"
\end{lstlisting}
...
\begin{lstlisting}
    graph = "model_chunk1"
\end{lstlisting}
There is a parameter offset cutoff for every suite parameter list, but there is
only one cycle point cutoff at the suite initial cycle point.  Because of this,
if you want to replicate a workflow over several runs {\em with different start
points} this simplification helps with all of the runs in the static case, but
only one of them in the dynamic case - namely the one that starts at the suite
initial cycle point.  In the future we plan to provide this kind of
simplification at the start of any date-time sequence, not just the suite
initial point.  It will be optional though, because delayed-start cycling tasks
sometimes need to be triggered off earlier tasks.
