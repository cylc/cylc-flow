...
private cycle id and spawn their own successors when the time is
...
be populated with tasks with many different cycle ids. The task pool
...
ids (times) independently spawning successors at the next valid cycle id for
...
of other tasks with other cycle ids running, or not, at the time.
...
determined entirely by the suite definition, and a {\em cycle id} which is
...
sequences. A cycling task may run at each cycle id in a given sequence. For
example, a sequence might be a set of date/times every 6 hours starting from a
particular date/time. A cycling task may run for each date/time in that
sequence.
...
a sequence can be defined with only one valid id, in which case a task
...
be the initial cycle time, so that our extra tasks and dependencies apply
...
use the initial cycle time by default, which is what we want. We've also missed
...
At the initial cycle time, \lstinline=foo= will depend on the
...
However, in our initial cycle time example, the dependence on
\lstinline=foo.20130807T1200= will be ignored, because that task's cycle time
is earlier than the suite's initial cycle time and so it cannot run. This
means that the initial cycle time dependencies for \lstinline=foo= actually 
look like:
\lstset{language=suiterc}
\begin{lstlisting}
...
times with hours matching $00$ or $12$. $time$ is implicit because this keeps
graphs clean and concise, given that the majority of tasks will typically
depend only on others with the same cycle time. Cycle time offsets can only
appear on the left of a pair, because a pairs define triggers for the right
task at cycle time T.  However, \lstinline@A => B[-PT6H]@, which is illegal,
can be reformulated as a {\em future trigger} \lstinline@A[+PT6H] => B@ (see
...
initial cycle time". Cylc allows you to start (or end) at any particular time,
...
[scheduling]
    [[dependencies]]
        ...
        # Repeat once at the initial cycle time 
        [[[ R1 ]]]
            graph = "cold_foo => foo"

        # Repeat every 1 day starting at the initial cycle time
        [[[ P1D ]]]
            graph = "daily_bar => daily_baz"

        # Repeat every 5 minutes starting at the initial cycle time
        [[[ PT5M ]]]
            graph = "frequent_qux => frequent_wibble"

        # Repeat every 2 weeks starting at 00:00 after the initial cycle time
        [[[ T00/P2W ]]]
            graph = "bidiurnal_wobble => bidiurnal_wubble"

        # Repeat every month, starting 5 days after the initial cycle time
        [[[ +P5D/P1M ]]]
            graph = "monthly_offset_fred => monthly_offset_flob"

        # Repeat once at 06:00 after the initial cycle time
        [[[ R1/T06 ]]]
            graph = "delayed_cold_corge => corge"

        # Repeat 3 times, every day at 08:30 after the initial cycle time
        [[[ R3/T0830 ]]]
            graph = "triple_grault => triple_garply"

        # Repeat 3 times, every month at 00:00 on the first of the month after the initial cycle time
        [[[ R3/01T00 ]]]
            graph = "triple_waldo => triple_xyzzy"

        # Repeat 5 times, every month starting on Monday following the initial cycle time
        [[[ R5/W-1/P1M ]]]
...
be relative to the initial cycle time. For example, \lstinline=T00= by itself
would mean the next occurrence of midnight that follows, or is, the initial cycle time.
Entering \lstinline=+PT6H= would mean 6 hours after the initial cycle time.
Entering \lstinline=-P1D= would mean 1 day before the initial cycle time.
Entering no information for the {\em date-time} implies the initial cycle time
itself.
...
bounded by the final cycle time if given.
...
behaviour near the end of the suite, at the final cycle time. We can
...
[scheduling]
    [[dependencies]]
        ...
        # Repeat once at the final cycle time 
        [[[ R1//+P0D ]]]
            graph = "cold_foo => foo"

        # Repeat 5 times, every 1 day, ending at the final cycle time
        [[[ R5/P1D ]]]
            graph = "daily_bar => daily_baz"

        # Repeat every 2 weeks ending at 00:00 following the final cycle time
        [[[ P2W/T00 ]]]
            graph = "frequent_qux => frequent_wibble"

        # Repeat every 1 day ending at 00:00 following the final cycle time
        [[[ R//T00 ]]]
            graph = "bidiurnal_wobble => bidiurnal_wubble"
\end{lstlisting}
\lstset{language=transcript}
