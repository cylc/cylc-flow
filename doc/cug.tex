...
\subsection{New Features Not Yet Covered In the Quick Start Guide}

\begin{myitemize}
\item  One-off asynchronous tasks (no associated cycle time).
\item  Repeating asynchronous tasks for parallel processing of satellite passes.
\end{myitemize}

Asynchronous tasks, and their new example suites, are partially
documented in Section~\ref{AsynchronousTasks}.
...
\section{Asynchronous Tasks (No Cycle Time)}
\label{AsynchronousTasks}

{\em Documentation of this new feature (post version 3.4.0) will
eventually be integrated into the preceding sections of the User Guide}.
...
\subsection{One-off Asynchronous Tasks}
...
\subsubsection{Comparison with One-off Non-asynchronous ``startup'' Tasks}

One-off non-asynchronous startup tasks run only when a cycling suite is
...
The distinction between cold and warm starting is only meaningful for
cycling tasks, however, so {\em one-off asynchronous tasks run every
time a suite is started} (restarts just resurrect the prior suite state, as
usual). Thus, while one-off asynchronous tasks can precede cycling
tasks in the same suite, as shown above, it may be that non-asynchronous 
startup tasks are more appropriate for this purpose, and that one-off
asynchronous tasks are best used in constructing entirely non-cycling
suites.

\subsubsection{One-off Asynchronous Example Suite}

See suite \lstinline=[admin]:examples:async_oneoff=

\subsection{Repeating Asynchronous Tasks}
...
\subsubsection{Repeating Asynchronous Example Suite}
