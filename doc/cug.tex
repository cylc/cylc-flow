...
on the remote host. Similarly the remote suite directory gives task
access to suite files via \lstinline=$CYLC_SUITE_DEF_PATH= on the remote
host and to the suite bin directory via \lstinline=$PATH=. If a remote
suite definition directory is not given the local path will be used 
with the local user's home directory, if present, substituted for the
...
from the suite until the job submissions completed). This problem was
revealed when network connection timeouts were introduced in cylc-4.5.0
- submitting many tasks at once, in sequence, could take quite some time
and the first tasks to begin running could suffer messaging timeouts and
fail as a result.

From cylc-4.5.1 all tasks ready at the same time are submitted in
parallel, which is very much faster. Additionally, to be safe we have
reverted to having no network connection timeouts by default, but you can
configure one if you like in the suite definition (which applies to
connections from running tasks) or on the command line (to override a 
suite timeout for suite start-up commands, and for one-off connections
by user-invoked suite-connecting commands). The downside of having no
network connection timeout set is that certain exceptional conditions
can result in connection processes hanging indefinitely. To see this,
try hitting Ctrl-Z to stop a running suite and then attempt a
\lstinline=cylc scan= or \lstinline=cylc dump SUITE=, for instance,
first with no timeout, then using the \lstinline@-t,--timeout=@ command
option.
