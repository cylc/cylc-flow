...
representations that are very concise because sections of the graph that
repeat at different hours of the day only have to be defined once.
Here's an example showing a simple task tree with dependencies that vary
at certain hours:
\lstset{language=suiterc}
\begin{lstlisting}
[scheduling]
...
# SUITE.RC
    ...
    => C"  # OK
\end{lstlisting}
(note that line continuation characters, as in other uses of them, 
must be the final character on the line, so you cannot put a trailing
