...
\lstinline=cylc -V x.y.z= or \lstinline@cylc -Version=x.y.z@. This
feature will only work properly for versions 4.5.0 and later.

\subsubsection{Available Cylc Versions}

To list available versions of cylc installed in parallel with the
invoked version, run \lstinline=cylc -V avail= or
\lstinline@cylc -Version=avail@.
...
run a sub-command that no longer exists in the initially-invoked version
of cylc will fail. Additionally, the command line must necessarily be
parsed first by the invoked sub-command in order to determine which
argument refers to the suite (the suite name is always the first
non-option argument but only sub-commands know if particular options
have their own arguments or are just logical flags, and can therefore
determine which is the first true argument); consequently any attempt to
use a command option that is illegal in the initially-invoked version of
cylc will also fail.
