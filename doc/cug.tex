...
is wasted in polling.  For tasks that may have a highly variable arrival
time, this configuration may be impractical and the external trigger
mechanism supplies an attractive alternative.  If the incoming data
are cycle-point-specific, the cycle point can be specified as part of the
message:
\lstset{language=suiterc}
\begin{lstlisting}
# suite "data-proc"
[scheduling]
    ...
    [[special tasks]]
        external-trigger = variable-wait("data arrived for $CYLC_TASK_CYCLE_POINT")
    [[dependencies]]
        [[[T00]]]
            graph = init-process => variable-wait => post-process
\end{lstlisting}

Once the variable-length waiting is finished, an external detection system
should notify the suite like:
\lstset{language=transcript}
\begin{lstlisting}
...
the new event.  The suite passphrase must be installed on triggering account.
In this case, the event will trigger for the second cycle point but not the
first because of the cycle-point matching.
