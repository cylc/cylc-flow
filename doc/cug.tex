...
\begin{figure}
    \begin{center}
        \includegraphics[width=0.6\textwidth]{graphics/png/orig/gsummary.png}
    \end{center}
...
header \lstinline=cylc.batch\_sys\_manager= module.
...
from cylc.batch\_sys\_handles.pbs import PBSHandler


class QSUBHandler(PBSHandler):
    """A user defined batch system handler."""
    DIRECTIVE\_PREFIX = "#QSUB "


BATCH\_SYSTEM\_HANDLER = QSUBHandler()
\end{lstlisting}
...
\begin{myitemize}
    ...
    \item in the \lstinline=lib/cylc/job\_sys\_handlers= directory of
