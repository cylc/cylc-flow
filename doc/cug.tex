...
bar.1              # a task with an integer cycle point %Q non-cycling task?
\end{lstlisting}
...
\end{lstlisting}
For any given cycle point \lstinline=T= in the sequence defined by the
...
cycle point to do it.  The following sections illustrate several ways of
...
valid at this single date/time cycle point. We can choose this single
date/time to be the initial cycle point date/time, so that our extra tasks and
dependencies apply at the very start of the suite.
...
use the initial cycle point date/time by default, which is what we want. We've
also missed out specifying the period - this is set by cylc to a zero interval
in this case (although given that it never repeats, it's not important).
...
\lstinline=foo.20130807T1200= will be ignored, because that task's cycle point
is earlier than the suite's initial cycle point and so it cannot run. This
means that the initial cycle point dependencies for \lstinline=foo= actually
look like:
\lstset{language=suiterc}
\begin{lstlisting}
...
\lstinline@A[+PT6H] => B@ (see Section~\ref{InterCyclePointTriggers}).
...
tasks; these have no associated cycle points and don't spawn successors
...
cycle point in cycling tasks; it increments when a repeating asynchronous
task (below) spawns.
\begin{figure}
\begin{minipage}[b]{0.5\textwidth}
\lstset{language=suiterc}
...
behaviour near the end of the suite, at the final cycle point's date/time. We
can also represent this in cylc with a collapsed form:
\lstinline=R[limit?]/[interval]=
\lstinline=R[limit?]//[date-time]=
...
on one-off synchronous start-up tasks, which have an associated cycle point even
...
# (with dependence between cycle points too)
[scheduling]
    [[dependencies]]
        graph = "prep1 => prep2"
        [[[0,12]]]
...
Section~\ref{TutInterCyclePointTriggers}; to summarize here briefly:
...
assigned a cycle point (even though they don't cycle) whereas {\em
asynchronous} tasks are not. In the first cycle point the previous-cycle trigger is
ignored and the first cycling tasks trigger off the initial tasks; subsequently
dependence on the initial tasks is ignored and the inter-cycle-point trigger
...
point can in principle have its inputs satisfied by a previous instance of
itself, {\em or} by a cold-start task with (nominally) the same cycle point.
In effect, the cold-start task masquerades as the previous-cycle-point trigger
...
don't have to be ignored by cylc after the first cycle point, so it is
...
If a newly spawned task's cycle point is ahead of the oldest non-finished
...
past any future triggers (see Section~\ref{InterCyclePointTriggers}.
...
configurable rate and offset relative to the suite's initial cycle point.
This affects the trigger time of any clock-triggered tasks in the suite,
and the length of time between cycle points if simulating ``caught up''
...
Instead, consider organising restart files by cycle point, through a file
or directory naming convention, and keep them in a simple rolling
archive (cylc's filename templating and housekeeping utilities can easily
do this for you). Then, given availability of external inputs, you can
