...
where to install it. All legal site and user config items are defined
...
\lstset{language=transcript}
\begin{lstlisting}
shell$ cylc get-site-config > $HOME/.cylc/user.rc
\end{lstlisting}
...
examples
 `-tutorial
   ...
   | `-three      inter-cycle-point dependence + an asynchronous task
   `-oneoff
...
by cylc site and user config files, as shown above in~\ref{CUI}.
...
( perl -e "setpgrp(0,0);exec(@ARGV)"
  /home/oliverh/cylc-run/tut.oneoff.basic/log/job/1/hello/01/job </dev/null
    1>/home/oliverh/cylc-run/tut.oneoff.basic/log/job/1/hello/01/job.out
    2>/home/oliverh/cylc-run/tut.oneoff.basic/log/job/1/hello/01/job.err &
    echo $!; wait )
'Finished'
...
directory} where later they can be viewed alongside the job output logs. They
...
(echo "perl -e \"setpgrp(0,0);exec(@ARGV)\"
  /home/oliverh/cylc-run/tut.oneoff.jobsub/log/job/1/hello/01/job
    1>/home/oliverh/cylc-run/tut.oneoff.jobsub/log/job/1/hello/01/job.out
    2>/home/oliverh/cylc-run/tut.oneoff.jobsub/log/job/1/hello/01/job.err"
    | at now)
'Finished'
...
# ...
ssh -oBatchMode=yes wrh-1.niwa.co.nz 'test -f /etc/profile && . /etc/profile
  1>/dev/null 2>&1; test -f $HOME/.profile && . $HOME/.profile 1>/dev/null 2>&1;
  (mkdir -p $HOME/cylc-run/tut.oneoff.remote/log/job/1/hello/01 ||
   mkdir -p $HOME/cylc-run/tut.oneoff.remote/log/job/1/hello/01) &&
   cat >$HOME/cylc-run/tut.oneoff.remote/log/job/1/hello/01/job.tmp &&
   mv $HOME/cylc-run/tut.oneoff.remote/log/job/1/hello/01/job.tmp
     $HOME/cylc-run/tut.oneoff.remote/log/job/1/hello/01/job &&
   chmod +x $HOME/cylc-run/tut.oneoff.remote/log/job/1/hello/01/job &&
   rm -f $HOME/cylc-run/tut.oneoff.remote/log/job/1/hello/01/job.status &&
   ( perl -e "setpgrp(0,0);exec(@ARGV)"
     $HOME/cylc-run/tut.oneoff.remote/log/job/1/hello/01/job </dev/null
       1>$HOME/cylc-run/tut.oneoff.remote/log/job/1/hello/01/job.out
       2>$HOME/cylc-run/tut.oneoff.remote/log/job/1/hello/01/job.err &
       echo $!; wait)' < 
  /home/oliverh/cylc-run/tut.oneoff.remote/log/job/1/hello/01/job
\end{lstlisting}
...
sequences. A cycling task may run at each cycle point in a given sequence
(cycle). For example, a sequence might be a set of date-times every 6 hours
starting from a particular date-time. A cycling task may run for each
...
a sequence can be defined with only one valid cycle point - in that case, a
task belonging to that sequence may only run once.
...
(times) ending in $00$ or $12$ hours, between specified initial and final cycle
...
parallel out to the {\em runahead limit}, and each \lstinline=bar=
will trigger off the corresponding instance of \lstinline=foo= at the
same cycle point.  The runahead limit prevents uncontrolled spawning of
...
\begin{myitemize}
    ...
    \item The suite runahead limit is configurable - see~\ref{RunaheadLimit}.
    \item Other date-time cycling is also supported (multihourly, daily,
 monthly, yearly...) - see~\ref{AdvancedCycling}.
\end{myitemize}
...
previous instance \lstinline=foo[-PT12H]=. Figure~\ref{fig-tut-two} shows
...
Note that the \lstinline=[[[T00,T12]]]= (at 00 and 12 hours) syntax could be
written as \lstinline=[[[PT12H]]]= (repeat every 12 hours, starting at the
initial cycle point) provided that the initial cycle point was always at 00 and
12 hours.
...
At the initial cycle point, \lstinline=foo= will depend on the
...
different cycle points.

\paragraph{One-off (Non-Cycling) Tasks}
...
cycle points for which the subsequent graph section is valid.
...
# (no dependence between cycle points here)
[scheduling]
    [[dependencies]]
        [[[T00,T12]]]
            graph = "foo => bar & baz => qux"
...
[scheduling]
    [[dependencies]]
        ...
        # Repeat 3 times, every month at 00:00 on the first of the month after the initial cycle point
        [[[ R3/01T00 ]]]
            graph = "triple_waldo => triple_xyzzy"

        # Repeat 5 times, every month starting on Monday following the initial cycle point
        [[[ R5/W-1/P1M ]]]
...
For example, we can write our suite like this:
...
[scheduling]
    ...
    [[dependencies]]
        [[[R1]]]
            graph = "prep"
        [[[R1/T00]]]
            graph = "prep[^] => foo"   # ^ implies prep at the initial cycle point
        [[[R1/T12]]]
            graph = "prep[^] => baz"   # ^ is initial cycle point, as above
        [[[T00]]]
            graph = "foo[-P1D] => foo => bar"
        [[[T12]]]
            graph = "baz[-P1D] => baz => qux"
...
$CYLC_TASK_LOG_ROOT              # ~/cylc-run/foo.bar.baz/log/job/X.20110511T1800Z.1
...
definitions, see the Jinja2 template processor (\ref{Jinja2}).
...
in site/user config (see~\ref{task_comms_method}).  The default polling
intervals can be overridden for all suites there too
...
#QSUB -e /home/hilary/cylc-run/my.suite/log/job/a.1.1.err
#QSUB -l nodes=1,walltime=00:01:00
#QSUB -o /home/hilary/cylc-run/my.suite/log/job/a.1.1.out
#QSUB -N a.1
#QSUB -I bar=baz
#QSUB -cwd
\end{lstlisting}
...
cycle point that is later than the suite's initial cycle point. It can be
considered an inferior alternative to a restart because it may result in some
tasks rerunning.  A warm start may be required if a restart is not possible
because the suite state files were accidentally deleted (for instance). The
warm start cycle point must be given on the command line:
\lstset{language=transcript}
\begin{lstlisting}
shell$ cylc run --warm SUITE [START_CYCLE_POINT]
\end{lstlisting}
In this case the scheduler starts by loading a first instance of each 
...
are assumed not to be needed for a warm start; see the note on cold-start tasks
...
Here are the default site and user config items relevant to task state 
...
A connection timeout can be set in site and user config files 
...
In the case of {\em cold starts only} the initial cycle point will also
be passed through to task execution environments as
\lstinline=$CYLC_SUITE_INITIAL_CYCLE_POINT=.  The intended use of this
variable is to allow tasks to determine whether they are running in the
initial cold-start cycle point (when different behaviour may be required) or
in a normal mid-run cycle point. {\em This is not done for warm starts}
...
previous cycle point (for each task) has been run and completed entirely.
It follows that in a warm start tasks are really in a normal mid-run
cycle point, and because no actual previous state is referenced
\lstinline=$CYLC_SUITE_INITIAL_CYCLE_POINT= gets the value
...
cold-start cycle point is stored in suite state dump files.
...
\begin{myitemize}
    ...
    \item Automatic state dump backups, named pre-intervention state dumps:
        these are used to restart a suite from a previous state of operation.
        They are mentioned in the Tutorial (\ref{Tutorial}). Watch the
    ...
    \item Handling spin-up processes via temporary tasks and adding
        ...
        the initial cycle point can be in the past; and (b) you can give
        a final cycle point after which the task will be eliminated from
        the suite.
...
run, the only cycle point that can subsequently run is the next one. This
is dangerous because if, accidentally or otherwise, the task runs for the
wrong cycle point, its restart files will be corrupted such that the correct
cycle point can no longer run (probably necessitating a cold-start).
Instead, consider organising restart files by cycle point with a file
or directory naming convention, and keep them in a simple rolling
archive. Then, given availability of external inputs, you can
easily rerun the task for any cycle point still in the restart archive.
...
\subsection{Put Task Cycle Point In All Output File Paths}
...
get overwritten by the next cycle point.

The \lstinline=cylc cycle-point= command computes offsets (in hours, days,
months, and years) from a given or current date-time cycle point, and
optionally inserts the resulting computed date-time, or components of it, into
a given filename template string.
...
to call the same file transfer script or command, with task-dependent
...
environment by cylc - see~\ref{TaskExecutionEnvironment}). Then you can run
multiple instances of the suite
...
by cycle point date-time''.  Under cylc, a product generation task will
trigger as soon as its prerequisites are satisfied (i.e.\ when its input files
are ready, generally) regardless of whether other tasks with the same cycle
point have finished or have yet to run. If your product delivery or
presentation system demands that all products for one cycle point are uploaded
(or whatever) before any from the next cycle point, then be aware that this
may be quite inefficient if your suite is ever faced with catching up from a
significant delay or running over historical data.
...
generation tasks at the same cycle point, and then declare it to be sequential
(see~\ref{SequentialTasks}.

\subsection{Clock-triggered Tasks Wait On External Data}

All tasks in a cylc suite know their own private cycle point, which is usually
a date-time, but most don't care about the wall clock time - they just run
when their prerequisites are satisfied. The exception to this is
{\em clock-triggered} tasks, which wait on a wall clock time expressed as an
offset from their own cycle point date-time as a real-life date-time, in
addition to any other prerequisites. The usual purpose of these tasks is to
retrieve real time data from the external world, triggering at roughly the
expected time of availability of the data. Triggering the task at the right
time is up to cylc, but the task itself should go into a check-and-wait loop
in case the data is delayed; only on successful detection or retrieval should
...
operation as required.
    ...
    \subsection{Factor Out Common Configuration}
        ...
        filters can be used to make this safer, however
        (see~\ref{EnvironmentFilter}).
...
\end{lstlisting}
      ...
      \subsection{Use The Graph For Scheduling}
          ...
          to change its behaviour significantly from one cycle point to
          the next, consider that the graph is usually the proper place
...
[SECTION]
    ...
    a very very long item = value2
\end{lstlisting}
This does not show nesting level clearly, and long items push everything off to
the right. The following layout does preserves proper indentation, but the
whole block will need reformatting if an even longer item is added (or the
current longest one removed or renamed) in the future:
\lstset{language=suiterc}
\begin{lstlisting}
[SECTION]
    a short item          = value1
...
numeric form before cylc 6. The numbers implied a number of seconds,
minutes (most common), or hours, depending on the setting.
...
Nearly all timeouts/delay numbers in cylc were in minutes - only
\lstinline=[runtime][[my_task]][[[suite state polling]]]interval= and
\lstinline=[runtime][[my_task]][[[simulation mode]]]run time range= involved
seconds, and \lstinline=[scheduling]runahead limit= was in hours.
\lstinline=[scheduling]runahead limit= is a special case which is discussed in
\ref{cylc-6-migration-runahead-limit}.
...
\lstinline=[runtime][[my_task]]retry delays= & 2*30, 360, 1440 & 2*PT30M, PT6H, P1D \\
...
See Section \ref{runahead limit}.

The \lstinline=[scheduling]runahead limit= option was written as a number of
hours in pre-cylc-6 suites. This number is now in ISO 8601 format for date-time
...
See Section \ref{initial cycle point}.
...
Start-up tasks and mixed-asynchronous tasks are removed from cylc 6, and 
using cold-start tasks is no longer a preferred option. Instead, you should
use the initial/repeat-once notation as detailed in Sections
\ref{initial-non-repeating-r1-tasks} and \ref{AdvancedStartingUp}.

For genuinely repeating tasks with hours-based cycling (not start-up or
asynchronous or cold-start) the syntax has only minor changes:
