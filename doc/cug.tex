...
$ cylc db print --central
 admin:examples:CUG_1    "Cylc User Guide Example 1"  ~admin/cylc/examples/CUG_1
 admin:examples:CUG_73   "Cylc User Guide <SNIP>"     ~admin/cylc/examples/CUG_7.3
 admin:examples:CUG_74   "A two-task test suite"      ~admin/cylc/examples/CUG_7.4
 admin:examples:CUG_QS1 "Cylc User Guide <SNIP> "    ~/dmin/cylc/examples/CUG_QS/one
 admin:examples:CUG_QS2 "Cylc User Guide <SNIP>"     ~admin/cylc/examples/CUG_QS/two
 admin:examples:CUG_QS3 "Cylc User Guide <SNIP>"     ~admin/cylc/examples/CUG_QS/three
...
This is just a matter of unpacking the new cylc release and re-running
\lstinline=cylc admin create-cdb= to replace the old example suites with
the new ones in the central suite database. 

Note however that the central database is currently associated with a 
particular cylc installation - it i held in \lstinline=$CYLC_DIR/CDB=.
Consequently you need to copy the old CDB into the new cylc installation
prior to running \lstinline=cylc admin create-cdb=, in order to retain
all suites registered in the old central database.
...
$ cylc database print --central | grep Quick
 admin:examples:CUG_QS1 "<SNIP>Quick Start Example 1" ~admin/cylc/examples/CUG_QS/one
 admin:examples:CUG_QS2 "<SNIP>Quick Start Example 2" ~admin/cylc/examples/CUG_QS/two
 admin:examples:CUG_QS3 "<SNIP>Quick Start Example 3" ~admin/cylc/examples/CUG_QS/three
DONE
\end{lstlisting}
%\lstset{ basicstyle=\color{basic}\footnotesize\ttfamily }
...
in your \lstinline=.profile= login script, as described in 
...
Matched:
   admin:examples:CUG_1
   admin:examples:CUG_73
   admin:examples:CUG_74
...
REGISTERED examples:CUG_1 "<SNIP> Example 1" ~oliverh/suites/examples/CUG_1
Copying suite definition
REGISTERED examples:CUG_73 "<SNIP> Graph Example" ~oliverh/suites/examples/CUG_73
Copying suite definition
REGISTERED examples:CUG_74 "A two-task test suite" ~oliverh/suites/examples/CUG_74
Copying suite definition
...
\lstinputlisting{../examples/CUG_QS/one/suite.rc}
...
FILE: /home/admin/cylc/examples/CUG_QS/three/suite.rc
...
\lstinline=examples:CUG_73= (unfortunately the suite was registered under 
...
\begin{figure}
    ...
    \end{center}
    \caption[Suite \lstinline=examples:CUG_73= Validation.]{\small Suite
    \lstinline=examples:CUG_73= Validation}.
...
The example suites all use registration specific I/O paths so that
you can run multiple instances of them at once without interference.
...
cylc lockserver (see Appendix~\ref{TheCylcLockserver}) so this will not
be difficult. In the meantime,
    ...
    \item any other suite-related documentation, control files, etc.
        \begin{myitemize}
            ...
            \item files in the suite definition directory can be
                ...
                through the environment variable \lstinline=$CYLC_SUITE_DEF_PATH=
...
[environment]
    ...
    WORKSPACE = /$TMPDIR/$CYLC_SUITE_REG_PATH
...
\lstset{language=bash}
\begin{lstlisting}
$ cylc graph examples:CUG_73 2011052200 2011052206
...
\begin{myitemize}
    ...
    \item {\bf conditional expressions}: \lstinline@ (A | B) & C => D@ 
...
Task families are defined in a special section of the suite.rc file, and 
...
Task family members can be defined by an explicit list of task names, as
above, or a list-generating Python expression. See
Appendix~\ref{SuitercTaskFamilies} and the ``generator'' example suites
for details.

If a family's member tasks are all very similar, they can be defined 
all at once under a single task definition section. See
Appendix~\ref{SuitercTasks} and the ``generator'' example suites
for details.
...
[environment]             # GLOBAL ENVIRONMENT
    ...
    PREV_CYCLE = $( cylc util cycletime -s 12 )
[tasks]
    [[A]]                 # TASK A PROPERTIES
...
\subsubsection{Defining Multiple Tasks At Once}
\label{MultiTaskDef}

Groups of similar tasks that differ only in a few configuration
details can be defined in single suite.rc task definition sections.
Potential applications include groups of similar obs processing tasks,
and forecasting ensembles.

Groups of similar tasks are likely to be candidates for treatment as
task families, but they don't have to be.

For tasks families, the task names can be defined by an explicit list
of names, or a generator expression, under \lstinline=[task families]=,
then the family name can be used in place of individual task names 
under the \lstinline=[tasks]= section, and subsequent configuration
items will then apply to each member of the group. For non-family 
groups, the list of explicit names or the generator expression can 
take the place of the individual task names under \lstinline=[tasks]=.
Finally, because at least a few configuration items will need to vary
across the otherwise similar tasks, cylc replaces the variable
\lstinline=$(TASK)= with the actual task name, at task definition time, 
in all configuration items. 

The annotated example suites,
\begin{myitemize}
    \item \lstinline=examples:generator_explicit=
    \item \lstinline=examples:generator_expression=
\end{myitemize}
show exactly how to use this new feature.  See also Appendix~\ref{SuitercTasks}.
...
\begin{lstlisting}
$CYLC_SUITE_REG_NAME   # foo.bar.baz (for example)
...
access to suite files via \lstinline=$CYLC_SUITE_DEF_PATH=, and to the
...
Section~\ref{ATaskDefinitionExample} (also registered as 
\lstinline=examples:CUG_74= in the central suite database):

\begin{lstlisting}
% cylc submit --dry-run examples:CUG_74 A%2011052400
DRY RUN: create the task job script and show how it would be executed.
 ...
 > JOB SUBMISSION: /tmp/cylc-A%2011052400-jfHtqy </dev/null 1>
     /home/oliverh/CylcJobLogs/examples/CUG_74/A%2011052400-DuaPUt.out 2>
     /home/oliverh/CylcJobLogs/examples/CUG_74/A%2011052400-DuaPUt.err &
\end{lstlisting}
...
export CYLC_SUITE_DEF_PATH="/home/oliverh/cylc/examples/CUG_7.4"
export CYLC_SUITE_REG_NAME="examples:CUG_74"
export CYLC_SUITE_REG_PATH="examples/CUG_74"
...
\begin{lstlisting}
# SUITE.RC
job submission log directory = $HOME/CylcJobLogs/$CYLC_SUITE_REG_PATH
\end{lstlisting}
...
{\em Documentation here will eventually be integrated
into the preceding sections of the User Guide}.

\subsection{Asynchronous Tasks}
\label{AsynchronousTasks}

{\em Introduced at version $3.4.0$}

Cylc now has {\em one-off asynchronous tasks} for constructing one-off
suites of tasks that have no associated cycle time, and 
{\em repeating asynchronous tasks} for processing satellite
passes (these propagate a special ``data ID'' through the task tree, 
and can process consecutive passes in parallel if the data happens to 
come in quickly). 

Asynchronous tasks have an arbitrary integer tag instead of a cycle
time. Cylc can manage both asynchronous and cycling tasks within the
same suite.

\subsubsection{One-off Asynchronous Tasks}

These tasks are defined with no sub-heading under [dependencies] 
...
In this listing two cycling tasks are made to wait on a chain of three 
one-off asynchronous tasks (they can be used in entirely non-cycling
suites, or for initial processing prior to starting the cycling tasks).

\paragraph{C.f. One-off Synchronous ``startup'' Tasks}

One-off synchronous startup tasks run only when a cycling suite is
...
they will be intended as startup tasks - so currently they will
only run in a cold start.  For completeness though, in a future release
we will make it possible to have one-off asynchronous in a cycling suite
that run even if the suite is warm started. 

\paragraph{One-off Asynchronous Example Suite}

See suite \lstinline=[admin]:examples:async_oneoff=

\subsubsection{Repeating Asynchronous Tasks}

These tasks can be used to construct suites for processing asynchronous 
satellite datasets, wherein the data arrives at (effectively) random
intervals. The first task in the suite 
should be of the {\em daemon} type, which runs indefinitely watching 
for incoming data. When new data is detected it sends a message 
that matches a registered pattern containing some unique data ID. 
The daemon task proxy extracts the ID and dynamically registers a new
output, containing the actual ID, that downstream tasks can trigger off.
The downstream tasks likewise have prerequisites containing the pattern,
and when these are satisfied the actual ID is substituted
into their registered outputs. Each asynchronous task proxy passes the
ID to its real task as \lstinline=$ASYNCID= so that it can process the
correct dataset. Repeating asynchronous tasks are defined under special 
sub-headings in the [dependencies] section of the suite.rc graph, e.g.:
...
If the asynchronous datasets happen to become available in quick succession,
entire trees of asynchronous processing can run in parallel.

\paragraph{Repeating Asynchronous Example Suite}

See suite \lstinline=[admin]:examples:async_repeat=

\subsection{Defining Multiple Tasks At Once}

{\em Introduced at version $3.5.0$}

{\em This is documented in Section~\ref{MultiTaskDef}}

\subsection{Default Start And End Cycles}

{\em Introduced at version $3.4.0$}

\begin{myitemize}
\item  Suite graphing now has default start and end cycles, and you may define 
    your own default start and end cycles for graphing in the suite.rc file.
\item  Default initial and final cycle times for running a suite can now 
    optionally be defined in the suite.rc file, so you don't have to keep 
    typing them on the command line, or GUI panel, for repetitive testing.
\end{myitemize}
...
\begin{myitemize}
    ...
    \item Where to look for output - suite log, suite stdout/stderr,
        task stdout/stderr (this is documented in the Quick Start Guide)
