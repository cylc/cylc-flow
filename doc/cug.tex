...
\begin{myitemize}
    \item {\bf Items} are of the form \lstinline@key = value@.
...
\paragraph{Warm-Starting Suites}
...
suite state, it loads all cycling tasks at a given cycle time assuming
...
operation as required.
    ...
    \subsection{Factor Out Common Configuration}
        ...
        be defined only once (it may be tempting to define all
        environment variables once under \lstinline=[root]=, but
        this is analagous to overuse of global variables in programming,
        and makes it difficult to determine which variables matter to
        which tasks). Cylc supports several ways of addressing this problem:
    
        \begin{myitemize}
            \item {\em Multiple inheritance} under \lstinline@[runtime]@
                - shared items can be put into namespaces inherited by
                related tasks. This can be done over multi-level
                multiple-parent hierarchy.

            \item {\em Jinja2 variables} can be defined once and
                referenced throughout the suite definition. This is not
                as efficient as multiple inheritance when a significant
                number of items are shared by a significant number of
                tasks, but it is more efficient for single variables
                shared by just a few tasks that don't have anything else
                in common (e.g.\ the location of a shared file).  
        \end{myitemize}

        Jinja2 can also be used to avoid defining intermediate
        environment variables for the sole purpose of deriving other
        environment variables at task run time. E.g. instead of this:
\lstset{language=suiterc}
\begin{lstlisting}
[runtime]
    [[root]]
...
\end{lstlisting}
        ...
        If Jinja2 variables are occasionally needed in external scripts,
        translate them directly in environment sections:
\lstset{language=suiterc}
\begin{lstlisting}
    [[[environment]]]
...
\end{lstlisting}
    ...
    by the first-parent runtime namespace hierarchy (i.e.\ the immediate
    family a task belongs to is its first parent). Tasks should
    generally be grouped into families that reflect their purpose within
    the structure of the suite rather than by technical detail such as
    common job submission method or host. This goal often coincides
...
[runtime]
    ...
    [[obs1, obs2, obs3]]
        inherit = OBSPROC
\end{lstlisting}
    and you can demote parents from primary to secondary if necessary:
\lstset{language=suiterc}
\begin{lstlisting}
[runtime]
    [[foo]]
...
Good style is arguably a matter of taste.  That said, for collaborative
development of complex systems it is important to settle on a clear and
consistent style. You may find the following suggestions useful.

\subsection{Line Indentation}

The suite.rc file format consists of \lstinline@key = value@ items 
...
[SECTION]
    ...
    [[SUBSECTION]]
        a short item = value1          # OK
        a very very long item = value2 # OK
\end{lstlisting}
    Don't align \lstinline@item = value@ pairs on the \lstinline@=@
    character like this:
\lstset{language=suiterc}
\begin{lstlisting}
[SECTION]
             a short item = value1 # BAD
    a very very long item = value2 # BAD
\end{lstlisting}
    This does not reflect nesting level clearly, and long items 
    push everything off to the right. The following form may be
    acceptable though:
\lstset{language=suiterc}
\begin{lstlisting}
[SECTION]
...
\end{lstlisting}
    ...
    \item {\em Line comments} should be indented to the same level as the
        section or item they refer to - anything less prevents quick block
        re-indentation in text editors.
   
    \item The internal lines of {\em multiline string values} should not
        be indented as if they were suite.rc syntax in their own right -
        the indentation whitespace just ends up inside the string. In
        particular, \lstinline=command scripting=
        strings are inserted verbatim into task job scripts.  These,
        like task job logs, are made easily accessible for debugging
        purposes, and filling them with bizzarely indented code does not
        help. An example:
...
[runtime]
    [[foo]]
        command scripting =""" # BAD
            echo Hello World!
            echo Goodbye World!"""
\end{lstlisting}

    \lstinline@graph@ strings, however, have no meaning outside of the
    suite definition and can be free-form for maximum clarity.
\lstset{language=suiterc}
\begin{lstlisting}
[scheduling]
    [[dependencies]]
...
\end{lstlisting}
   ...
   \item {\em Embedded Jinja2 Code} is not part of the suite.rc syntax - 
...
\end{lstlisting}
   \end{myitemize}
    ...
    \begin{myitemize}
        \item {\em Indent line comments} to section level.

        \item Avoid {\em numbered comments} - future changes can create a
            renumbering nightmare.

        \item Avoid {\em unnecessary comments}. These just have to be   
            kept up to date with all code changes, or else the result
            - misleading comments - may be worse than having no comments
            at all.
            If context and clear item names will do, leave it at that.  

        \item Avoid full page width ``section divider'' comment lines - 
            these cause problems for editors set to shorter line widths.
    \end{myitemize}

    \subsection{Other Considerations} 
    
        \begin{myitemize}
            \item {\em Naming conventions} - consider, for example,
                using uppercase family names and lower case task names,
                so that you can tell which a namespace represents at a glance.

            \item Don't write command scripting for each task to print
                its input variables to stdout. There is a maintenance
                cost to this verbosity, and for little benefit because
                it is as easy to view task job scripts - which contain
                the variable definitions -as job output logs.

        \end{myitemize}

   \subsection{A Style Example}
