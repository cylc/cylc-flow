...
\begin{myitemize}
    \item In real time forecasting it may need to wait on new real time data
        before each run.
    \item Job run length limits on compute hosts may require single long
        runs to be split into multiple shorter runs. (This also makes
        incremental post-processing easier.)
\end{myitemize}
This section defines the terminology associated with cycling, and explains the
two main ways that a workflow engine can implement cycling.

\subsection{Cycling Terminology}

In general usage, depending on context the word ``cycle'' can refer to a
sequence of date-times, e.g.\ ``an hourly cycle''; or to a particular
date-time, e.g.\ ``the current cycle''.

Cylc terminology is more precise: a {\em cycle point} refers to a particular
...
\subsection{Ways Of Cycling: Dynamic and Static}

There are two main ways that a workflow engine could implement cycling, in
principle:
\begin{myitemize}
    \item {\em Dynamic Cycling:} A workflow could be extended to future cycle
        points as needed at run time, {\em potentially indefinitely}.  Each
        time a task runs it will be labelled with its next cycle point. For
        example, a task called \lstinline=model= might repeat yearly with cycle
        points \lstinline=2021=, \lstinline=2022=, and so on.  Dynamic cycling
        has traditionally involved repeating entire workflows sequentially, but
        cylc does it without any hard cycle point boundaries - see
        Section~\ref{HowCylcWorks}.

    \item {\em Static Cycling:} If a suite is finite in extent and not too big,
        it could be handled as single monolithic workflow so that no cycling is
        needed at run time. Each model run is represented by a different
        logical task, e.g.\ \lstinline=model_runA= might represent the
        \lstinline=2020= run, \lstinline=model_runB= the \lstinline=2021= run,
        and so on; or the tasks might be called \lstinline=model_1=,
        \lstinline=model_2=, and so on., with integer values
        mapping to the proper date-time cycle points; or even
        \lstinline=model_2020=, \lstinline=model_2021= and so on. {\em It is not
        possible to continue a static workflow indefinitely}.

    \item {\em Mixed Cycling:} Both static and dynamic cycling can be combined
        in the same suite (although there may be no good reason to do this in
        cylc!).  For example, instead of an ongoing 6-hourly dynamic cycle, we
        could have a ongoing daily dynamic cycle with four static ``chunks''
        inside each daily cycle point.
\end{myitemize}

Dynamic cycling (or simply ``cycling'') in cylc is configured by associating
workflows with general ISO 8601 recurrence expressions, for any date-time (or
integer) sequence.  Multiple cycling sequences can be used at once.   See
Section~\ref{ConfiguringScheduling} and many other parts of the User Guide.

Static cycling in cylc can be configured by defining every task instance
(for every cycle point) individually; or by expanding {\em parameterized tasks}
(Section~\ref{Parameterized Tasks}) or Jinja2 loops (Section~\ref{Jinja2}) to
do the same automatically.

Dynamic cycling is the most powerful and flexible method, and it is the only
way to run large operational suites that need to continue indefinitely (plus
cylc does the difficult date-time arithmetic for you, and it is easier to
manage and monitor an incrementally evolving non-monolithic workflow).
