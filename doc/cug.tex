...
\begin{myitemize}
    ...
    \item {\bf Pyro (Python Remote Objects), version 3.9.1$+$, latest
        tested 3.15}.  Not Pyro 4.x yet; as of mid 2011 version 3.x
        is still recommended for production use.  Pyro is used by cylc
...
Beware of old Linux distributions that come packaged with Pyro 3.9 or
earlier, which is not compatible with the new-style Python classes used
in cylc.  It has been reported that Ubuntu 10.04 (Lucid Lynx), released
in September 2009, suffers from this problem.  Attempting to run
...
version of cylc can act as a tranparent front end to any other installed
...
[runtime]
    [[A]]
        [[[outputs]]]
            upload1 = "NWP products uploaded for <CYLC_TASK_CYCLE_TIME>"
\end{lstlisting}
...
\begin{myitemize}
    \item \lstinline=<CYLC_TASK_CYCLE_TIME>= and
        ...
        optional computed positive offset) in the message strings
        registered for explicit internal (pre-completion) task outputs.
        They correspond to \lstinline=$CYLC_TASK_CYCLE_TIME= in the task
