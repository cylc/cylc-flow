...
perform the following actions by right-clicking on central suites (or
by the cylc command line):
\begin{myenumerate}
...
Anyway, you should now see the following suite.rc file in your editor: 
\lstset{language=suiterc}
\lstinputlisting{../examples/QuickStart/a/suite.rc}
\lstset{language=transcript}
(Note that if you use the vim editor you can have cylc-specific syntax
...
example suite definition it would look like this:
\lstset{language=suiterc}
\begin{lstlisting}
[runtime]
    [[GetData]]
...
\begin{figure}
    ...
    \end{center}
    \caption[Suite {\em QuickStart.a} at startup, 06 or 18 hours.]{\scriptsize
...
\begin{figure}
    ...
    hours]{\scriptsize Suite {\em QuickStart.a} at startup with an
...
\begin{figure}
    ...
    {\scriptsize Suite {\em QuickStart.a} running, showing multiple
...
\begin{myitemize}
    ...
    \item {\bf Any other sub-directories and files} - documentation,
        ...
        \begin{myitemize}
            ...
            \item Tasks access via \lstinline=$CYLC_SUITE_DEF_PATH=
...
\begin{myitemize}
    \item {\bf [cylc] } - aspects of suite configuration that are not task-related, e.g.:
        \begin{myitemize}
            \item simulation mode
            \item suite logging and state dump directories
        \end{myitemize}
    \item {\bf [scheduling] } - determines when tasks are ready to run.
        \begin{myitemize}
...
to get the suite started because A[T] depends on A[T-6] and at
...
\begin{minipage}[b]{0.5\textwidth}
    ...
    \end{center}
\end{minipage}
\caption[Dependency Graph Example 1.]{\scriptsize Dependency Graph Example 1.}
\label{fig-dep-eg-1}
\end{figure}
...
{\bf The small suite definitions used in the following subsections can
...
asynchronous tasks, as shown in Figure~\ref{fig-test4}. Alternatively,
you can use {\em one-off synchronous task} instead of asynchronous 
tasks, if it is appropriate for the initial non-cycling tasks to have 
a cycle time - see Figure~\ref{fig-test5}.
