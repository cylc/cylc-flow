...
Documentation of this new feature (since cylc-3.4.0) will eventually be
integrated into the preceding sections of the User Guide.
...
One-off asynchronous tasks perform essentially the same function as the
previously documented one-off startup tasks, except that they don't need
an associated cycle time.
...
satellite datasets (wherein the data arrives at essentially random
intervals as far as cylc is concerned). The first task in the suite 
...
They in turn have prerequisites containing the data pattern which when 
satisfied result in them inserting the matched actual ID into their 
outputs, and so on. Each asynchronous task passes the ID to its 
real task as \lstinline=$ASYNCID= so that it can process the data
as required.

Repeating asynchronous tasks are defined under special 
sub-headings in the [dependencies] section of the suite.rc graph:
