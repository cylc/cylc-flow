...
\lstinline=python setup.py install --prefix=/path/to/install/location=,
or use the \lstinline=easy_install= command to do it all in one step.
Either way Jinja2 installation requires the final installed package
location to be present in the \lstinline=PYTHONPATH= variable, so you
have to arrange for this first. You made also need to create the
installed package directory yourself first (if so, the install will 
abort and print the name of the missing directory in the error message).
...
in multiple sections, but individual dependencies must be unique across
the entire graph. Thus the following graph is valid:
\begin{lstlisting}
# SUITE.RC
[scheduling]
    [[dependencies]]
...
Currently this is one of the few remaining errors not caught by suite
validation. Instead the suite will abort on start-up with a ``duplicate
prerequisite'' error:
\lstset{language=transcript}
\begin{lstlisting}
...
As the suite.rc file is parsed, any occurrence of
\lstinline=<NAMESPACE>= in any runtime configuration item
is immediately replaced with the actual name of the namespace; and after 
inheritance processing any occurrence of \lstinline=<TASK>= is
replaced with the actual name of the task.
...
functionally equivalent to \lstinline=$CYLC_TASK_NAME=.  But in other
runtime config items that are never actually interpretred by the shell
(e.g.\ \lstinline=description=) \lstinline=<TASK>= must be used to
specialize from the list of names in the namespace section heading to a
particular task name.

{\em The {\em multidef} example suite illustrates use of these internal
variables.}
...
use one or both of [scheduling][[special task]] lists, ``tasks to
include at start-up'' or ``tasks to exclude at start-up'' (documented in 
