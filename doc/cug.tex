...
\subsubsection{Jinja2 Template Inputs And Default Values}

Jinja2 variable values can be passed in from the cylc command
line using the \lstinline=--set= and \lstinline=--set-file= options.
If a Jinja2 variable is not set to a value in-suite then a value must be
passed in for it, unless it has a default value.

For example, here's a suite that takes its initial cycle time from
the command line:
\lstset{language=suiterc}
\begin{lstlisting}
#!Jinja2
[scheduling]
    ...
    [[dependencies]]
        [[[Yearly(2010,3)]]]
            graph = "foo"
\end{lstlisting}
This will fail validation unless a value is passed in:
\lstset{language=transcript}
\begin{lstlisting}
# with no value set on the command line:
shell$ cylc val my.suite
Jinja2 Template Error
UndefinedError('START' is undefined)

# with a value set on the command line:
shell$ cylc val --set START=2010 my.suite
Suite my.suite is valid for cylc-5.4.0
\end{lstlisting}
However, if you define a default value like this,
\lstset{language=suiterc}
\begin{lstlisting}
#!Jinja2
[scheduling]
    initial cycle time = {{ START | default(2010) }}
    [[dependencies]]
        [[[Yearly(2010,3)]]]
            graph = "foo"
\end{lstlisting}
then the suite will validate (and run etc.) with no value passed in from
the command line:
\lstset{language=transcript}
\begin{lstlisting}
shell$ cylc val my.suite
...
The \lstinline=$CYLC_DIR/examples/jinja2/defaults= is another example.

{\em Warning:} suites started with template variables on the command
line do not restart with the same values set - they must be set again on
the restart command line.
...
\subsection[CYLC\_SUITE\_INITIAL\_CYCLE\_TIME]{The CYLC\_SUITE\_INITIAL\_CYCLE\_TIME variable}

For a suite {\em cold start} the initial cycle time is passed to task
execution environments as \lstinline=$CYLC_SUITE_INITIAL_CYCLE_TIME=.
This allows tasks to determine whether or not they are running in the
initial cold-start cycle, when different behaviour may be required.
This is not done by default for warm starts because a warm start is
really an implicit restart that does not reference a particular previous
suite state - so cylc can't know what the original initial cycle was and
...
\begin{myitemize}
    ...
    \item Sub-suites: to run another suite inside a task, just invoke the 
        subsuite, with appropriate start and end cycles - probably just a
