...
via the filesystem, i.e.\ the input/output file relationships embodied in
inter-task dependencies. Since cylc-4.6.0, however, tasks can also communicate 
information to other downstream tasks via an environment variable
broadcast mechanism. Variables may be broadcast to all subsequent tasks, 
or targetted specifically at a specific task, all subsequent tasks with a 
given name, or all tasks with a given cycle time:

\lstset{language=bash}
\begin{lstlisting}
# (TASK IMPLEMENTATION SCRIPTING)
# broadcast some environment variables to downstream tasks:
cylc broadcast FOO "the quick brown"              # to all tasks
cylc broadcast BAR "fox jumped" TaskA%2012080806  # to a specific task
cylc broadcast BAZ "over the"   TaskA             # to all TaskA's
cylc broadcast QUX "lazy dog"   2012080806        # to all at 2012080806
\end{lstlisting}

All current broadcast variables that target a particular task at the 
instant it is ready to be submitted will be written to its job script
after the rest of the task execution environment (broadcast variables 
...
Within suite.rc scripting sections single quotes can be used inside
triple-quoted multi-line strings so long as there is whitespace or a
newline before the closing triple quotes."""
