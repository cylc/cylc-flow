...
override the previous entry, not add to it.  To get a naked option flag
such as \lstinline=-cwd= in SGE you must give a quoted blank space as the
...
If all task I/O is specified relative to the suite share or task work
directories (\lstinline=$CYLC_SUITE_SHARE_DIRECTORY= and
\lstinline=$CYLC_TASK_WORK_DIRECTORY=) then you should be able to run multiple
copies of the suite without interference between them, and other users should
be able to copy the suite and run it too with minimal or no modification.
...
able to run those tasks without manually creating their output directories
first.

\subsection{Make Suites As Self-Contained As Possible}

Tasks can of course run or use external files. In some cases this may be
necessary, but it does leave suites vulnerable to external breakages.  Files
stored under the suite definition directory can be accessed via
\lstinline=$CYLC_SUITE_DEF_PATH=, and the suite bin directory is automatically
added to \lstinline=$PATH= in the task execution environment.  It is therefore
possible for a suite to be completely self-contained (aside from exposure to
the network, filesystem, and OS) if every file needed by its tasks is stored
under the suite definition directory.

\subsubsection{Distinguish Between Source and Installed Suites}

If you have multiple suites there may be a tradeoff between self-containment
and duplication of task scripts etc., but this doesn't particularly matter if
you use a deployment or installation system that automatically extracts,
builds, and installs suite components (scripts, executables, control files,
etc.) from external revision control repositories prior to, or at the start of,
the suite run.  The idea is that the suite definition, along with any files
stored and version-controlled with it, should be extracted to a source location
before a run. The suite source location, which provides access to the
repository can be used for further suite development.  At run time the content
of the suite definition directory should be installed by your deployment system
to an ``installed suite'' location from which the suite will be executed (the
default suite run directory is ideal for this).  The deployment system may be
able to handle installation of external files too, or this can be done by
special deployment tasks at suite start-up.  This clean separation of source
and installed suites makes self-containment easier to achieve and allows
development to continue without interfering with running installed suites.
Rose~\ref{Rose} provides tools to facilitate this mode of working.
...
Start-up tasks and ``repeating asynchronous'' tasks have been removed from cylc
6, and use of cold-start tasks is no longer recommended. Instead, you should
use the initial/repeat-once notation as detailed
