...
As a suite runs its task proxies move through a series of defined
states, which are shown in suite monitor and control applications:

\begin{myitemize}
    \item {\bf waiting} - a waiting task is present in the suite but its
        prerequisites are not yet satisfied and, for clock-triggered
        tasks, its trigger time has not come up yet.

    \item {\bf ready} - a task is ready to run if its prerequisites are
        satisfied and, for clock-triggered tasks, its trigger time has
        come up. This is really a virtual state - such tasks are
        immediately queued or submitted, unless held.

    \item {\bf held} - a task that is held will not be submitted even if
        it is ready to run, until it is released from the hold. Held
        tasks still get their prerequisites satisfied as other tasks
        run. Tasks that are already running cannot be held.

    \item {\bf runahead} - this is a special {\em held} state for tasks 
        that have got ``too far ahead'' and exceeded the suite runahead
        limit. Runahead-limited tasks are released automatically 
        when the rest of the suite catches up sufficiently (see {\em
        The Suite Runahead Limit}, Section~\ref{RunaheadLimit}).

    \item {\bf queued} - a task that is queued is ready to run but is
        temporarily prevented from running by the internal queue to
        which it is assigned (see {\em Internal Queues},
        Section~\ref{InternalQueues}).

    \item {\bf submitted} - a task that is ready to run and not currently
        limited by an internal queue will immediately be submitted to
        run via the job submission method defined for it. It will then stay
        in the submitted state until it actually starts executing (which
        may be some time if, for example, it is submitted to an external
        batch queueing system).

    \item {\bf running} - a task enters the running state by reporting
        to cylc that it has actually started executing (via a messaging
        call near the top of the task job script). The running state
        lasts until the task notifies cylc of completion of execution,
        successful or not.

    \item {\bf succeeded} - a task that has reported successful
        completion of execution remains present in the suite, in the
        succeeded state, so long as its outputs may be required to
        satisfy the prerequisites of other waiting tasks, and then it is 
        automatically removed from the suite.

    \item {\bf failed}- a task that has stopped executing after
        reporting failure remains present in the suite the failed state
        indefinitely until dealt with by the suite operator
        (e.g.\ by retriggering it after fixing the problem that caused
        the failure).
...
Cylc suites are naturally self-limiting to the extent that tasks are
only submitted to run if their prerequisites are satisfied - i.e.\ if
they are actually ready to run). But even so some suites may cause 
trouble by generating too much activity at once, particularly if you  
are running in delayed/catch-up/historical operation and are therefore
not limited by clock-triggered tasks that normally wait on real time
data. There are two kinds of problems to be aware of here: overburdening 
an external batch queueing system, or the task host hardware, by submitting 
too many tasks at once - this is prevented by cylc's {\em internal queues};
and overburdening cylc itself by letting loosely constrained tasks run
off far into the future (which would require cylc to keep vast numbers of
``succeeded'' tasks in the suite until other tasks, which may depend on
them, have caught up) - this is prevented by the suite runahead
limit.
...
In a cylc suite each task can cycle ahead independently except as
constrained by dependence on other individual tasks, and any tasks that
are not so constrained will tend to race off ahead.  This may eventually
swamp cylc because a large number of ``succeeded'' tasks will have to be
maintained in the suite until the slower tasks, which may depend on
them, catch up. Typically this applies only to historical operation
where clock-triggered tasks that normally wait on real time data become
unconstrained because the real time data is available in advance and the
wall clock has long passed the clock-triggered tasks' designated trigger
times (determined by comparing task cycle time, plus some offset, to the
wall clock). 

To control this sort of behaviour cylc has a configurable {\em suite
runahead limit} that prevents the fastest tasks from getting too far
...
and queues do coincide you can assign task families en masse to queues
using task family names, as illustrated in the example suite listing below.
...
The following topics have yet to be fleshed out.  For
more help on any topic, see cylc command line help (which is is included
verbatim in Section~\ref{CommandReference}); the suite.rc reference
(Section~\ref{SuiteRCReference}); gcylc help menus; the {\em
Quick Start Guide} (Section~\ref{QuickStartGuide}); and the 
...
\begin{myitemize}
    ...
    \item Understanding suite evolution, particularly in catch up
