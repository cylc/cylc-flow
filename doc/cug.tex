...
definition directory at registration time, or on suite start up.  It is loaded
by the suite daemon at start-up and used to authenticate all client connections.
Clients on the suite host account automatically load the passphrase from the
suite definition directory. The passphrase is also installed automatically in
the suite run directory on hosts for running task jobs. On other accounts that
do not share the same HOME directory with the suite host account and do not have
non-interactive SSH access to the suite host account, it is possible to
install the passphrase manually. (A client will attempt to install the
passphrase automatically via non-interactive SSH.)
...
a login shell for a given host, you can set 
\lstinline=[hosts][HOST]cylc executable=
for the relevant host to \lstinline=True= (this is the default behaviour).
If the environment is already correct without the login shell, but the cylc
executable is not in \lstinline=$PATH=, then 
\lstinline=[hosts][HOST]cylc executable=
can be used to specify the path to the cylc executable.
