...
\begin{figure}
    ...
    \end{center}
    \caption[The only safe multicycle job schedule?]{\small The best
...
time operation this intercycle dependencies
...
Cylc installs into a normal user account (although it can easily be
pressed into a system-level install); just unpack the release tarball in
your desired location, which we shall call \lstinline=$CYLC_DIR=.
...
just unpack the cylc release tarball into \lstinline=$CYLC_DIR=.
...
\begin{figure}
    ...
    {\small The large suite of Figure~\ref{fig-ecox-1} running.}
\label{fig-ecox-2}
\end{figure} 
...
command scripting or calling external commands, scripts, or
executables).

As such, cylc has quite a number of example suites that illustrate most
facets of suite construction. They are all runnable and can be copied
and modified by users to test almost anything. Some just configure a
suite dependency graph, in which case cylc will run dummy tasks 
according to the graph; some also configure task runtime properties 
(e.g.\ command scripting and environment variables) within the suite
definition; and some have real task implementations that generate 
and consume real files and which will fail if they are not executed in
the right order. 

\subsection{Using The Example Suites}

The master example suites are held centrally under
\lstinline=$CYLC_DIR/examples=, but they are exported to the central
suite database for general use during the cylc installation 
process (see Section~\ref{Installation}).  Use \lstinline=gcylc= or 
\lstinline=cylc db print --central <admin>= to see what's available.
Throughout the User Guide any reference to an example suite uses
\lstinline=<admin>= as the username of the account under which cylc is
installed (the suite owner's username is used as the first component of
a suite name in the central database); replace this as appropriate for
your local cylc installation, and import the suite to your private
database so that you can run it and modify it.
\begin{lstlisting}
...
% cylc db import <admin>.cylc-x:y:z.QuickStart.a eg.a $TMPDIR
COPY /home/<admin>/cylc/CDB/<admin>/cylc-x:y:z/QuickStart/a 
  TO /tmp/eg/a
REGISTER eg.one: /tmp/eg/a

% cylc db print eg
eg.a | Quick Start Example A | /tmp/eg/a

% cylc db edit eg.a
\end{lstlisting}

All of the example suites use their registration names, portably via
suite and task identity variable supplied by cylc, in all suite 
and task I/O paths, so you can run multiple copies (or even instances)
of the same example suite at once, under different names, without 
...
You can't run suites directly from the central database, but you can
perform the following actions by right-clicking on  suites in the
central database (or using the cylc command line):
...
Some operations, such as unregistering and importing, can be done on
groups of tasks - just right-click on the group name.
...
COPY /home/<admin>/cylc/CDB/<admin>/cylc-x:y:z/QuickStart/a 
  TO /tmp/QuickStart/a
REGISTER QuickStart.a: /tmp/QuickStart/a
COPY /home/<admin>/cylc/CDB/<admin>/cylc-x:y:z/QuickStart/b 
  TO /tmp/QuickStart/b
REGISTER QuickStart.b: /tmp/QuickStart/b
COPY /home/<admin>/cylc/CDB/<admin>/cylc-x:y:z/QuickStart/c 
  TO /tmp/QuickStart/c
REGISTER QuickStart.c: /tmp/QuickStart/c
\end{lstlisting}
...
% cylc db pr -t Quick
QuickStart    
 |-a       Quick Start Example A | /tmp/QuickStart/a
 |-b       Quick Start Example B | /tmp/QuickStart/b
 `-c       Quick Start Example C | /tmp/QuickStart/c
\end{lstlisting}
...
Some operations such as copying, unregistering, and exporting, can be
done on groups of tasks - just right-click on the group name.

\subsection{Viewing The QuickStart.a Suite Definition}
...
editor}. You can of course do this manually but this way is quick and
convenient and you don't have to remember suite definition directory
locations. For suites that use include-files you can optionally edit the
file inlined; it will be split back into its constituent include-files
when you save the file and exit the editor. If you use the vim editor
you can have nice cylc-specific syntax highlighting and section folding
- see Section~\ref{SyntaxHighlighting}.
...
whenever you save the file.\footnote{If you're editing an inlined
suite.rc file, it will update when you save the file and exit the
editor.}
...
\begin{figure}
    ...
    \end{center}
    \caption[The {\em QuickStart.a} dependency graph]{\small The
    {\em QuickStart.a} dependency graph.}
\label{fig-QuickStartA-graph18}
\end{figure}

\subsection{Running The QuickStart.a Suite}
...
configured been defined in our example suite so the tasks all default to
using the dummy task command line inherited from the {\em root
namespace} (see Section~\ref{SuiteDefinition}):
\begin{lstlisting}
...
execution environment by cylc. The `runtime' argument refers to the top
level suite.rc section that defines task run time properties including 
command scripting (which in this case defaults to the root namespace). 
...
\begin{figure}
    ...
    \end{center}
    \caption[Suite {\em QuickStart.a} at startup, 06 or 18 hours.]{\small
    Suite {\em QuickStart:one} at startup with an initial
...
\begin{figure}
    ...
    \end{center}
    \caption[Suite {\em QuickStart.a} at startup, 00 or 12
    hours]{\small Suite {\em QuickStart.a} at startup with an
...
\begin{figure}
    ...
    {\small Suite {\em QuickStart.a} running, showing multiple
...
\begin{figure}
    ...
    {\small Suite {\em QuickStart.a} stalled after the
...
too far into the future in delayed operation. It
is of little conseqence in real time operation\footnote{So long as it
is sufficient to span the normal range of cycle times present in the
suite - task that only run once per day, for example, have to spawn a
successor that is 24 hours ahead.} because clock triggered tasks are
then constrained by the wall clock, and other tasks have to wait on
...
\subsection{QuickStart.b - Handling Model Cold-Starts Properly}
...
\begin{figure}
    ...
    \end{center}
    \caption[The {\em QuickStart.b} graph with model cold-start task.]{\small
...
Run {\em QuickStart.b} to confirm that that no manually
triggering is required to get the suite started this time.


\subsection{QuickStart.c - With Real Tasks}
...
recent back ups are kept automatically.
...
These files will contain the complete stdout and stderr
record for tasks whose initiating processes do not detach and exit
before all task processing is finished.  The final location of the
internal output of detaching tasks, whose initiating processes submit
internal jobs before exiting early, is up to the task implementation.
...
FILE: /tmp/oliverh/QuickStart/c/suite.rc
   SECTION: [runtime]->[[GetData]]->[[[environment]]]
      [53]:             GETDATA_OUTPUT_DIR = $WORKSPACE
   SECTION: [runtime]->[[ColdModel]]->[[[environment]]]
      [60]:             MODEL_OUTPUT_DIR = $WORKSPACE
   SECTION: [runtime]->[[Model]]->[[[environment]]]
      [68]:             MODEL_OUTPUT_DIR = $WORKSPACE
   SECTION: [runtime]->[[PostA]]->[[[environment]]]
      [76]:             POSTA_OUTPUT_DIR = $WORKSPACE
   SECTION: [runtime]->[[PostB]]->[[[environment]]]
      [83]:             POSTB_OUTPUT_DIR = $WORKSPACE
...
[!SNIP!]
12 common items differ QuickStart.a(<) QuickStart.b(>)

   (top)
 <   title = Cylc User Guide Quick Start Example 1
 >   title = Cylc User Guide Quick Start Example 2

   [scheduling] [[dependencies]] [[[0,6,12,18]]]
 <   graph = 
                     Prep => GetData => Model => PostA
                     Model[T-6] => Model
 ...
 >   cold-start = ['ColdModel']
   ...
   [cylc] [[state dumps]]
 <   directory = /home/oliverh/cylc-run/QuickStart.a/state
 >   directory = /home/oliverh/cylc-run/QuickStart.b/state
...
objects. This can be done through gcylc or by 
\lstinline=cylc validate=:
\lstset{language=transcript}
\begin{lstlisting}
...
\begin{myitemize}
    ...
    \item {\bf A suite \lstinline=bin/= directory}.
        \begin{myitemize}
            ...
            \item Technically optional, as tasks can call external
                commands, scripts, or programs; or can be scripted 
    ...
    \item {\bf Any other sub-directories and files} - documentation,
        ...
        \begin{myitemize}
            ...
            \item The suite definition directory can be accessed
                portably through  
                \lstinline=$CYLC_SUITE_DEF_PATH=
                in task execution environments
...
a string item = the quick brown fox # quotes optional ...
another string item = "the quick, brown fox" # ... unless containing commas
...
\begin{myitemize}
    \item {\bf [cylc] } - how cylc itself behaves when running this suite, e.g.:
        \begin{myitemize}
            \item simulation mode
            \item suite logging and state dump directories
        \end{myitemize}
    \item {\bf [scheduling] } - items allowing cylc to determine when tasks are ready to run.
        \begin{myitemize}
            \item lists of tasks with special behaviour (e.g. clock-triggered tasks)
            \item the suite dependency graph, defining the relationships
                between tasks
        \end{myitemize}
    \item {\bf [runtime] } - how, where, and what to execute when a task is ready to run.
        \begin{myitemize}
            \item command scripting, environment, job submission
                methods, remote hosting, etc.
            \item an inheritance hierarchy of {\em namespaces} groups
...
errors, typographic errors, illegal items and illegal values. Some
values are complex strings that require further parsing by cylc to
determine their correctness (this is also done during validation). 
All legal entries are documented in the {\em Suite.rc Reference}
...
[runtime]
    [[root]]
        command scripting = """
    echo %(_greeting)s from $TASK_ID; sleep 10; echo BYE
                            """
\end{lstlisting}
\lstset{language=transcript}
...
['echo HELLO from $TASK_ID; sleep 10; echo BYE']
\end{lstlisting}

\subsection{Scheduling - Dependency Graphs}
\label{DependencyGraphs}
 
The [scheduling] section of a suite.rc file defines the relationships
...
suite dependency graph. Cylc graph notation makes clear textual
representations of actual graphs, but it is more concise than the real
thing because sections of the graph that repeat at different hours of
...
\begin{lstlisting}
% cylc graph UserGuide.73 2011052200 2011052206
# (or use right-click Graph in gcylc)
\end{lstlisting}
This is actually a complete valid suite that you can run (it runs
dummy tasks according to the suite graph, because no runtime 
properties have been defined, and you'll need to trigger task A manually
to get the suite started because A[T] depends on its own previous
instance A[T-6] and at start-up there is no previous cycle to satisfy
that prerequisite - how to handle this properly is described in 
...
side (which may be a single task or family, or several tasks that are
...
valid for for cycle times matching the list of hours specified for the
...
A suite definition can contain multiple graph strings are combined
...
string) for one-off asynchronous, cycling, and repeating asynchronous
tasks. Additionally, there may be multiple graph strings (under different
VALIDITY sections) for cycling tasks, if there are some tasks with
different dependencies at different cycle times.

\paragraph{One-off Asynchronous Tasks}

Figure~\ref{fig-test1} shows how to define a small suite of one-off
asynchronous tasks; these have no associated cycle time and don't spawn
successors (once finished the suite just exits).  The integer $1$ attached
to each graph node is just an arbitrary label, akin to the task cycle
time in cycling tasks.
\begin{figure}
\begin{minipage}[b]{0.5\textwidth}
\lstset{language=suiterc}
\begin{lstlisting}
...
\begin{minipage}[b]{0.5\textwidth}
    ...
    \end{center}
\end{minipage}
\caption[One-off Asynchronous Tasks]{\small One-off Asynchronous Tasks.}
\label{fig-test1}
\end{figure}
...
in which the tasks appear. Figure~\ref{fig-test2} shows how to
define a small suite of cycling tasks.
\begin{figure}
\begin{minipage}[b]{0.5\textwidth}
\lstset{language=suiterc}
\begin{lstlisting}
...
\begin{minipage}[b]{0.5\textwidth}
    ...
    \end{center}
\end{minipage}
\caption[Cycling Tasks]{\small Cycling Tasks.}
\label{fig-test2}
\end{figure}

\paragraph{Combined Graphs}

You can have a suite of cycling tasks that wait on a group of one-off
asynchronous tasks, as shown in Figure~\ref{fig-test4}.
...
they will be intended as start-up tasks - so {\em currently one-off
asynchronous tasks will only run in a cold-start}.
...
# (with dependence between cyclest too)
[scheduling]
    [[dependencies]]
        graph = "prep1 => prep2"
        [[[0,12]]]
...
\begin{minipage}[b]{0.5\textwidth}
    ...
    \end{center}
\end{minipage}
\caption[One-off Asynchronous and Cycling Tasks]{\small One-off Asynchronous and Cycling Tasks.}
\label{fig-test4}
\end{figure}
...
a regular expression designed to match the incoming IDs.  Addtionally,
...
ID pattern, and when these are satisfied the actual ID is substituted
into their registered outputs. Additionally, each asynchronous task
proxy passes the ID to its task execution environment as
\lstinline=$ASYNCID= to allow identification of the correct dataset.
...
\begin{minipage}[b]{0.5\textwidth}
    ...
    \end{center}
\end{minipage}
\caption[Repeating Asynchronous Tasks]{\small Repeating Asynchronous Tasks.}
\label{fig-test3}
\end{figure}
...
time too. If used more generally task families will unnecessarily
...
the suite is started (in this case the cold-start task in cylc will just
report itself successfully completed, thereby satisfying the
...
ColdFoo[T]. At start-up ColdFoo will do the job, and thereafter Bar[T-6]
will do it.
...
because a warm start is likely to involve re-running some of the tasks).
Warm-starts assume the existence of a previous cycle (i.e.\ that any
files from the previous cycle that are required by the
...
execute, and
where and how, when tasks are ready to run. The sections immediately
below [runtime] are, for want of a better term, {\em
namespaces}\footnote{We use the
term namespace in loose analogy with its meaning in modern programming
languages.  Possible future enhancements to cylc, such as ability to
import specific items from other namespaces rather than just wholesale
inheritance, may tighten the analogy.} that define runtime properties
for individual tasks or for families of tasks.
...
tree-like hierarchy (possibly with multiple trunks) of nested task
families, rooted on the root namespace, in which the leaves are the
individual tasks of the suite. 
...
The root namespace, being at the base of the inheritance hierarchy,
provides default configuration for all tasks in the suite. 

Most root items are unset by default, but some defaults values are
provided already, sufficient to allow simple test suites to be defined
by dependency graph alone - as can be seen from many of the examples 
...
Namespace (task and family) names can contain letters, digits, and
underscores. {\em Task names should not be hard wired into task
...
{\bf The two annotated {\em multidef} example suites use this feature.}
...
using passwordless ssh (which must be configured between
the suite and task owner accounts). 
...
families, will be automatically plotted in rectangular nodes to the
right of the live graph if they do anything interesting (submitted,
running, or failed).
...
\lstset{language=suiterc}
The position of task in the suite dependency graph determines its
relationship to other tasks and its valid cycle times. Runtime
properties such as execution environment and the command scripting used
to invoke task processing are defined under the suite.rc
...
\subsection{Tasks That Require No Modification For Cylc}

Any existing command, script, or executable can function as a cylc task
(or rather, perform the external - outside of cylc - processing that
...
\begin{myitemize}
    ...
    \item Any external processing must return zero exit status on 
        ...
        anyway) to allow cylc's automatic error detection to work.
\end{myitemize}
...
The following suite.rc listing shows tasks that run a couple of external 
scripts that are not cylc-aware, but which meet the requirements above 
(so no special treatment or script modification is required at all):
\lstset{language=suiterc}
\begin{lstlisting}
# SUITE.RC
[runtime]
...
the required round brackets around the suite.rc cycle time variable.}
...
suite stdout by cylc (scheduler or \lstinline=cylc submit=). In the
following shell transcript we generate a job script for a task in 
the QuickStart.c example suite and then examine it:
\lstset{language=transcript}
\begin{lstlisting}
% cylc submit --dry-run cylc-x:y:z.QuickStart.c Model%2011080506
> JOB SCRIPT: ~/cylc-run/cylc-x:y:z.QuickStart.c/log/job/Model%2011080506-1317298378.191123
> THIS IS A DRY RUN. HERE'S HOW I WOULD SUBMIT THE TASK:
~/cylc-run/cylc-x:y:z.QuickStart.c/log/job/Model%2011080506-1317298378.191123 </dev/null 
    1> ~/cylc-run/cylc-x:y:z.QuickStart.c/log/job/Model%2011080506-1317298378.191123.out 
    2> ~/cylc-run/cylc-x:y:z.QuickStart.c/log/job/Model%2011080506-1317298378.191123.err &
\end{lstlisting}
...
export CYLC_SUITE_REG_NAME=cylc-x:y:z.QuickStart.c
export CYLC_SUITE_REG_PATH=cylc-x:y:z/QuickStart/c
...
\begin{lstlisting}
# task job script:
~/cylc-run/cylc-x:y:z.QuickStart.c/log/job/Model%2011080506-1317298378.191123
# task stdout:
~/cylc-run/cylc-x:y:z.QuickStart.c/log/job/Model%2011080506-1317298378.191123.out 
# task stderr:
~/cylc-run/cylc-x:y:z.QuickStart.c/log/job/Model%2011080506-1317298378.191123.err
\end{lstlisting}
...
[runtime]
    [[foo]]
        command scripting = "cylc run SUITE $CYCLE_TIME --until=$CYCLE_TIME"
\end{lstlisting}
\end{myitemize}
\lstset{language=transcript}
%\pagebreak
...
The suite will now abort at startup if it cannot connect to the
