...
they can trigger off a wall clock time expressed as an offset from cycle point:
\lstset{language=suiterc}
\begin{lstlisting}
[scheduling]
    [[special tasks]]
...
when the wall clock time reaches \lstinline=2015-08-23T02=.
...
see~\ref{ClockTriggeredTasks}) tasks can depend on events reported by an
...
new data for processing. This is an alternative to having long-running tasks
that poll for external events.

Note that cylc does not directly support ``filesystem events'' because systems
such as \lstinline=inotify= on Linux are not portable. However, external
watcher processes can of course use inotify to detect filesystem events, if
that is appropriate, before triggering the suite.
...
[scheduling]
    ...
    [[special tasks]]
        external-triggered = get-data("new sat X data avail")
    [[dependencies]]
        [[[P1]]]
...
Note that only one task in a suite can trigger off a given external message.
This is no restriction however because other tasks can trigger off the
...
(see~\ref{ClockTriggeredTasks}), otherwise they could clutter up your batch
