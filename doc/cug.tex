...
Suites need to track the progress of their tasks. Cylc now has three
ways of achieving this, configured per task host in the site/user config
files. One suite can use different task communication mechanisms for 
different task hosts if necessary. \begin{myenumerate}
\item {\bf task-to-suite messaging:} task job scripts encapsulate
task scripting in a wrapper that automatically invokes messaging
commands to report progress back to the suite. The messaging commands
can be configured to work in two different ways:
    \begin{myenumerate}
        \item {\bf Pyro:} direct messaging via network sockets, using
        Pyro (Python Remote Objects). 
        \item {\bf ssh messaging:} for tasks hosts that block 
        access to the network ports required by Pyro, this communication
        method uses passwordless ssh to re-invoke task messaging
        commands on the suite host (where ultimately Pyro is still used).
    \end{myenumerate}
\item {\bf one-way polling:} for task hosts that do not allow return
routing back to the suite host for Pyro or ssh, cylc can now poll tasks
at configurable intervals using passwordless ssh, for all supported job
submission methods. 
\end{myenumerate}

The Pyro connection method is the default because it is direct and
efficient, whereas ssh messaging just inserts an extra step in the process.
Polling is less efficient, particularly for very quick-running suites,
and you should be careful to avoid spamming task hosts - each poll opens
(and then closes) a new ssh connection. Polling subprocesses are batched by
cylc, and the number invoked at once can be configured in suite definitions:
...
provide a solution if Pyro ports are blocked and you don't want to 
use polling.
\item you may be able to persuade system administrators to provide
networking routing to one or more dedicated cylc servers, if not to the
wider world. 
\item it is possible to run cylc itself on HPC login nodes, to avoid the 
need for {\em remote} task tracking, but depending on what software is
installed on the HPC this may preclude use of the gcylc GUI and
suite visualization tools.
\end{myitemize}
...
# Polling interval in seconds for the 'poll' task communication method
# (wherein 'submitted' and 'running' tasks are polled periodically). 
task polling interval = float( min=5, default=10 )
...
[hosts]
    ...
    [[HOST]]
       # Method of communication of task progress back to the suite:
        ...
        # (the polling iterval is not task-host dependent, see above)
\end{lstlisting}
...
suite registration database. Remote user command invocations, however,
