...
Note that the suite title held in the registration database is parsed
...
exchange area for tasks on same task host. The suite share directory can
be accessed via \lstinline=$CYLC_SUITE_SHARE_DIR= and its location can
be set in the cylc site/user config files. 

A {\em task work directory} is also created automatically for each task, 
Its location is held in the \lstinline=$CYLC_TASK_WORK_DIR= variable.
Task command scripting is executed from within the work directory (i.e.
it is the task's {\em current working directory}. In non-detaching
tasks the work directory is automatically removed again if it is empty
when the task finishes. The top level location of the task work and
share directories can be set in the cylc site/user config files, but in
the case of work-directories you can set the low-level work
sub-directory under \lstinline=[runtime]= in your suite definitions. The
default sub-directory name is the task ID, which gives each task a
unique working directory. You can override this to give groups of tasks
the same work directory, however, thus creating private share spaces for
tasks that read and write files from their current working directory.
...
\begin{myitemize}
    \item {\bf base} (gcylc graph view only) - greyed out base graph
