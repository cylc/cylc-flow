...
cylc release, unless perhaps some timeouts occur because your suite host
is slow or overloaded - otherwise please consider reporting test
failures to the cylc-dev mailing list.
...
And finally, cylc now has a one-way polling mechanism for tracking task 
progress on task hosts that do not allow routing back to the suite host.

See Section~\ref{RunningSuites} for more on how cylc's
...
Remote tasks must have access to cylc via \lstinline=$PATH= on the task
host.  This can be achived by the system or user login scripts,
\lstinline=/etc/profile= or \lstinline=$HOME/.profile= - the remote job
submission mechanism automatically sources these.
...
at the top of task job scripts before the first cylc messaging call.
...
\subsection{Job Poll And Kill}

For each supported job submission method, cylc now also supports polling
for real task status, and job kill, from the gcylc GUI and command line
(\lstinline=cylc poll= and \lstinline=cylc kill=).
In addition to on-demand polling, formerly submitted and running tasks
are polled automatically on suite restarts (Section~\ref{RestartingSuites})
and on job submission and execution timeouts, and one-way polling can be
used to track tasks on hosts that do not allow return routing for 
task-initiated messaging (Section~\ref{RunningSuites}).
...
represent a task state, they are displayed to flesh out the graph
structure where it is not covered by the current live task pool.
