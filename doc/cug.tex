...
The remote cylc directory must be specified to give remote tasks access 
to cylc commands, if cylc is not in the default \lstinline=$PATH= 
on the remote host. If \lstinline=$PATH= is configured in your
\lstinline=$HOME/.profile= script it will be picked up as the remote
job submission mechanism automatically sources \lstinline=.profile= if
it exists. Similarly the remote suite directory gives tasks access to
