...
Other suite-connecting commands work in the same way too; see~\ref{RemoteControl}.
...
This section explains how tasks are submitted
...
automatically on suite restart (\ref{RestartingSuites})
...
Runahead limiting prevent the fastest tasks in a suite from getting too far
...
active cycle points. The default value is 3 active cycle points; see~\ref{ref max
active cycle points}. Alternatively the interval between the
slowest and fastest tasks can be specified as hard limit;
see~\ref{ref runahead limit}.
...
Broadcast settings targetted at a specific task ID or cycle point expire and
...
\begin{myitemize}
    ...
    \item Sub-suites: to run another suite inside a task, just invoke the
        subsuite, with appropriate start and end cycle points (probably a
...
proxy appears to disappears from the suite. The following may help
