...
\subsubsection{Graph Headers}

In the above examples graph headings are used to define recurring date-times
when tasks are to run. For example in the following scenario,
...
It can be desirable to exclude a date from a recurrence, this can be achieved
by appending the date to the recurrence prefixed with an exclamation mark.
For example \lstinline=[[[ PT1D!20000101 ]]]= means run daily except on the
...
suite \lstinline=foo= will only run once as what would be its second run has
been excluded.
...
The following examples cover the various ways of writing graph headers in cylc.
\lstset{language=suiterc}
\begin{lstlisting}
[[[ R1 ]]]         # Run once at the initial cycle point
...
a particular point from a recurrence also works for integer cycling
meaning you can write:

\lstset{language=suiterc}
\begin{lstlisting}
[[[ R/P4!8 ]]]       # Run with step four, to the final cycle point
                     # but not at point 8
[[[ R3/3/P2!5 ]]]    # Run with step two from point 3 but not at
                     # point 5
[[[ R/+P1/P6!14 ]]]  # Run with step six from one step after the
