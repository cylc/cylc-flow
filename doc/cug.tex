...
Of course particular tasks can also be singled out at run time, in common
...
See the variable listing above, and Sections~\ref{LocalWork} and
\ref{LocalShare}.
...
submission subprocess itself takes ten seconds to complete before the
task starts executing; and \lstinline=fail=, in which the job submission
subprocess itself always fails.  
...
ahead of the pack cylc has to keep them around in the ``succeeded''
...
``runahead held'' state until other tasks catch up sufficiently.

In cylc-4.5.1 and earlier it was possible to stall a suite by setting
the runahead limit too low; now however it is applied in such a way
that it simply constrains the number of different cycles that can run at
once. If the limit is set to the smallest interval between cycles, or
less than that, just a single cycle will run at a time.

The default runahead limit is now set to {\em twice the smallest of the
cycling intervals of the suite's cycling modules} (i.e.\ the graph
section headings). For a suite that has 1- and 24-hourly cylcing tasks
the default limit will be 2 hours, so that two of the hourly cycles will
run at once in between the 24-hourly cycles. 
...
\subsection{Communication Between Tasks: Environment Variable Broadcast}

Communication of information from one task to another normally takes place 
via the filesystem, i.e.\ input/output file releationships expressed in
task dependencies. Since cylc-4.6.0, however, tasks can also communicate 
some information to other downstream tasks via an environment variable
broadcast mechanism. The following task scripting, for instance, broadcasts 
three environment variables to other tasks: \lstinline=$VERSE1= will be
available to all subsequent tasks for the duration of the current suite
run; \lstinline=$VERSE2= will be available only to tasks with the same
cycle time as the broadcasting task; and \lstinline=$VERSE3= will be
available only to tasks with a cycle time six hours beyond that of the 
broadcasting task.

\lstset{language=bash}
\begin{lstlisting}
# (TASK IMPLEMENTATION SCRIPTING)
# broadcast some environment variables to downstream tasks:
cylc broadcast VERSE1 "the quick brown fox"
cylc broadcast VERSE2 "jumped over" $CYLC_TASK_CYCLE_TIME
cylc broadcast VERSE3 "the lazy dog" $( cylc cycletime --offset-hours=6 )
\end{lstlisting}

When a task is ready to run any broadcast variables valid at job
submission time will be written to its job script, after the rest of
the task execution environment is defined so that broadcast variables 
take precedence over existing variables of the same name.

Broadcast variables targetted at a particular cycle time expire as the
suite moves on, but untargetted variables persist throughout the entire
suite run and so should be used sparingly. 
...
Additionally, the command line must be parsed first by the sub-command
of the invoked version in order to determine which command line argument
references the suite (the suite name is always the first non-option
argument but only sub-commands know if other options have their own
arguments or are just logical flags, and can therefore determine which
is the first true argument); consequently any attempt to use a command
option that is illegal in the invoked version of cylc will also fail.

Finally, note that the task messaging commands are exempt from version 
compatibility checking, to avoid the overhead while a suite is running.
