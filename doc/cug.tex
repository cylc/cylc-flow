...
Cylc's main scheduling loop currently determines which tasks are ready
to run; then submits them to run; then processes network requests
(incoming task messages, etc.); and then repeats the whole process. {\em
In the near future we plan to have continuous request handling and job
submission in background worker threads} but in the meantime if you have
a large ensemble of tasks that all need to submit at once to a remote
host, no request handling can be done until the entire batch has
submitted. For large task ensembles that need to submit all at once 
this means the first tasks to submit may have to wait a while for their
``task started'' message calls to hang up, as they may start executing 
before all ensemble members are submitted. 

Since cylc-4.5.1 tasks ready at the same time can be processed in
sequential batches whose members are submitted in parallel. The default
batch size of 50 can be overridden in the suite definition:
\begin{lstlisting}
# SUITE.RC
[cylc]
