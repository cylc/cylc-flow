...
(Section~\ref{ProgSuiteGenJinja2}) for some examples. 
...
not suite your needs you can extend cylc with your own user-defined 
methods (below, Section~\ref{DefiningNewJobSubmissionMethods}).

\begin{myitemize}
    \item {\bf background} - run tasks directly in a background shell.

    \item {\bf at} - submit tasks to the rudimentary Unix 
         \lstinline=at= scheduler (\lstinline=atd= must be running).

\end{myitemize}

Tasks in a real forecasting system would normally be submitted to an
industrial strength batch queue scheduler. Current built-in methods are:

\begin{myitemize} 
    
    \item {\bf loadleveler} - submits task job scripts to loadleveler
        using the \lstinline=llsubmit= command. 
        Loadleveler directives provided in the suite.rc file: 
...
[runtime]
    [[__NAME__]]
        [[[directives]]] 
            ...
            baz = waz
\end{lstlisting}
        are written to the top of the task job script like this: 
\lstset{language=bash}
\begin{lstlisting}
#!/bin/bash
# DIRECTIVES
...
\end{lstlisting}
    ...
    \item {\bf pbs} - submits task job scripts to PBS (or Torque)
        using the \lstinline=qsub= command. 
        PBS directives provided in the suite.rc file: 
\lstset{language=suiterc}
\begin{lstlisting}
# SUITE.RC
[runtime]
...
\end{lstlisting}
    ...
    \item {\bf sge} - submits task job scripts to Sun Grid Engine
        using the \lstinline=qsub= command. 
        SGE directives provided in the suite.rc file: 
\lstset{language=suiterc}
\begin{lstlisting}
# SUITE.RC
[runtime]
...
Note that {\em multiple entries for the same PBS or SGE option must be
comma-separated on the same line}, in the suite.rc file. This is because
repeating the option on another line will override the previous entry,
not add to it. And, the right-hand side must be quoted to hide the
comma from the suite.rc parser (commas are allowed only for list
values; directives are treated as singular).

Note also that {\em to get a naked option flag such as \lstinline=-cwd= 
in SGE you must give a quoted blank space as the directive value.}

Finally, {\em default directives are provided to set the job name and
stdout and stderr file paths}. These can be overridden if you wish
(but stdout/stderr directory paths should be set via the
appropriate suite.rc item rather than using the directives here).
...
Note that to change the form of the command used to submit a  job you do
not need to create a new job submission method; just override the
COMMAND\_TEMPLATE in the relevant job submission section of your
suite.rc file.
