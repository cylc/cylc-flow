...
how to install everything under a new sub-directory in the cylc source
tree in your home directory, \lstinline=$CYLC_DIR/external=. 

Download the Pyro, Graphviz, and Pygraphviz source distributions from
the locations given at the beginning of Section~\ref{Requirements} and
move the tarballs to the new installation directory.

\subsubsection{Cylc}

Cylc already installs into a normal user account - just unpack the 
cylc tarball as described in Section~\ref{UnpackingTheCylcTarball}.
After unpacking you can create the suite database and upload the example
suites to it as just described in
Section~\ref{CreatingTheCentralSuiteDatabase}.
...
\end{lstlisting}
Now you just need to take note of the resulting sub-directories under
\lstinline=installed=, which are somewhat dependent on your Python
version and platform, and add these to the \lstinline=PYTHONPATH=
variable in your cylc environment script. Edit
\lstinline=$CYLC_DIR/environment.sh=, uncomment the paragraph 
starting with 
\lstinline=# FOR LOCAL INSTALLATION OF PYRO, GRAPHVIZ, AND PYGRAPHVIZ=,
and alter the path to suit your installation. 

At this point you should be able to source the cylc environment script
and use cylc commands:
\begin{lstlisting}
...
3.3.3
\end{lstlisting}
You can now run cylc suites via the command line or the tree suite
control GUI, but you can't graph suites or use the graph suite control
GUI until you install graphviz and pygraphviz.
...
$ make install
\end{lstlisting}
This will install graphviz files into the bin, include, and lib
sub-directories of your local installation directory. The local installation
section of your cylc environment script (above) provides access to the
graphviz executables in this bin directory, although you will probably not
need to use them. The graphviz lib and include locations are required 
when installing Pygraphviz (next).
...
\end{lstlisting}
(Check these are the correct library and include paths by looking at the
contents of the directories).  Finally, install pygraphviz:
\begin{lstlisting}
$ . $CYLC_DIR/environment.sh  # if you haven't done this already
$ python setup.py install --prefix=$CYLC_DIR/external/installed
\end{lstlisting}
This should install the Pygraphviz modules in the same place as the 
Pyro modules, e.g.:
\begin{lstlisting}
...
\end{lstlisting}
If not, add another directory path to the PYTHONPATH variable in your
cylc environment script.

You should now have access to all cylc functionality. 
Test by running the automated suite database test (Section~\ref{RTADT})
and the automated scheduler test (Section~\ref{RTAST}), then go on 
to the {\em Quick Start Guide} (Section~\ref{QuickStartGuide}).
...
In a future release we make the central suite database 
...
\begin{myenumerate}
    \item Before upgrading (under the old cylc version), unregister the
        {\em examples} group from the central database and from the cylc
        admin user's private database. 
    ...
    \item Run \lstinline=cylc admin create-cdb= to register the example
