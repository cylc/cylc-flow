...
In addition, the task job script has \lstinline=set -u= and \lstinline=set -o
pipefail= turned on explicitly to ensure that any variable must be defined on
reference (which is very useful for picking typos) and that a command fails if
any part of a pipe fails.
...
the task. Different tasks can have different job submission methods.  Like
other runtime properties, you can set a suite default job submission method and
