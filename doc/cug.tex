...
{\em future trigger} \lstinline@A[T+12] => B@ (see
...
Cold-start tasks have to be declared as such in the suite definition
so that cylc knows they are one-off (non-spawning) initial tasks, but
also because they play a critical role in suite warm-starts. A suite
that has previously been running and then shut down can be warm-started
at a particular cycle time as an alternative to {\em restarting} from the 
pre-shutdown suite state (although restarting is the preferred option
...
files from the previous cycle required by the new cycle are already in
place) so cold-start tasks do not need to run {\em but} cylc itself
