...
The need for Jinja2 processing can be declared with a bang-hash
interpreter comment at the very top of the suite.rc file:
\begin{lstlisting}
#!Jinja2
# This is a SUITE.RC with embedded Jinja2 expressions
# ...
\end{lstlisting}
See {\em Jinja2: Suite Generation Templates}
...
\subsection{Jinja2: Suite Generation Templates}
\label{ProgSuiteGenJinja2}

The Jinja2 template processor adds powerful structural logic to cylc
suites. You can use embedded variables, mathematical expressions, loop
control structures and conditional expressions to generate the final
suite definition. Potential uses include automatic generation
of repeated groups of similar tasks and dependencies, and inclusion or
...
\begin{myitemize}
    \item {\bf the first line of the suite.rc must be:}
        \begin{lstlisting}
#!Jinja2
        \end{lstlisting}
    \item the template engine processes the file first, so 
        Jinja2 expressions can appear anywhere, even  
        inside suite.rc strings.
\end{myitemize}

The following suite definition (the \lstinline=Jinja2= example in the 
central suite database) includes or excludes special cleanup tasks
according to the value of a logical flag, and it automatically
generates all dependencies, and the correct family relationships, for a
group of tasks that is repeated for each city in the suite. To add a new
city and associated tasks and dependencies simply add the new city
name to list at the top of the file. The suite is graphed, with 
the New York City task family expanded, in Figure~\ref{fig-jinja2-ny}.

\lstset{language=suiterc}
\lstinputlisting{../examples/jinja2/suite.rc}
\lstset{language=transcript}
...
\begin{figure}
    ...
    \end{center}
    \caption[Jinja2 example suite graph.]{\scriptsize
    The dependency graph for the Jinja2 examples suite, with the 
