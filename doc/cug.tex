...
for suite graphing and the gcylc graph view.  For example
...
\subsection{How Tasks And Control Commands Interact With Suites}

All interaction with a running suite by control commands, gcylc GUI
instances, and messaging commands from running tasks, uses the Pyro
network RPC (Remote Procedure Call) protocol. Each suite listens on its
own network port; cylc starts grabbing ports at 7766 by default - 
this is configured in the site config file
\lstinline=$CYLC_DIR/conf/site/site.rc=, see 
\lstinline=cylc get-global-config --help=.

However, if the Pyro network ports are blocked at your site you can get
remote control commands and remote task messaging calls to re-invoke
themselves on the suite host using passwordless ssh, so that the
ultimate Pyro connection to the suite only occurs on the suite host.

\subsubsection{Pyro Connections And Secure Passphrases}
\label{passphrases}

All Pyro connections to a suite require passphrase authentication. The
passphrase is just an arbitary single line of text. A secure MD5
checksum is passed across the network, not the raw passphrase itself. 
A random suite passhprase is generated in the suite definition directory
when a suite is registered, but you can create your own as you wish.

Passphrases currently have to be installed manually in any user account,
local or remote, that hosts tasks from the suite, or from which you want
to interact with the suite without using the ssh remote communication 
method. Legal passphrase locations, in order of preference, are:
\begin{myenumerate}
...
may not be known or accessible. On remote task hosts the suite
definition directory defaults to the same location relative to
\lstinline=$HOME= as on the suite host; or the remote location can be
specified explicitly in the suite definition. 

\subsubsection{Ssh-Pyro Connections}

For ssh-based remote task messaging and suite control, ssh keys must be
installed in the relevant accounts to allow passwordless ssh connections
back to the suite host account. The suite passphrase is then only
required on the suite host.  
...
[runtime]
    [[NAME]]
        [[[remote]]]
            ...
            ssh messaging = True
\end{lstlisting}
    \item For user-invoked suite control commands, use
        the \lstinline=--use-ssh= command line option along with the 
        \lstinline=--host= and/or \lstinline=--owner= options. Commands
        that normally prompt before taking action will also require use
        of \lstinline=-f,--force=.
\end{myitemize}
...
\begin{myitemize}
    ...
    \item It has been suggested that network {\em port forwarding} may 
        provide a solution.
    \item We plan to develop a one-way polling mechansim to track the
        progress of remote tasks that are unable to communicate back to
        their suite, but this has not been implemented yet.
\end{myitemize}
...
the real tasks) may move through a number of defined states:
...
\begin{myitemize}
    ...
    \item {\bf queued} - ready to be submitted (i.e.\ prerequisites
        satisfied) but temporarily held in an internal
        cylc queue (see Section~\ref{InternalQueues}). 

    \item {\bf submitting} - ready to submitted (i.e.\ prerequisites
        satisfied) and passed to the background thread that handles task
        job submission.

    \item {\bf submitted} - submitted by the configured job submission
        method, but not yet running (may be waiting in an external batch
        scheduler queue, for example).

    \item {\bf running} - a ``task started'' message was received, indicating 
        that the task is now executing.

    \item {\bf retrying} - a ``task failed'' message was received, but the 
        task has a configured retry lined up after some delay, during which 
        the task proxy waits in the {\em retrying} state. See
        Section~\ref{TaskRetries}.

    \item two finished states:
        \begin{myitemize}
            \item {\bf succeeded} - a ``task succeeded'' message was received. 
               The task proxy will remain in the suite in this state
               until cylc determines its outputs are no longer needed
               to satisfy the prerequisites of other waiting tasks.

            \item {\bf failed} - a ``task failed'' message was received, or 
                some other error condition such as job submission
                failure occured. Task proxies in the {\em failed} state
                are not automatically removed from the suite.
        \end{myitemize}

    \item two held (paused) states:
        \begin{myitemize}
            \item {\bf held} - a {\em held} task will not be submitted,
                even if it is ready to run, until released.
                Only waiting tasks can be held. The suite operator can
                deliberately hold tasks, and cylc itself puts tasks on
                hold under certain circumstances. For example, if the 
                suite has been told to shut down after currently running 
                tasks have finished, waiting tasks will be put on hold 
                while any currently running tasks are finishing.
            ...
            \item {\bf runahead} - a task in the {\em runahead} state has
                ...
                managed by cylc, but the size of the limit is
                configurable. See Section~\ref{RunaheadLimit}.
        \end{myitemize}
\end{myitemize}
...
\begin{myitemize}
    \item {\bf ready} - to reset a task to {\em ready} is to to set it to
        the {\em waiting} state and set all of its prerequisites satisfied.
\end{myitemize}
...
{\em runahead} held state until other tasks catch up sufficiently.
