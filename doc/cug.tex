...
\subsection{Site And User Configuration}
\label{SiteAndUserConfiguration}

Cylc uses site and user configuration files to set some important
global parameters such as the range of network ports, and which editor
to use for editing suite definitions:

\begin{lstlisting}
$CYLC_DIR/conf/site/site.rc  # site config file
$CYLC_DIR/conf/user/cylc.rc  # default user config file
$HOME/.cylc/cylc.rc          # private user config file
\end{lstlisting}

The site config file should be adapted by the cylc admin user at
installation time; settings in it cannot be overridden by users. The
default user config file, held centrally, contains global settings that 
users can override, if necessary, after copying it to make a private
user config file.
