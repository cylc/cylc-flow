...
\begin{myitemize}
    ...
    \item {\bf background} - run tasks directly in a background shell.
     ...
     \item {\bf at\_now} - submit tasks to the rudimentary Unix 
         \lstinline=at= scheduler (\lstinline=atd= must be running).

\end{myitemize}

Tasks in a real forecasting system would normally be submitted to a
batch queue scheduler. Current built-in methods are:
...
\end{lstlisting}
    ...
    \item {\bf pbs} - submits task job scripts to PBS
...
And here is the \lstinline=PBS= method is quite simple:
