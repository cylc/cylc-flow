...
\end{lstlisting}
\lstset{language=transcript}
cylc will use passwordless ssh to execute the task on the declared host
account (\lstinline=owner@host=) by the configured job submission method.
For this to work,
\begin{myitemize}
    \item passwordless ssh must be configured between suite and task
    host accounts.
    \item cylc must be installed on task hosts so that remote tasks can
    use cylc messaging commands to communicate back to the suite.
    \begin{myitemize}
        \item Pyro is needed by cylc on tasks hosts - unless ssh messaging
        is used - but other cylc software dependencies such as graphviz and
        Jinja2 are not.
    \end{myitemize}
    \item the suite definition directory, or some fraction of its
    content, can be installed on the task host if the remote task needs it.
\end{myitemize}
...
\paragraph{Access To Cylc On Task Hosts}

Remote tasks must have access to cylc on the task host. There are
several ways of configuring your remote \lstinline=$PATH= for cylc:

\begin{myenumerate}

\item system or user login scripts: \lstinline=/etc/profile= or
\lstinline=$HOME/.profile= - the remote job submission mechanism
automatically sources these.

\item Use {\em cylc bin directory} items in site/user config file host
sections to set the cylc location for all suites running on the host.

\end{myenumerate}

It is also possible to configure your \lstinline=$PATH= manually
in runtime namespace {\em intial scripting} sections, which end up
at the top of task job scripts before the first cylc messaging call. The
other methods are preferred, however, because this is really a
