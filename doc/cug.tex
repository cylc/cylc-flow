...
operation as required.
    ...
    \subsection{Factor Out Common Configuration}
        ...
        this is somewhat analagous to overuse of global variables in
        programming, and makes it difficult to determine which variables
        matter to which tasks). Cylc has several ways of addressing this
        problem:
        ...
        \begin{myitemize}
            ...
            \item {\em Jinja2 variables} can be defined once and
                ...
                as efficient as multiple inheritance when many items are
                shared by many tasks, but it is more efficient when 
                single items are shared by just a few tasks that
                don't have anything else in common (e.g.\ an environment
...
\end{lstlisting}
    ...
    should generally be grouped into families that reflect their purpose
    within the structure of the suite rather than by technical detail
...
\end{lstlisting}
    ...
    acceptable though as it preserves the proper indentation:
\lstset{language=suiterc}
\begin{lstlisting}
[SECTION]
    a short item          = value1 # OK
...
\end{lstlisting}
    ...
    \item {\em Line comments} should be indented to the same level as the
        section or item they refer to - anything less hampers quick block
        re-indentation operations in text editors.
    ...
    \item The internal lines of {\em multiline string values} should not
        ...
        purposes, and filling them with bizarrely indented code does not
        help. For example, this:
...
\end{lstlisting}
    ...
    Dependency \lstinline@graph@ strings, however, have no meaning
    outside of the suite definition - so they can be free-form with 
    white space and comments used to display the structure of the
    suite as clearly as possible:
\lstset{language=suiterc}
\begin{lstlisting}
[scheduling]
    [[dependencies]]
...
\end{lstlisting}
   ...
   \item Embedded {\em Jinja2} code is not part of the suite.rc syntax - 
       it can be indented from the left margin on its own terms.
\lstset{language=suiterc}
\begin{lstlisting}
    [[OPS]]
{% for T in OPS_TASKS %}
...
\end{lstlisting}
    ...
    \subsection{Other} 
        ...
        \begin{myitemize}
            \item {\em Naming conventions} - consider, for example,
                ...
                so that you can tell which a namespace is at a glance.
   ...
   \subsection{An Example In The Recommended Style}
