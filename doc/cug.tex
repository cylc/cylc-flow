...
\subsubsection{Ways Of Cycling: Dynamic and Static}
...
\begin{myitemize}
    ...
    \item {\em Mixed Cycling:} Both static and dynamic cycling can be combined
        ...
        could have a ongoing daily dynamic cycle with four static steps in each
        daily cycle point.
\end{myitemize}
...
(Section~\ref{Using Parameters For Static Cycling}) or Jinja2 loops
(Section~\ref{Jinja2}) to do the same automatically.
...
\lstinline=[runtime]= with the notation \lstinline@<NAME=VALUE>@.
...
[scheduling]
    [[dependencies]]
        graph = """model<run> => post_proc<run>  # general case
                   model<run=1> => check_first_model  # special case"""
[runtime]
    [[model<run>]]
...
\subsubsection{Using Parameters For Static Cycling}
\label{Using Parameters For Static Cycling}

Static and dynamic cycling are contrasted in Section~\ref{Ways Of Cycling}. We
generally recommend using dynamic cycling in cylc. Nevertheless, parameter
expansion can be used to efficiently generate static workflows in which a long
model run (and associated tasks) is split into multiple shorter steps (or
``chunks''), with each run represented by a different task.
...
\end{lstlisting}
And here's how to do the same thing statically, with parameterized tasks:
\lstset{language=suiterc}
\begin{lstlisting}
[cylc]
    [[parameters]]
...
can easily be added in both cases. But note that the cycle point arithmetic has
to be done manually in the static case, and large or long-running suites must
use dynamic cycling (see Section~\ref{Ways Of Cycling}).
...
\begin{figure}
    ...
    \end{center}
    \caption[Static (top) and dynamic cycling (bottom) versions of the same
    workflow.]{\scriptsize Static and dynamic cycling versions of
    the same workflow. The only the first three cycle points is shown in the
dynamic case - which is not an option in the static case.}
    \label{fig-eg2}
\end{figure}

Static and dynamic cycling can be combined. Here's a yearly dynamic cycling
suite with four static chunks in each cycle point:
\begin{lstlisting}
[cylc]
    [[parameters]]
        chunk = 1..4
...
to just use 3-monthly dynamic cycling:
\begin{lstlisting}
[scheduling]
    initial cycle point = 2020-01
    [[dependencies]]
...
Consider a portable dynamic date-time cycling workflow of model jobs that can
each take too long to run, or not, depending on the host platform. This could
be handled without changing the cycling structure of the whole suite, by
splitting each run into a variable number of shorter steps, and using more
steps on less powerful hosts.

\paragraph{Cycle Point And Parameter Offsets At Start-Up}

In dynamic cycling, cylc ignores anything earlier than the suite initial
...
it may be somewhat easier to use static cycling if you need multiple date-time
sequences {\em with different start points} in the same suite. We plan to
allow this sequence-start simplification for any date-time sequence in the
future, not just at the suite initial point, but it needs to be optional
...
{\em This section needs to be revised - the Parameterized Task feature
    ...
    a cleaner way to auto-generate suite structure without coding messy Jinja2
loops.}
