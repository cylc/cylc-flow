...
guarantees compatibility but is generally not necessary.
...
\subsection{Cross-Version Suite Compatibility}
\label{compat}
...
version. This allows construction of new-version suites while older
existing ones are still in use, without having to reconfigure your
environment or explicitly invoke different cylc versions for different
suites.
...
\subsubsection{Version Compatibility Limitations}
...
compatibility checking (which is in fact done by the
...
invoked if the sub-command has undergone a name change between the
versions concerned. 
...
\end{lstlisting}
where \lstinline=$CYLC_TASK_DUMMY_RUN_LENGTH= can be set in the suite 
definition and defaults to 10 seconds; this is exported to the task
execution environment by cylc. The command arguments reflect the suite
definition section nesting. 
