...
then next forecast model run. In real
...
of by their real time characteristics, other metaschedulers
...
Cylc has a battery of self-diagnosing tests, invoked by the command 
\lstinline=cylc test-battery=.  These are primarily intended to check
that new developments don't break existing functionality, but you can
also run them after installation to check that everything works
properly.  See \lstinline=cylc test-battery --help= before running the
tests.
...
The \lstinline=cylc get-global-config= command retrieves current
...
examples.tutorial.hworld.jinja2   /tmp/examples/tutorial/hworld/jinja2
...
examples.tutorial.hworld.remote   /tmp/examples/tutorial/hworld/remote
# ...
\end{lstlisting}
...
examples         
 `-tutorial      
   `-minimal      the minimal complete runnable cylc suite
   |-cycling     
...
REREGISTER examples.tutorial.hworld.jinja2 to tut.hworld.jinja2
#...
shell$ cylc db print -x tutorial
tut.hworld.external   Hello World! from an external task script
# ...
\end{lstlisting}
...
\begin{lstlisting}
# pass:
shell$ cylc validate tut.hworld.basic
Suite tut.hworld.basic is valid for cylc-5.3.0
shell$ echo $?
0
# fail:
...
\hilight{ suite: \lstinline=tut.hworld.basic= }
\vspace{3mm}

Here's the traditional {\em Hello World} program rendered as a cylc
suite:
\lstset{language=suiterc}
\lstinputlisting{../examples/tutorial/hworld/basic/suite.rc}
\lstset{language=transcript}
...
thanks to some some defaults designed to allow quick
prototyping of new suites, is submitted to run as a background job on
...
\lstset{language=suiterc}
\lstinputlisting{../examples/tutorial/minimal/suite.rc}
\lstset{language=transcript}
...
\lstset{language=transcript}
\begin{lstlisting}
shell$ cylc edit tut.hworld.basic # in-terminal
shell$ cylc edit -g tut.hworld.basic & # or GUI
\end{lstlisting}
Alternatively, start gcylc on the suite,
\lstset{language=transcript}
\begin{lstlisting}
shell$ gcylc tut.hworld.basic &
\end{lstlisting}
...
\lstset{language=transcript}
\begin{lstlisting}
shell$ cylc run --no-detach tut.hworld.basic
*************************************************************************
...
SUBMIT No.1(1,1):
    (/home/hilary/cylc-run/tut.hworld.basic/log/job/hello.1.1 </dev/null
    1>/home/hilary/cylc-run/tut.hworld.basic/log/job/hello.1.1.out
    2>/home/hilary/cylc-run/tut.hworld.basic/log/job/hello.1.1.err &
    echo $!; wait )
...
a {\em suite log}. The suite automatically shuts down if and
when all tasks have succeeded.

\subsubsection{GUI}
The cylc GUI can also start and stop suites, or connect to running
suites. Start \lstinline=gcylc= for the tutorial example,
\begin{lstlisting}
shell$ cylc gui tut.hworld.basic &
\end{lstlisting}
and use the tool bar {\em Play} button, or the {\em Control}
$\rightarrow$ {\em Run Suite} menu item, to run the suite again.
You may want to alter the suite definition slightly to make the task
take longer to run. Try right-clicking on the \lstinline=hello= task to
to view its output logs. The three {\em suite views} available 
(dot, tree, and graph) will make more sense later when we add more
tasks. Closing the GUI does not affect the suite itself. 
...
\hilight{ suite: \lstinline=tut.hworld.jobsub= }
\vspace{3mm}
...
\lstset{language=transcript}
\begin{lstlisting}
shell$ cylc log tut.hworld.basic hello.1
\end{lstlisting}
Or a new job script can be generated on the fly for inspection,
\lstset{language=transcript}
\begin{lstlisting}
shell$ cylc jobscript tut.hworld.basic hello.1
\end{lstlisting}
...
The \lstinline=hello= task defaults to running as a background job on
the suite host. To submit it to the Unix \lstinline=at= scheduler
instead, configure its job submission settings as in 
\lstinline=tut.hworld.jobsub=:
\lstset{language=suiterc}
\begin{lstlisting}
[runtime]
    [[hello]]
...
\lstinline=at=-specific job submission command will be printed
to stdout:
\lstset{language=transcript}
\begin{lstlisting}
shell$ cylc run --no-detach tut.hworld.jobsub
#...
1 task ready
  * hello.1 submitting now
SUBMIT No.1(1,1):
    (echo "/home/hilary/cylc-run/tut.hworld.jobsub/hello.1.1 \
     1>/home/hilary/cylc-run/tut.hworld.jobsub/hello.1.1.out \
     2>/home/hilary/cylc-run/tut.hworld.jobsub/hello.1.1.err" | at now)
  * hello.1 submission succeeded
...
\lstinline=share/= directory is made available to all tasks as a common
share space. Job log filenames have the {\em task try number} appended
...
\begin{lstlisting}
# suite logs:
shell$ cylc log    tut.hworld.basic           # suite event log
shell$ cylc log -o tut.hworld.basic           # suite stdout log
shell$ cylc log -e tut.hworld.basic           # suite stderr log
# task logs:
shell$ cylc log    tut.hworld.basic hello.1   # task job script
shell$ cylc log -o tut.hworld.basic hello.1   # task stdout log
shell$ cylc log -e tut.hworld.basic hello.1   # task stderr log
\end{lstlisting}
...
\hilight{ suite: \lstinline=tut.hworld.remote= }
\vspace{3mm}

The \lstinline=hello= task defaults to running on the suite host. To
make it run on a remote host instead change its runtime configuration 
as in \lstinline=tut.hworld.remote=:
\lstset{language=suiterc}
\begin{lstlisting}
[runtime]
    [[hello]]
...
\item Cylc must be installed on the task host.

\item For the default task communication method Pyro must be installed
on the task host. Other software dependencies like graphviz and Jinja2
...
host. In the example there is nothing to install because the
...
\lstset{language=transcript}
\begin{lstlisting}
shell$ cylc run --no-detach tut.hworld.remote
# ...
1 task ready
  * hello.1 submitting now
SUBMIT No.1(1,1): ssh -oBatchMode=yes server1.niwa.co.nz \
    "test -f /etc/profile && . /etc/profile 1>/dev/null 2>&1; \
     ...
     mkdir -p $(dirname $HOME/cylc-run/tut.hworld.remote/log/job/hello.1.1) \
     && cat >$HOME/cylc-run/tut.hworld.remote/log/job/hello.1.1 && \
     chmod +x $HOME/cylc-run/tut.hworld.remote/log/job/hello.1.1 && \
     (($HOME/cylc-run/tut.hworld.remote/log/job/hello.1.1 </dev/null \
     1>$HOME/cylc-run/tut.hworld.remote/log/job/hello.1.1.out \
     2>$HOME/cylc-run/tut.hworld.remote/log/job/hello.1.1.err) & echo $!; wait )" \
     < /home/hilary/cylc-run/tut.hworld.remote/log/job/hello.1.1
  * hello.1 submission succeeded
...
\item See Sections~\ref{tutPassphrases} and~\ref{ConnectionAuthentication}.
\end{myitemize}


\subsection{Task Triggering}

\hilight{ suite: \lstinline=tut.hworld.goodbye= }
\vspace{3mm}

To make a second task called \lstinline=goodbye= trigger after  
\lstinline=hello= finishes successfully, alter the suite graph 
as in \lstinline=tut.hworld.goodbye=:
\lstset{language=suiterc}
\begin{lstlisting}
[scheduling]
    [[dependencies]]
...
Run \lstinline=tut.hworld.goodbye= and check the output from the new
task:
\lstset{language=transcript}
\begin{lstlisting}
shell$ cat ~/cylc-run/tut.hworld.goodbye/log/job/goodbye.1.1.out 
# (or cylc log -o tut.hworld.goodbye goodbye.1
JOB SCRIPT STARTING
...
\hilight{ suite: \lstinline=tut.hworld.suicide= }
\vspace{3mm}
...
Try running \lstinline=tut.hworld.suicide=, which also configures
...
\hilight{ suite: \lstinline=tut.hworld.inherit= }
\vspace{3mm}
...
\lstset{language=suiterc}
\lstinputlisting{../examples/tutorial/hworld/inherit/suite.rc}
...
\hilight{ suite: \lstinline=tut.hworld.ftrigger1= }
\vspace{3mm}
...
hierarchy shown:
\lstset{language=suiterc}
\lstinputlisting{../examples/tutorial/hworld/ftrigger1/suite.rc}

Verbose validation shows the family member substitution done by cylc
when the suite definition is parsed:
\lstset{language=transcript}
\begin{lstlisting}
shell$ cylc val -v non-cycling/multi/suite.rc
...
...
Experiment with the \lstinline=tut.hworld.ftrigger1= suite to see
how this works.

\subsection{Triggering Off Families}

\hilight{ suite: \lstinline=tut.hworld.ftrigger2= }
\vspace{3mm}
...
\lstset{language=transcript}
\begin{lstlisting}
shell$ cylc val -v tut.hworld.ftrigger2
...
Graph line substitutions occurred:
...
Experiment with \lstinline=tut.hworld.ftrigger2= to see how this
works.
...
\lstset{language=suiterc}
Suite definitions can style dependency graphs with an 
optional \lstinline=[visualization]= section as at the 
end of \lstinline=tut.hworld.ftrigger2=:
\lstset{language=suiterc}
\begin{lstlisting}
[visualization]
...
\lstset{language=transcript}
\begin{lstlisting}
shell$ cylc graph tut.hworld.ftrigger2 &    # dependency graph
shell$ cylc graph -n tut.hworld.ftrigger2 & # runtime inheritance graph
\end{lstlisting}
...
\begin{figure}
    ...
    \end{center}
    \caption{The {\em tut.hworld.ftrigger2} dependency and runtime inheritance graphs}
...
\hilight{ suite: \lstinline=tut.hworld.external= }
\vspace{3mm}
...
bin directory:
\lstset{language=bash}
\lstinputlisting{../examples/tutorial/hworld/external/bin/hello.sh}
Make the task script executable, and change the \lstinline=hello= task
runtime section to invoke it:
\lstset{language=suiterc}
\lstinputlisting{../examples/tutorial/hworld/external/suite.rc}
...
cycling dummy tasks).  Note the that graph is now defined under an
...
interval in the suite. Cycling tasks can be declared {\em sequential} to
prevent successive instances running in parallel, if necessary
(Section~\ref{ModelRestartDependencies}).
...
is a non-cycling task that does have an associated cycle time, as in
\lstinline=tut.cycling.four=:
\lstset{language=suiterc}
\begin{lstlisting}
[scheduling]
    [[special tasks]]
...
The conditional OR trigger means that this does not actually rely on
...
it is possible to insert cold-start tasks into a running suite to
cold-start a model mid-stream after some failure that requires skipping
cycles and thus breaking the normal warm-cycling trigger. Conversely,
because cylc does now ignore pre-initial-cycle triggers, the cold-start
OR construct is no longer necessary if you'd rather use a start-up task
as described above. 
...
Cylc has built in support for the Jinja2 template processor, by which 
we can embed code in a suite definition to generate the final result
seen by cylc.

The \lstinline=tut.hworld.jinja2= suite illustrates two common 
...
\lstset{language=suiterc}
\lstinputlisting{../examples/tutorial/hworld/jinja2/suite.rc}

To view the result of Jinja2 processing with the Jinja2 flag 
\lstinline@MULTI@ set to \lstinline=False=:
\lstset{language=transcript}
\begin{lstlisting}
shell$ cylc view --jinja2 --stdout tut.hworld.jinja2
\end{lstlisting}
\lstset{language=suiterc}
\begin{lstlisting}
title = "A cylc single Hello World! suite"
[scheduling]
    [[dependencies]]
        graph = "hello"
[runtime]
...
\lstset{language=suiterc}
\begin{lstlisting}
title = "A cylc multi Hello World! suite"
[scheduling]
    [[dependencies]]
        graph = "hello => BYE"
[runtime]
...
\hilight{ suite: \lstinline=tut.hworld.retry= }
\vspace{3mm}
...
\lstset{language=suiterc}
\lstinputlisting{../examples/tutorial/hworld/retry/suite.rc}
...
resubmits.  It only enters the {\em failed} state on the final definitive
failure.

Experiment with \lstinline=tut.hworld.retry= to see how this works.
...
\begin{lstlisting}
shell$ cylc monitor --owner=SUITE_OWNER --port=SUITE_PORT SUITE_NAME
\end{lstlisting}
...
simple dummy suite.  The example suites packaged with cylc (in
\lstinline=/path/to/cylc/examples=, and see use of
\lstinline=cylc import-examples= above) may also be of use.
They all run ``out the box'' and can be copied and modified at will.
...
\item Hold (pause) a suite mid-run with \lstinline=cylc hold=, 
    ...
    before using \lstinline=cylc release= to continue on.
...
nwp          
 ...
 `-test      
   `-region1  Local Model TEST Region1  /home/hilary/nwp_suites/Regional/TESTING/Region1
\end{lstlisting}
...
\begin{lstlisting}
# task job script:
~/cylc-run/tut.hworld.basic/log/job/hello.1.1
# task stdout:
~/cylc-run/tut.hworld.basic/log/job/hello.1.out 
# task stderr:
~/cylc-run/tut.hworld.basic/log/job/hello.1.err
\end{lstlisting}
...
\begin{lstlisting}
shell$ cylc monitor --owner=USER --host=HOST SUITE
\end{lstlisting}
...
\begin{lstlisting}
shell$ cylc monitor --owner=USER --host=HOST --port=PORT SUITE
\end{lstlisting}
...
A battery of reference tests is used to automatically test cylc before
