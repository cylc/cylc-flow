...
the remove tasks that cannot run (or use \lstinline=cylc purge= to
remove their downstream dependents too) and manually trigger the first
post-gap task.\footnote{Prior to release 5.4.7 cylc had a special {\em
sequential} task type that did not spawn a successor until finished,
which allowed multi-cycle restart dependencies to be represented in the
graph and automatic post-gap triggering after manually removal of the
gap tasks. However, this feature was removed because (a) true sequential
behaviour under all circumstances (e.g.\ after manual interventions that
result in multiple instances of a task in the pool) can only be achieved
by explicit inter-cycle triggers, not by delayed spawning, and (b)
manual intervention is required for this sort of scenario anyway, so the
operator might as well manually the trigger the first post-gap task as
well.}
...
is difficult to achieve in particular cases, however, you can use
an artificial inter-cycle trigger to force sequential behaviour.
...
operation as required.
    ...
    \subsection{Factor Out Common Configuration}
        ...
        properties. Jinja variables are more efficient when single items
