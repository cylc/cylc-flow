...
\section{The Meaning Of {\em Cycle Time}}
...
\begin{myitemize}
    \item {\bf Linux} or {\bf Unix}\footnote{The cylc codebase
        assumes Unix-style file paths in places, but it could easily
        made more portable if necessary.} 
    \item {\bf Python} Version 2.4 or greater (latest tested: 2.7.2) but not
        Python 3.x as yet.\footnote{Python 2.4 was released in November
        2004. Python 3 is the future of Python, but it is not backward
        compatible with 2.x and consequently still has significantly
        less library and third party support.  As of mid 2011, Python
        2.7 is still the standard for new Linux distributions.}\newline
        \url{http://python.org}
    \item {\bf Pyro} 3.9.1 or greater (latest version tested
        3.15.) but not Pyro 4.x as yet\footnote{Pyro 3.9 or earlier is
        not compatible with the new-style Python classes used by cylc.
        As of April 2011, Pyro 4, which is compatible with Python 3, is
        in development but it is still not recommended for production
        use.} \newline
        \url{http://irmen.home.xs4all.nl/pyro3}
\end{myitemize}

Beware of not-so-new Linux distributions that may provide Pyro 3.9 or
earlier, which is not compatible with cylc - upgrade if necessary. 

\subsubsection{More Or Less Optional}

\begin{myitemize}
    \item {\bf PyGTK}, a Python wrapper for the GTK+ GUI toolkit.
        PyGTK is included in most Linux Distributions. \newline
        \url{http://www.pygtk.org}
    \item {\bf graphviz} - a graph layout engine (latest version tested:
        2.28.0). \newline
        \url{http://www.graphviz.org}
    \item {\bf Pygraphviz}, a python interface to graphviz (latest version
        tested: 1.1).  \newline
        \url{http://networkx.lanl.gov/pygraphviz}
    \item {\bf Jinja2}, a template engine for Python (version tested: 2.6). \newline
        \url{http://jinja.pocoo.org/docs}

\end{myitemize}

PyGTK is required for the gcylc GUI, but you can control and monitor
...
Jinja2 is a ``template engine'' - by embedding Jinja2 expressions in 
a suite.rc file you can programmatically generate, using general
variables, mathematical expressions, and structural logic, the final
suite definition that is seen by cylc. This is a very powerful tool.
Potential uses include automatic generation of repeated groups of
similar tasks and dependencies, and inclusion or exclusion of entire
groups of tasks according to the value of a single flag. Cylc may also
support other template engines in the future, via plugins.

\subsubsection{Other Software Used Internally}

Cylc has absorbed the following software in somewhat modified form 
...
First install each of Pyro, graphviz, Pygraphviz, and Jinja2, on your
system, following the instructions provided with each package. If you 
do not have root access on your intended cylc host machine and cannot 
get a sysadmin to do this at system level, see
Section~\ref{LocalInstall} for local installation instructions. 

Finally, cylc typically installs into a normal user account; just unpack
the release tarball in your desired location - henceforth called
\lstinline=$CYLC_DIR=.
...
\begin{myitemize}
    ...
    \item {\bf A \lstinline=bin/= sub-directory}.
        \begin{myitemize}
            ...
            \item {\em Optional.} Alternatively, tasks can call external
    ...
    \item {\bf A \lstinline=python/= sub-directory}.
        \begin{myitemize}
            ...
            \item {\em Optional.} Alternatively, new job submission
                methods can be installed into the cylc source tree, or
                in any directory in your \lstinline=$PYTHONPATH=.
        \end{myitemize}
    \item {\bf Any other sub-directories and files} - documentation,
        control files, etc.
        \begin{myitemize}
            \item {\em Optional.} Ignored by cylc, but the entire suite
                definition directory tree is copied when you copy a
...
Cylc suites are defined entirely by a single extended-INI-format file
called {\em suite.rc} that resides in the suite definition directory.
Optionally, suite.rc files may be split up into multiple include-files.
...
%add new configuration items to cylc. 

\subsubsection{Raw Syntax}

The suite.rc file format is derived from the Python ConfigObj config 
file format 
(homepage: \url{http://www.voidspace.org.uk/python/configobj.html}), 
slightly modified by cylc to allow include-files, line continuation and
duplicate directive and environment environment variable definitions.
...
\label{SyntaxJinja2}
\subsubsection{Jinja2 Syntax}

In addition to the raw syntax described above, suite.rc files can now
contain embedded expressions for pre-processing by the Jinja2  template
engine, to allow programmatic generation of the final raw suite
definition seen by cylc.\footnote{Future cylc releases may provide a
plug-in interface to allow use of other template engines in addition to
Jinja2.}
...
# ...
\end{lstlisting}
Jinja2 syntax is well documented at \url{http://jinja.pocoo.org/docs}.
See {\em Using The Jinja2 Template Engine} (Section~\ref{The Jinja2
Template Engine}) for some example suites. 
