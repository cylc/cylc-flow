...
forecast cycle} is comprised of all tasks with a common {\em cycle}, which is
the nominal analysis time or start time of the forecast
...
\begin{figure}
    ...
    Task colors represent different cycles, and the small squares
...
private cycle and spawn their own successors when the time is
...
be populated with tasks with many different cycles. The task pool
...
\section{On The Meaning Of {\em Cycle} In Cylc}

You may be accustomed to the idea that a forecasting suite has a
``current cycle'', which is typically the analysis time or nominal start time
of the main forecast model(s) in the suite, and that the whole suite advances
to the next forecast cycle when all tasks in the current cycle have finished
...
Cylc suites advance by means of individual tasks with private cycles (times)
independently spawning successors at the next valid cycle for
...
of other tasks with other cycles running, or not, at the time.
...
determined entirely by the suite definition, and a {\em cycle} which is
usually a date/time or an integer representation:
\lstset{language=transcript}
\begin{lstlisting}
foo.20100808T00Z   # a task with a date/time cycle
bar.1              # a task with an integer cycle %Q non-cycling task?
\end{lstlisting}
Non-cycling tasks %Q still true?
 usually just have the tag \lstinline=1=, but this
...
Cycling is usually based around iterating through date/time or integer
sequences. A cycling task may run at each point (cycle) in a given sequence.
For example, a sequence might be a set of date/times every 6 hours starting
from a particular date/time. A cycling task may run for each date/time in that
sequence.

There may be multiple instances of this type of task running in parallel,
if the opportunity arises and their dependencies allow it. Alternatively,
a sequence can be defined with only one valid cycle, in which case a task
...
will have a succession of cycles (times) ending in $00$ or $12$ hours,
between specified initial and final cycles (or indefinitely, if no
final cycle is given), as shown in Figure~\ref{fig-tut-one}.
...
same cycle.  The runahead limit prevents uncontrolled spawning of
...
\end{lstlisting}
For any given cycle \lstinline=T= in the sequence defined by the
...
ignores triggers that reach back beyond the initial cycle.
...
Tasks can appear in more than one dependencies subsection graph, and we
can write a sequence that only gives a single date/time occurrence. Together,
this means that we can write an extra set of dependencies that are only
valid at this single date/time cycle. We can choose this single date/time to
be the initial cycle, so that our extra tasks and dependencies apply at the
very start of the suite.

The syntax for writing this single date/time occurrence is \lstinline=R1=,
which stands for \lstinline=R1/no-specified-date-time/no-specified-period=.
This is an adaptation of the ISO 8601 date/time standard's recurrence syntax (
\lstinline=Rn/DATETIME/PERIOD=) with some special context information supplied
by Cylc in place of our \lstinline=no-specified= data.

The \lstinline=1= means repeat once. As we've specified no date-time, Cylc will
use the initial cycle by default, which is what we want. We've also missed
out specifying the period - this is set by Cylc to a zero interval in this
case, although given that it never repeats, this never gets used.

There is a full primer for cycling syntax later in the user guide, which also
explains how the other syntax in this example (\lstinline=T00= and
\lstinline=T06=) is consistent with this.
...
At the initial cycle, \lstinline=foo= will depend on the
\lstinline=foo[-PT12H]= and also \lstinline=prep=:
\lstset{language=suiterc}
\begin{lstlisting}
prep & foo.20130807T1200 => foo.20130808T0000
\end{lstlisting}
\lstset{language=transcript}
...
\lstset{language=suiterc}
\begin{lstlisting}
foo.20130808T0000 => foo.20130808T1200
\end{lstlisting}
\lstset{language=transcript}

However, in our initial cycle example, the dependence on
\lstinline=foo.20130807T1200= will be ignored, because that task's cycle is
earlier than the suite's initial cycle and so it cannot run. This means that
the initial cycle dependencies for \lstinline=foo= actually look like:
\lstset{language=suiterc}
\begin{lstlisting}
prep => foo.20130808T0000
\end{lstlisting}
\lstset{language=transcript}
...
vary depending on the particular cycle:
\lstset{language=suiterc}
\begin{lstlisting}
[scheduling]
    [[dependencies]]
        [[[T00,T06,T12,T18]]] # validity (hours of the day)
            graph = """
...
valid for cycles matching the list of hours specified for the
...
\end{lstlisting}
\lstset{language=transcript}
implies that B triggers off A for cycles in which the hour matches $00$ or
$12$.
...
\end{lstlisting}
\lstset{language=transcript}
This means B[time] triggers off A[time-PT12H] (12 hours before) for cycles
with hours matching $00$ or $12$. $time$ is implicit because this keeps
graphs clean and concise, given that the majority of tasks will typically
depend only on others with the same cycle. Cycle offsets can only appear on
the left of a pair, because a pairs define triggers for the right task at
cycle $time$.  However, \lstinline@A => B[-PT6H]@, which is illegal, can be
reformulated as a {\em future trigger} \lstinline@A[+PT6H] => B@ (see
...
tasks; these have no associated cycle and don't spawn successors
...
cycle in cycling tasks; it increments when a repeating asynchronous
task (below) spawns.
\begin{figure}
\begin{minipage}[b]{0.5\textwidth}
\lstset{language=suiterc}
...
initial cycle". Cylc allows you to start (or end) at any particular time,
repeat at whatever frequency you like, and even optionally limit the number of
repetitions.
...
[scheduling]
    [[dependencies]]
        ...
        # Repeat once at the initial cycle
        [[[ R1 ]]]
            graph = "cold_foo => foo"

        # Repeat every 1 day starting at the initial cycle
        [[[ P1D ]]]
            graph = "daily_bar => daily_baz"

        # Repeat every 5 minutes starting at the initial cycle
        [[[ PT5M ]]]
            graph = "frequent_qux => frequent_wibble"

        # Repeat every 2 weeks starting at 00:00 after the initial cycle
        [[[ T00/P2W ]]]
            graph = "bidiurnal_wobble => bidiurnal_wubble"

        # Repeat every month, starting 5 days after the initial cycle
        [[[ +P5D/P1M ]]]
            graph = "monthly_offset_fred => monthly_offset_flob"

        # Repeat once at 06:00 after the initial cycle
        [[[ R1/T06 ]]]
            graph = "delayed_cold_corge => corge"

        # Repeat 3 times, every day at 08:30 after the initial cycle
        [[[ R3/T0830 ]]]
            graph = "triple_grault => triple_garply"

        # Repeat 3 times, every month at 00:00 on the first of the month after the initial cycle
        [[[ R3/01T00 ]]]
            graph = "triple_waldo => triple_xyzzy"

        # Repeat 5 times, every month starting on Monday following the initial cycle
        [[[ R5/W-1/P1M ]]]
...
\lstinline=R[limit?]/[date-time]=
\lstinline=R[limit?]//[interval]=
\lstinline=[date-time]/[interval]=
\lstinline=R[limit?] # Special limit of 1 case=
\lstinline=[date-time]=
\lstinline=[interval]=.
...
be relative to the initial date/time cycle. For example, \lstinline=T00= by itself
would mean the next occurrence of midnight that follows, or is, the initial cycle.
Entering \lstinline=+PT6H= would mean 6 hours after the initial cycle.
Entering \lstinline=-P1D= would mean 1 day before the initial cycle.
Entering no information for the {\em date-time} implies the initial cycle
itself.
...
bounded by the final cycle's date/time if given.
...
\lstinline=20140420T06=
\lstinline=20140425T06=
\lstinline=20140430T06=. This kind of form can be used for specifying special
behaviour near the end of the suite, at the final cycle's date/time. We can
also represent this in Cylc with a collapsed form:
\lstinline=R[limit?]/[interval]=
\lstinline=R[limit?]//[date-time]=
\lstinline=[interval]/[date-time]=
...
[scheduling]
    [[dependencies]]
        ...
        # Repeat once at the final cycle
        [[[ R1//+P0D ]]]
            graph = "cold_foo => foo"

        # Repeat 5 times, every 1 day, ending at the final cycle
        [[[ R5/P1D ]]]
            graph = "daily_bar => daily_baz"

        # Repeat every 2 weeks ending at 00:00 following the final cycle
        [[[ P2W/T00 ]]]
            graph = "frequent_qux => frequent_wibble"

        # Repeat every 1 day ending at 00:00 following the final cycle
        [[[ R//T00 ]]]
            graph = "bidiurnal_wobble => bidiurnal_wubble"
\end{lstlisting}
\lstset{language=transcript}
...
on one-off synchronous start-up tasks, which have an associated cycle even
though they are non-cycling - see Figure~\ref{fig-test5}.
...
cycle, but some may depend on others with other cycles. This notably
applies to warm-cycled forecast models, which depend on their own 
previous instances (see below); but other kinds of intercycle
...
beyond the initial cycle. That said, the presence of an inter-cycle
...
assigned a cycle (even thought they don't cycle) whereas {\em
...
can in principle have its inputs satisfied by a previous instance of
itself, {\em or} by a cold-start task with (nominally) the same cycle.
...
suite state, cylc loads all cycling tasks at a given cycle assuming
...
to trigger off \lstinline=A= at a future cycle that does not exist. To avoid
...
Initial and final cycles, if supplied via the suite.rc file or the
...
\begin{myitemize}
    \item \lstinline=[T]= and \lstinline=[T+n]= \newline
        Where \lstinline=n= is an integer cycle offset, the unit
        ...
        actual offset cycle of the task. The syntax is the same as
        for cycle offsets in the suite graph.
...
their dependencies and valid cycles.  Nevertheless, it is legal to
...
definition, and for cycling tasks they should contain the cycle in
...
a placeholder for the cycle, with the actual message emitted by the
task, which uses \lstinline=$CYLC_TASK_CYCLE_TIME=. The registered
message string is held in the suite daemon to be matched with
incoming task messages - it cannot contain shell variables because
...
cycle offsets in message outputs.
...
name, cycle, and a submit number that increments if the same task is
...
If a newly spawned task's cycle is ahead of the oldest non-finished
...
into the running suite. If a task definition (and its use in the graph) is
deleted, existing task proxies of the of the deleted type will run their
...
given name, or all tasks with a given cycle; see broadcast command help
for details.

Broadcast settings targetted at a specific task ID or cycle expire and
are forgotten as the suite moves on. Untargetted variables and those
...
warm start) the cycle at which it starts can be given on the command line or
hardwired into the suite.rc file:
\begin{lstlisting}
...
In the case of {\em cold starts only} the initial cycle will also
...
cold-start cycle is stored in suite state dump files.
...
configurable rate and offset relative to the suite's initial cycle.
...
in suite \lstinline=other.suite= at the same cycle. Other task states can be
polled like this,
\begin{lstlisting}
...
To trigger off remote tasks with different cycles just arrange for
...
remote task \lstinline=dog= with cycles of \lstinline=3,9,15,21=
hours,
\begin{lstlisting}
[scheduling]
    [[ dependencies]]
...
\end{lstlisting}
This results in \lstinline=DOG= having cycles of
...
\begin{myitemize}
    ...
    \item Handling spin-up processes via temporary tasks and adding
        ...
        the initial cycle can be in the past; and (b) you can give
        a final cycle after which the task will be eliminated from
        the suite.
...
same cycle. In other words, failure at any point during execution
...
wrong cycle, its restart files will be corrupted such that the correct cycle
can no longer run (probably necessitating a cold-start).
Instead, consider organising restart files by cycle, through a file
...
however, perhaps by use of a hardwired non cycle dependent temporary file or
similar, then it can be forced to run sequentially -
see Section~\ref{SequentialTasks}.

\subsection{Put Task Cycle In All Output File Paths}
\label{PutCycleTimeinIO}

Putting task cycle in output file or directory names makes archiving and
cleanup easier, and it facilitates task re-runnability and avoidance of
previous-instance dependence (above) by ensuring that important files do not
get overwritten by the next cycle.

The \lstinline=cylc cycletime= command computes offsets (in hours, days,
months, and years) from a given or current date/time cycle, and optionally
inserts the resulting computed date/time, or components of it, into a
...
by cycle date/time''.  Under cylc, a product generation task will trigger as
...
have finished or have yet to run. If your product delivery or presentation
system demands that all products for one cycle are uploaded (or whatever)
before any from the next cycle, then be aware that this may be quite
inefficient if your suite is ever faced with catching up from a significant
delay or running over historical data.
...
generation tasks at the same cycle, and then declare it to be sequential
(see Section~\ref{SequentialTasks}.

\subsection{Clock-triggered Tasks Wait On External Data}

All tasks in a cylc suite know their own private cycle, which is usually a
date/time, but most don't care about the wall clock time - they just run when
their prerequisites are satisfied. The exception to this is
{\em clock-triggered} tasks, which wait on a wall clock time expressed as an
offset from their own cycle as a real-life date/time, in addition to any other
...
\begin{myitemize}
    ...
    \item Right-click on a task and choose the ``Focus'' option to restrict
        the graph display to that task's cycle. Anything interesting happening
        in other cycles will show up as disconnected rectangular nodes to the
        right of the graph (and you can click on those to intantly refocus to
        their cycles).
...
%instantiated at the initial cycle, or at the next subsequent valid
%cycle for the task. Any tasks that have no prerequisites (and, if
...
%\end{lstlisting}
%
%Each entry shows the time of logging, the name and cycle of the
