...
\begin{figure}
    \begin{center}
        \includegraphics[width=1.0\textwidth]{screenshots/dbviewer.png}
    \end{center}
\caption[The cylc dbviewer GUI]{\scriptsize
The cylc dbviewer GUI, showing one suite running on port 7766.}
\label{fig-db}
\end{figure} 
...
\end{myitemize}
This module is currently packaged with cylc under
\lstinline=$CYLC_DIR/ext=, with installation instructions. If it is
installed cylc will automatically use it in place of a slower ordered
dictionary implemented in Python. 
...
Site and user config files can be auto-generated, with all settings
intially commented out, by running this command:
\begin{lstlisting}
% cylc get-global-config --write-site/--write-user
\end{lstlisting}

The content of both of these config files, in terms of legal items and
...
To view the content of the resulting suite database, run 
\lstinline=cylc db print= command or its GUI counterpart
\lstinline=cylc db dbviewer=:
...
Note that the graphviz build, reportedly, may fail on systems that do
not have QT installed, hence the \lstinline@./configure --with-qt=no@
option above.
...
Right-click on the {\em examples.QuickStart.a} suite in the db viewer and choose
...
Now start a suite control GUI by right-clicking on the suite in the db viewer
...
you like. If you start a suite via the db viewer, however, the output is 
...
\begin{figure}
    ...
    \end{center}
\caption[ A cylc suite log from the db viewer.]{\scriptsize A cylc suite log from the db viewer.}
\label{fig-suite-log}
\end{figure}

Figure~\ref{fig-suite-log} shows a suite log viewed from within db viewer. The 
...
\begin{lstlisting}
% get-config -i '[runtime][GetData][job submission][log directory]' examples.QuickStart.a 
...
By right-clicking on a suite in your database, from within the db
viewer, or using cylc commands, you can:
\begin{myenumerate}
...
Note that the suite title shown in the db viewer is parsed from the suite.rc
file at the time of initial registration; if you change the title (by
editing the suite.rc file) use \lstinline=cylc db refresh= or the db viewer's 
...
or suite groups in the db viewer GUI, as shown in Figure~\ref{fig-db}.
...
with a list of names in the section heading. Particular tasks can then
be singled out at run time by testing the
...
[runtime]
    [[ensemble]]
        command scripting = "run-model.sh ic-$CYLC_TASK_NAME"
    [[m1, m2, m3]]
        inherit = ensemble
\end{lstlisting}
...
suite with Jinja2 variables worked out.
...
Note that cycle time in the output message registration is 
expressed by a special placeholder variable (see
Section~\ref{SPHV}) not by the corresponding environment variable. This
is because registered output messages
...
open (cylc starts grabbing ports at number 7766 by default.\footnote{The
...
\subsection{Determining The Suite Network Port}

All Pyro communication with a running suite requires knowing the network
port on which the suite is listening. Running tasks know their own
suite's port number because the suite puts it in the task execution
environment, but user commands must determine the port number somehow.
Through to the 4.5.1 release, inclusive, cylc used port scanning to find
the target port, but this could potentially cause delays on hosts
with a large number of suites running at once. Consequently suites now
write their own port number to \lstinline=$HOME/.cylc/ports/<SUITE>= at
start-up, and commands issued on the suite host read this file to get
the port number. Commands issued on a remote host use passwordless ssh 
to the suite owner account to retrieve the port number. Alternatively,
if passwordless ssh is not configured to the suite host the port number
can be given on the command line with the \lstinline=--port= option. 
If you accidentally delete a suite's port file while the suite is still
running, use \lstinline=cylc scan= to determine the port number to 
...
cylc commands or db viewer the entire suite definition directory
