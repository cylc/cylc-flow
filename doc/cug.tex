...
gcylc showing a user's suite database, with one suite running 
on port 7766.}
\label{fig-db}
\end{figure} 
...
for suite graphing and the graph-based control GUI view.  For example
...
\subsection{User Suite Database}

Each cylc user has a suite database for organising with her own
...
intention, it was decided to remove the functionality entirely for the
following reasons:
...
\begin{myitemize}
    ...
    \item Small groups of light cylc users, on the other hand,
        can easily just share suites by means of copying suite
        definition directories from mutually readable storage locations
        (their home directory spaces, typically); they are then free to
        use preferred revision control tools and so on as they see fit.
\end{myitemize}
