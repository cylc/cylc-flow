...
``node groups'' can also be defined for non-family groups.  Default styling can
be applied using `default node attributes' or by styling the `root' family.
Note that order determines precedence so families should be styled {\em before}
their member nodes, or else the family styling will override that of the members.
