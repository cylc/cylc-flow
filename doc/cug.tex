...
\begin{myitemize}
 ...
 monthly, yearly...) - see Section~\ref{OtherCyclingModules}.
\end{myitemize}
...
initial cycle time) provided that the initial cycle time was always at 00 and
12 hours.
...
point is earlier than the suite's initial cycle point and so it cannot run. This
...
\begin{lstlisting}
[scheduling]
    initial cycle time = 20200401
    final cycle time = 20200405
    [[dependencies]]
...
using the \lstinline=R1= notation (e.g. as in Section
\ref{initial-non-repeating-r1-tasks}. For example:
\lstset{language=suiterc}
\begin{lstlisting}
[cylc]
    UTC mode = True
...
However, what if we have a staggered start (e.g. a task set beginning at \lstinline=T00=,
and another at \lstinline=T12=) with multiple cycles needing to feed off a
starting task or tasks?
...
if the interval is null, this implies the task at the initial cycle time.
...
[scheduling]
    ...
    [[dependencies]]
        [[[R1]]]
            graph = "prep"
        [[[R1/T00]]]
            graph = "prep[] => foo"   # prep[] implies prep at the initial cycle time
        [[[R1/T12]]]
            graph = "prep[] => baz"   # prep[], as above
        [[[T00]]]
            graph = "foo[-P1D] => foo => bar"
        [[[T12]]]
            graph = "baz[-P1D] => baz => qux"
...
\begin{lstlisting}
[scheduling]
    initial cycle time = 20100808T06Z
\end{lstlisting}
