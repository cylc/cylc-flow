...
\subsubsection{Optional But Recommended}
...
\begin{myitemize}
    ...
    \item {\bf Jinja2}, a template processor for Python (version tested:
...
The Jinja2 template engine adds powerful structural logic to cylc suites
- you can use variables, mathematical expressions, loop control
structures and conditional expressions to generate the final suite
definition. 
...
Suite.rc file content conforms to the ConfigObj extended INI format
(\url{http://www.voidspace.org.uk/python/configobj.html}) 
...
directives that have already been defined earlier in the file.

Additionally, embedded template processor expressions may be
preprocessed out to programatically generate the final suite definition
...
# ...
\end{lstlisting}
See {\em Programatic Suite Generation With Jinja2}
...
\subsection{Programatic Suite Generation With Jinja2}
\label{ProgSuiteGenJinja2}

The Jinja2 template processor adds powerful structural logic to cylc
suites. You can use variables, mathematical expressions, loop control
structures and conditional expressions to generate the final suite
definition seen by cylc. Potential uses include automatic generation
...
Jinja2 is well documented at \url{http://jinja.pocoo.org/docs}, so we
just provide an example suite here that uses it. How the suite works
should be reasonably self-evident to anyone familiar with standard
...
central suite database) includes or excludes special clean up tasks
...
name to list at the top of the file.
