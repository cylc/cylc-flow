...
start-up. Hey cannot be used in conditional trigger expressions with 
...
\subsection{Remote Control And Secure Passphrases}

Use of secure passphrases is now compulsory (a random passphrase is
generated automatically when a suite is registered, if one does not
already exist in the suite definition directory). This means cylc can
allow control of suites from remote hosts and other user accounts.
The suite passphrase must be installed in any account that requires
access to the suite - including remote task hosts. 

Cylc's remote task messaging commands look for the passphrase at 
\lstinline=$CYLC_SUITE_DEF_PATH/passphrase=; if not found there (e.g. if
the suite definition is not installed on a remote task host) they will
look in \lstinline=$HOME/.cylc/SUITE_HOST/SUITE_OWNER/SUITE/passphrase=;
and then in \lstinline=$HOME/.cylc/SUITE/passphrase=. The latter
locations are convenient for remote suite control, in which case the
suite definition directory will not be known. Finally, you can specify
the passphrase location on the cylc commandline.
...
\subsection[Suite Security]{Suite Security and Secure Passphrases}

Cylc now restricts access to running suites to host accounts holding a special
passphrase with secure permissions (as for ssh keys).  Remote task host
accounts, and any accounts used to remotely control a suite via cylc commands
or GUI, must all use the same passphrase.  The passphrase itself is never
transferred across the network (a secure MD5 checksum is). This should 
guarantee suite security so long as your user accounts aren't breached. 

A random passphrase is generated automatically when you register the suite; it
must then be distributed to any remote task or remote control hosts that will
need to connect to the running suite.
