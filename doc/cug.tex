...
\section{Cycling: Dynamic vs Static }
...
\end{myitemize}
And there are several ways this could be achieved in principle:
\begin{myitemize}
    \item {\em Dynamic Cycling:} A workflow can be extended as needed at run
        time, {\em potentially indefinitely}.  Each time a task runs it will be
        assigned a different cycle point. For example, a task called
        \lstinline=model= might repeat yearly with cycle points
        \lstinline=2021=, \lstinline=2022=, etc.  Dynamic cycling has
        traditionally involved repeating entire single-cycle workflows
        sequentially, but cylc does it without any hard cycle boundaries -
        see Section~\ref{HowCylcWorks}.

    \item {\em Static Cycling:} If a workflow is finite and not too big it
        can be pre-defined as a single monolithic workflow, so that the
        scheduler does not need to cycle anything at run time. (This is how
        workflow tools that do not know about cycling can still run cycling
        tasks).  Each model run is represented by a different task:
        \lstinline=model_runA= might represent the \lstinline=2020= run,
        \lstinline=model_runB= the \lstinline=2021= run, and so on; or the
        tasks might be called \lstinline=model_1=, \lstinline=model_2=, etc.,
        with integer values mapping to the proper date-time cycle points; or
        even \lstinline=model_2020=, \lstinline=model_2021= etc. {\em It is not
    ...
    \item {\em Mixed Cycling:} Both static and dynamic cycling can be combined
        ...
        could have a daily dynamic cycle with four static ``chunks'' inside
        each daily cycle point.
\end{myitemize}
...
Static cycling in cylc can be configured by writing out every task
individually; or by expanding {\em suite parameters} to auto-generate tasks
(statically, before the scheduler starts up) - see Section~\ref{Parameterized
Tasks}, or with Jinja2 loops to auto-generate tasks (statically again) - see 
Section~\ref{Jinja2}.

Dynamic cycling is the most powerful and flexible method. It is the only good
way to run large operational suites that need to continue cycling indefinitely;
cylc does the difficult date-time arithmetic automatically; and there is a
performance advantage too (no need to manage and monitor {\em all} the tasks in
