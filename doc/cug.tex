...
\begin{myitemize}
    ...
    \item Figure~\ref{fig-cylc-vim} - a cylc suite definition in the vim editor.
    \item Figure~\ref{fig-cylc-vim2} - a cylc suite definition with embedded Jinja2 expressions, in vim.
...
\begin{figure}
    \begin{center}
        \includegraphics{screenshots/commandhelp.png}
    \end{center}
...
\begin{figure}
    \begin{center}
        \includegraphics[height=0.35\textheight]{screenshots/suiterc-simple.png}]
    \end{center}
...
\begin{figure}
    ...
    \end{center}
    \caption[A cylc suite definition with embedded Jinja2 expressions, in {\em vim}]{\scriptsize
    A cylc suite definition with embedded Jinja2 expressions {\em vim}}
    \label{fig-cylc-vim2} 
\end{figure} 
...
\begin{myitemize}
    \item {\bf A Linux or Unix OS.} Cylc assumes Unix-style file paths
        in places, and a few command scripts are written in the Bash shell.
        In principle it could be made more portable without much effort.
        Cylc has run successfully on Mac OSX.
...
\end{lstlisting}
where \lstinline=<AdminUserName>= should be replaced with the username of the
cylc installation account on your system (the suite owner's username is always
the first registration name component for suites in the central
database); replace this as appropriate for your local cylc installation. 
Note that the dots in the cylc release version number are replaced, in central 
database suite registration names, with hyphens (because `.' is the registration
name delimiter). Type \lstinline=cylc db print --help= to see what the
...
and the graph based suite control GUI.  For example (change your admin 
username and cylc version number appropriately, after inspecting the 
content of the new central database):
\begin{lstlisting}
...
copying the old central database into the new installation 
(\lstinline=$CYLC_DIR/CDB/=),\footnote{The
central database is currently associated with the cylc
installation; in future we will separate the two.}
and running the new \lstinline=cylc admin create-cdb= to
load the new example suites. Finally, change the newly registered
central suite group to something sensible using  
\lstinline=cylc db reregister= - it will have an ungainly string of
digits attached to ensure uniqueness. 

\subsection{Pseudo Backward Compatibility}
\label{compat}
...
version (technically this also provides pseudo forward compatibility).
This allows construction of new-version suites while older existing ones
are still in use, without having to reconfigure your environment or
explicitly invoke different cylc versions for different suites.
...
\end{lstlisting}
And on the {\em second line} for Jinja2 suites:
\begin{lstlisting}
#!Jinja2
#!cylc-4.2.0
# suite definition follows ...
\end{lstlisting}
If cylc releases are installed in parallel under the same top level
directory then the location of the required cylc installation is
automatically computed from that of the invoked one. Otherwise,
for unrelated install locations, the full path can be specified:
\begin{lstlisting}
#!/path/to/cylc-4.2.0
# suite definition follows ...
\end{lstlisting}
...
determines the legality of requested sub-commands prior to version
compatibility processing (which is in fact done by the
sub-command\footnote{This is because the top level command interface
just passes options and arguments, which vary according to the
sub-command, on to the sub-command; it does not understand them
itself and so it cannot determine which one, if any, is the 
suite reference.}) so the correct cylc version must be explicitly
invoked in the unlikely event that the sub-command has undergone a name
change between the versions concerned. 

\subsubsection{A Usage Suggestion}

You can use the Jinja2 template processor (see Section~\ref{Jinja2}) to
store the required cylc version in a single variable, which can then 
also be used (for example) to specify the right cylc version to use on a
remote task host:
\lstset{language=suiterc}
\begin{lstlisting}
#!Jinja2
...
\lstinline=<AdminUserName>= below with the correct username for your
system) during cylc installation, 
...
as the TARGET suite name, and \lstinline=$HOME/suites= (or some other
...
\begin{lstlisting}
% cylc db import <AdminUserName>.cylc-x-y-z.QuickStart QuickStart $HOME/suites
...
situation for any suite in fact), so you can run multiple copies of 
the same suite at once without any interference between them. 

Use \lstinline=gcylc= or \lstinline=cylc db print --central <AdminUserName>= to
see the example suites that are currently available. Now 
...
<AdminUserName>         
 `-cylc-x-y-z   
   `-QuickStart 
     ...
     |-c   Quick Start Example C | ~/cylc/CDB/<AdminUserName>/cylc-x-y-z/QuickStart/c
     `-z   Quick Start Example Z | ~/cylc/CDB/<AdminUserName>/cylc-x-y-z/QuickStart/z
% cylc db import <AdminUserName>.cylc-x-y-z.QuickStart.z eg.z $HOME/suites
...
In order to allow secure remote control of running suites, cylc requires
that each suite has an associated passphrase file. The content of the
passphrase file is essentially arbitrary, so when a suite is registered
a passphrase is generated automatically in the suite definition
directory, if one does not already exist there. The same passphrase file 
must be distributed to any other user accounts on the local or remote
hosts from which you need access to the running suite. For more information 
see Section~\ref{passphrases}.


\subsection{Private Suite Database}

Each cylc user has a private suite database for working with his/her own
...
\subsubsection{Private Database Location}

Your private suite database, currently, will be \lstinline=$HOME/.cylc/LDB/db=.

\subsection{Central Suite Database}
...
\subsubsection{Central Database Location}

The central suite database is currently located here:
\lstinline=$CYLC_DIR/CDB/db=, and is accessed through the filesystem 
on the cylc host machine. Consequently it has to be world, or at least
group, writeable, which is not very secure.
...
database so that users can easily share suites across the network}. In
the meantime, ``importing'' a suite manually from another user on
another host is simply a matter of copying the suite definition
directory over, registering the local copy in your private database, 
and then exporting it to the local central database.
...
tasks the work directory is automatically removed again {\em if it
...
\end{lstlisting}
These can be used, for example, for a cleanup task to determine whether
or not it is running for the final time.
...
Jinja2 variable values can be modified by filters. Some built in filters
are available (see Jinja2 documentation), and cylc supports loading of
custom Jinja2 filters.  These should be defined as Python functions (the
function name defines the filter name) in a module of the same name under 
a \lstinline=Jinja2/= sub-directory of the suite definition directory, or
for use by mulitple suites, in \lstinline=$HOME/.cylc/Jinja2=.

The example suite \lstinline=$CYLC_DIR/examples/jinja2/filter= shows how 
to use a custom filter to left-pad integer values with '0' characters,
for use in generated task names.
...
\begin{myitemize}
    ...
    \item {\bf base} (Graph Control GUI only) - off-white base color
        graph nodes can be used to connect the nodes representing
        current live task proxies according to the structure of the suite).
\end{myitemize}

\subsection{Suite Security: Passphrases}
\label{passphrases}

Cylc restricts access to running suites (by running tasks, cylc commands
and GUI) to host accounts holding a special passphrase with secure
permissions.  The passphrase itself is never transferred across the
network (a secure MD5 checksum is). This should guarantee suite security
so long as your user accounts aren't breached. 

A random passphrase generated automatically when a suite is registered,
if one does not already exist in the suite definition directory. 
The suite passphrase must be installed in any account that requires
access to the suite - including remote task hosts. 

The passphrase can be held in any of the following locations, in
order of preference:
\begin{myitemize}
    ...
    \item (or given on the commandline)
\end{myitemize}
(the suite hostname should be the fully qualified domain name). The
latter locations are suitable for remote suite control in which case
the suite definition directory may not be known or accessible.

\subsection{Limiting Suite Activity}
...
\subsection{Other Topics In Brief}
