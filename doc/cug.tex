...
(e.g. ``current cycle'' or ``current cycle time''). Here, we use
...
************************************************************************
*              The Cylc Suite Engine [5.4.13-113-g89224]               *
...
************************************************************************
SUBMIT No.1(1,1): ( perl -e "setpgrp(0,0);exec(@ARGV)" $HOME/cylc-run/tut.oneoff.basic/log/job/hello.1.1 </dev/null 1>$HOME/cylc-run/tut.oneoff.basic/log/job/hello.1.1.out 2>$HOME/cylc-run/tut.oneoff.basic/log/job/hello.1.1.err & echo $!; wait )
Stopping: 
  + all cycling tasks have spawned past the final cycle 1
Requesting Job Submission stop - ignoring remaining items (currently 0)
Requesting Event Handlers stop - after remaining items (currently 0)
Requesting Poll & Kill Commands stop - after remaining items (currently 0)
Thread-4 exit (Job Submission)
Thread-2 exit (Event Handlers)
Thread-3 exit (Poll & Kill Commands)
Suite shutting down at 2014-06-04T10:18:27.142923Z
DONE
\end{lstlisting}
...
stdout. More detailed information is written, time-stamped, to
...
shell$ cylc run --no-detach tut.oneoff.basic 
************************************************************************
*           The Cylc Suite Engine [5.4.13-113-g89224-dirty]            *
*             Copyright (C) 2008-2014 Hilary Oliver, NIWA              *
*                                                                      *
* This program comes with ABSOLUTELY NO WARRANTY.  It is free software;*
* you are welcome to redistribute it under certain conditions. Details:*
*          `cylc license conditions'; `cylc license warranty'          *
************************************************************************
SUBMIT No.1(1,1): (echo "perl -e \"setpgrp(0,0);exec(@ARGV)\" $HOME/cylc-run/tut.oneoff.basic/log/job/hello.1.1 1>$HOME/cylc-run/tut.oneoff.basic/log/job/hello.1.1.out 2>$HOME/cylc-run/tut.oneoff.basic/log/job/hello.1.1.err" | at now)
Stopping: 
  + all cycling tasks have spawned past the final cycle 1
Requesting Job Submission stop - ignoring remaining items (currently 0)
Requesting Event Handlers stop - after remaining items (currently 0)
...
Thread-2 exit (Event Handlers)
Thread-3 exit (Poll & Kill Commands)
Suite shutting down at 2014-06-04T10:48:14.944483Z
DONE
\end{lstlisting}
...
|   |-- state
|   |-- state.20140604T104814.945181Z
|   `-- state.20140604T104814.942055Z
...
JOB SCRIPT STARTING
cylc (scheduler - 2014-06-04T11:20:29Z): goodbye.1 started at 2014-06-04T11:20:29Z
cylc Suite and Task Identity:
  Suite Name  : tut.oneoff.basic
  Suite Host  : server0
  Suite Port  : 7766
  Suite Owner : hilary
  Task ID     : goodbye.1
...
Goodbye World!
cylc (scheduler - 2014-06-04T11:20:39Z): goodbye.1 succeeded at 2014-06-04T11:20:39Z
...
will have a succession of cycle points (times) ending in $00$ or $12$ hours,
between specified initial and final cycle points (or indefinitely, if no
final cycle point is given), as shown in Figure~\ref{fig-tut-one}.
...
\lstinline@A[+PT6H] => B@ (see~\ref{InterCyclePointTriggers}).
...
(\lstinline@model:out1 => post1@ etc.; see~\ref{InternalTriggers}), but this
...
see {\em Reporting Internal Outputs Completed} (\ref{RIOC}).
...
[dependencies]
    [[PT6H]]
        ...
        graph = "A[-PT6H] => B"
\end{lstlisting}
inter-cycle-point and trigger type (and internal output) notation can be combined:
\begin{lstlisting}
...
restart files are completed (see~\ref{RIOC}) there's no need to wait for it to
finish:
\lstset{language=suiterc}
\begin{lstlisting}
[scheduling]
...
(\ref{TaskExecutionEnvironment}) - then to rename a task you
...
$CYLC_TASK_NAME                  # X
$CYLC_TASK_CYCLE_TIME            # 20110511T1800Z
...
definition, and for cycling tasks they should contain the cycle point in
...
a placeholder for the cycle point, with the actual message emitted by the
...
See~\ref{ModelRestartDependencies} for an example that uses
...
This section explains how tasks are submitted by cylc when they are ready to
