...
Embedded Jinja2 template processor expressions add general variables,
mathematical expressions, loop control structures and conditional
expressions, to cylc suite definitions. Suite.rc files containing Jinja2
expressions are automatically preprocessed to generate the final suite
definition seen by cylc. Potential uses include automatic generation of
repeated groups of similar tasks and dependencies, and inclusion or
exclusion of entire suite sections according to the value of a single
flag. 
...
inline Jinja2 code should be reasonably self-evident to anyone familiar
with standard programming techniques. 

The need for Jinja2 processing must be declared with a bang-hash
comment in the first line of the suite.rc file:
\begin{lstlisting}
#!Jinja2
# ...
\end{lstlisting}
...
definition, so Jinja2 expressions can appear anywhere in the file, even
inside strings and namespace headings for example.
...
Figure~\ref{fig-jinja2-ensemble}, shows how an ensemble of similar task
can be generated using Jinja2:
\lstset{language=suiterc}
\begin{lstlisting}
% cylc view --stdout --central cylc-4-0-0.jinja2.ensemble
#!jinja2
{% set N_MEMBERS = 5 %}
[scheduling]
    [[dependencies]]
...
\lstset{language=suiterc}
\begin{lstlisting}
% cylc view --stdout --processed --central cylc-4-0-0.jinja2.ensemble
#!jinja2
[scheduling]
    [[dependencies]]
        graph = """
