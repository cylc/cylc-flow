...
exit status if any of the following conditions occur:

\begin{myitemize}
    \item cylc fails (this always causes an abort)
    \item any task fails (unless told not to abort)
    \item the suite times out (which may indicate that a task has died
        without reporting failure to the suite)
    \item a nominated shutdown event handler, executed in the foreground
        so that cylc can detect its exit status, reports failure
\end{myitemize}
...
log to that from an earlier reference run.  This triggering information
disregards the timing and order of events that are not directly causally
connected, so a correct run will still verify as correct regardless of
how cycle interleaving, for example, varies according to the external
queing conditions and so on.

To prepare a reference log for a suite just run it with the
\lstinline=--reference-log= option, and manually verify the
correctness of the reference run.

To run a reference test just run the suite (in dummy mode for the most
...
note that you can change the default shutdown event handler, and
