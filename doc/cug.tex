...
incompatible versions of cylc set \lstinline=$CYLC_VERSION= in your environment
to the desired version. In the case of developers wishing to run their own copy
of cylc rather than a centrally installed one set \lstinline=$CYLC_HOME= in 
your environment to point to your cylc copy.
...
[SECTION]
    ...
    a very very long item = value2
\end{lstlisting}
    It does not reflect nesting level clearly, and long items push
    everything off to the right. The following may be acceptable
    though as it preserves the proper indentation:
\lstset{language=suiterc}
\begin{lstlisting}
[SECTION]
    a short item          = value1
...
[runtime]
    [[foo]]
        command scripting ="""
            ...
            echo Goodbye World!"""
\end{lstlisting}
        If taken as written this whitespace translates to spurious indentation
        in the task job script. As it happens this is just an aesthetic
        problem in bash scripts, but for Python job scripts (a possible 
        future enhancement) it would be a technical error.
...
echo Goodbye World!"""
        # or (includes a single newline character):
        command scripting ="""
echo Hello World!
echo Goodbye World!"""
\end{lstlisting}
   ...
   \end{myitemize}
    ...
    \begin{myitemize}
        ...
        \item Avoid {\em full page width ``section divider'' comments} - 
            ...
            for text editors auto-line-breaking on a lesser width.
