...
consists of a repeating set of tasks with dependencies: each task takes
input from one or more ``upstream'' tasks, and supplies output to one or
more ``downstream'' tasks. A metascheduler is a control system that
ensures tasks are not submitted to the batch queue scheduler(s) until
they are ready to execute*, i.e. when their prerequisites have been
satisfied by other tasks in the system. This paper describes a
metascheduling algorithm that ensures optimal operation of a complex
continuously cycling multi-model environmental forecasting system
without dictating execution order or requiring explicit definition of
the dependencies between tasks. Each task simply registers its own
...
historical case study periods, with maximum throughput. The absence of
explicit task sequencing logic or prescribed dependencies lends great
