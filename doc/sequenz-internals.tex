...
Metascheduling is the process of deciding when a task in a system of
tasks with dependencies is {\em ready} to be executed; i.e.\ when its
prerequisites have been satisfied by other tasks in the system*. This
paper describes an extremely flexible metascheduling algorithm that
...
and proxy objects then negotiate dependencies dynamically so that
correct execution order emerges naturally at run time. The method is
optimal in that tasks can be submitted the instant their prerequisites
are satisfied regardless of their associated ``forecast cycle'' or any
other consideration. The entire system can therefore catch up to real
time or run over historical case study periods with maximum throughput:
tasks from many different forecast cycles can run simultaneously where
dependencies allow. The absence of explicit task sequencing logic or
prescribed dependencies means that tasks can be switched on and off at
will, new tasks can be added without any change to the algorithm, and
partial parallel test systems that feed off the main operational system
can be created very easily. Our metascheduling controller, {\em
Sequenz}, can be interfaced to existing tasks (models etc.) with minimal
overhead. It is written in object oriented Python and uses the Python
Remote Object protocol ({\em Pyro}) to control tasks on multiple
platforms at once. 

(*) We are not using the more recent terminology referring to ``a single
...
From a control system design perspective, environmental forecasting
systems present something of a challenge. They consist of the forecast
models themselves, and any number of pre- and post-processing tasks,
each with an associated "forecast reference time" (this is generally the
nominal analysis time or start time for the forecast models themselves,
and for pre- and postprocessing tasks it is the reference time of the
associated forecast tasks). Each task has a regular series of reference
times at which it executes (e.g. 00, 06, 12, and 18 hours every day),
but this is not generally the same for every task in the system (for
instance, a global atmospheric model might run only twice per day, but
may drive regional models that run four times per day). For a particular
reference time, intertask dependencies dictate a pattern of execution
forming a Directed Acyclic Graph. However, forecasting systems are
continuously cycling and there are additional non-cotemporal
dependencies (between tasks with different reference times). Forecast
models ususally depend on their own previous instance, because each
forecast generates a ``background model state'' for the next. Post
processing tasks, on the other hand, don't depend on their own previous
instances, but they necessarily depend on upstream forecast tasks that
do. In addition, there may be some tasks that depend on non-cotemporal
tasks of another type (for instance, we have an hourly river catchment
model that takes input from the {\em most recent} 6-hourly regional
weather model, which it is allowed to run ahead of to a limited extent
as it assimilates incoming streamflow observations. Finally, there will
be some pre-processing tasks that have no upstream dependencies but are
...
For pragmatic reasons, such systems are usually simplified by allowing
only a single ``forecast cycle'' to run at a time, i.e. all tasks with
one reference time must complete before any from the next can start.
This would not normally affect real time operation(but see **) for which
there will be a gap between cycles anyway as the system waits on arrival
of real time observational data, or possibly on forecast fields from an
external system that is so constrained. However, this is a significant
impediment when catching up to real time operation after a delay, or
when running the entire system over historical case study periods; in
these situations external input for one or more cycles is immediately
available and consequently a lot of parallelism is possible if tasks
from multiple reference times can run at once (real dependencies
allowing). The biggest gain here is that successive instances of the
driving (upstream) forecast models can, in principle, run without delay,
regardless of ongoing downstream processing (dependent models and post
processing) associated with previous instances. Similarly, but
constrained but cotemporal upstream dependencies, the downstream
forecast models need not wait for an entire cycle to finish either, and
in, and in some cases successive instances of the same post-processing
task may be able to overlap (because these have no previous-instance
dependence; overlap can occur when a post processing task takes longer
to run than the delay between its upstream model starting and it starting).
