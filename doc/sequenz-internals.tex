...
A metascheduler controls submission of jobs to a batch queue scheduler.*
In a set of tasks with dependencies, a task is ready to be scheduled for
execution when its dependencies have been satisfied by other tasks in
the system. This paper describes a metascheduling algorithm that ensures
optimal operation of a complex continuously cycling multi-model
environmental forecasting system without dictating execution order or
requiring explicit definition of the dependencies between tasks. Each
...
scheduled for execution as soon as their prerequisites are satisfied
...
simultaneously where dependencies allow, and the entire system can catch
up to real time after a delay, or run over historical case study
periods, with maximum throughput. The absence of explicit task
sequencing logic or prescribed dependencies lends great flexibility:
...
that feed off the main operation can be created quickly. Our dynamic
