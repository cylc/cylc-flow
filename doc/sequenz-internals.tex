...
\begin{abstract}
    ...
    a set of tasks with dependencies are {\em ready} to be scheduled for
    ...
    not the topic of this paper.} Environmental forecasting systems are
    continuously repeating task sets with intra- and inter-cycle
    dependencies. Existing forecast control systems ignore the
    intercycle dependencies; this has no effect on normal real time
    operation but it dramatically reduces potential system throughput
    after a delay and is therefore a serious problem for operations that
    have little down time between forecast cycles. This paper describes
    a new metascheduling algorithm for continuously cycling forecasting
    systems that can run tasks from many different forecast cycles at
    once, where dependencies allow, to achieve maximum throughput in all
    modes of operation. It is also extremely flexible and widely
    applicable because it requires no system-specific scheduling logic:
    each task registers its own prerequisites without reference to other
    tasks in the system, and dependencies are negotiated dynamically as
    outputs are reported so that correct execution order emerges
    naturally at run time. Tasks can be added or removed without
