...
operation: for example, see the EcoConnect catchment model's unusual
mode of operation, below.


\subsection{Existing Systems}

It is natural to think about forecasting systems as a linear sequence
of distinct forecast cycles, because they necessarily are just that
...
at the time, but the vertical ordering of tasks is not meaningful.
 ...
 who must
guarantee that there are no dependency violations.  There could be some
means of crudely specifying tasks that can run in parallel but for a
realistic system it would be very difficult to get this to approach the
optimal single cycle job schedule.

The obvious design for a control system that could achieve the optimal
single cycle job schedule is, arguably, a Finite State Machine that
enforces a predetermined non-linear order of events as encoded in the
single cycle dependency graph, using hard coded logic: {\em IF task $b$
has finished AND task $c$ has finished THEN begin executing task $d$},
etc. Unfortunately though, this is highly system-specific, and hard
...
Considerably less optimal, in terms of scheduling, but potentially
more flexible, are systems that allows the user control over task order,
possibly with some means of crudely specifying which tasks can run in
parallel 


Both of these
designs are easy to comprehend and reasonably straightforward to
implement\footnote{We have glossed over many implementation details here
that are highly non-trivial, but they are nevertheless irrelevant to the
high level metascheduling algorithm.} and, as far as the author is
aware, most existing operational forecast control systems are variations
on one or the other (it is difficult to be sure of this, however,
because they are invariably system-specific in-house projects that are
not documented in the literature!).\footnote{DAGMan, which is part of
the Condor distributed workload management system [REF:...], is a
generic metascheduler for sets of jobs with dependencies, but it does
not appear to allow the kind of continuously dependent system described
in this paper.}
...
%the difference between their own run length and the maximum length of
%any cotemporal upstream execution paths that they depend on. 
