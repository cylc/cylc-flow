...
%Each task (forecast models, post-processing jobs, etc.) are represented
%by proxy objects that communicate with their real tasks across the
%network. Each task proxy is unaware of what other tasks exist in the
%system (tasks are is configured without reference to other tasks in the
%system (i.e.\ as if for standalone runs) but 
