...
%\title{An Optimal Dynamic Metascheduler for Continuously Cycling Multi-Model Forecasting Systems}
\title{A Next-Generation Control System For Cycling Multi-Model Forecasting Operations}
...
Environmental forecasting operations, from a job scheduling perspective,
consist of a large set of cycling (repeating) interdependent tasks for
which execution order must be carefully controlled to avoid dependency
violations.  A control system to automate this is a {\em metascheduler};
it decides, taking account of dependencies, when tasks are ready to be
sent to the job scheduler to be queued for execution.\footnote{The term
can also refer to {\it a single aggregate view of multiple Distributed
Resource Managers}, which is not the topic of this paper.} 

Normal real time operation consists of a series of distinct {\em
forecast cycles} in which all tasks have the same {\em reference time} -
the nominal forecast start time for a forecast model, or that of the
associated model forecast for data processing tasks. A new cycle is
typically initiated, after a gap in processing, by arrival of external
driving data (real time atmospheric observations, for instance), and
not all tasks necessarily run in every cycle (i.e. some tasks may run
more frequently than others). 

There are dependencies between, as well as within, forecast cycles (the
simplest example of this is that a forecast model typically depends on a
recent previous forecast for its initial ``background model state'').
These greatly increase the apparent complexity of the system, but
current control systems can ignore them because in normal real time
operation, where one cycle necessarily finishes before the next begins,
they are automatically satisfied.  
%\footnote{Taking account of all dependencies, however, does enable new
%modes of real time operation for which this isn't true.} 
In general, however, ignoring intercycle dependencies means that strict
sequential cycling is required even if the driving data for the next
cycle are available in advance, as often happens after delays. This can
be a serious problem for complex systems with little downtime between
cycles - it may take many cycles to catch up to ``on time'' operation
again. In principle, it should be possible to catch up very much faster
by interleaving tasks from successive cycles,\footnote{This also
applies, to an even greater extent, for historical case studies, and
parallel test systems that are delayed with respect to the main
operation: driving data is then available many cycles in advance.} but
to do so requires explicit handling of intercycle dependencies, in order
to avoid violating them. Unfortunately this removes the clear division
into distinct forecast cycles, making the system dependency graph an
unbounded continuous network that is too complex to handle with
traditional scheduling logic ({\em if A is finished and B is finished
then begin executing B}, and so on).

The new metascheduler presented here, {\em cyclon}, uses a novel dynamic
simulation-like algorithm that dramatically reduces the apparent
complexity of the general metascheduling problem just outlined; it is
able to present tasks to the scheduler(s) as soon as their prerequisites
are satisfied, regardless of forecast cycle, to achieve optimal
throughput in all modes of operation. The development of cyclon was
largely motivated by the need to interleave successive cycles of NIWA's
complex EcoConnect operation to achieve faster catchup from delays
(EcoConnect has little downtime between cycles). But, as a consequence
of its lack of system-specific scheduling logic, it is also extremely
flexible and widely applicable. Each task simply registers its own
prerequisites and reports its own outputs as they are generated, and
dependencies are negotiated dynamically so that correct execution order
emerges naturally at run time.
    %, with maximum functional parallelism,
{\em Cyclon} is easily interfaced to existing tasks (models etc.), and
control of complex systems, failure and recovery scenarios, etc., can
be tested quickly via a simulation mode that is indistinguishable, to
the controller, from real operation. Forecast model `restart'
dependencies are also handled explicity, which allows the system to
continue operating, with very little human intervention, over failures
that involve omitted cycles in the driving models. Cyclon is written in
Python and uses the Python Remote Object Protocol ({\em Pyro}) to
control tasks on multiple platforms at once. 
...
A control system should ideally queue tasks for execution as soon as
they are ready, but not before.  This is the best that can be done
whether or not the queued tasks begin executing immediately (which is
determined by the batch queue scheduler(s). A task is {\em ready} to
execute when its prerequisites have been satisfied by other tasks in the
system,\footnote{A task that depends on external data can be treated as
depending on the output of another task that waits on arrival of the
external data.} usually by generating, or otherwise making available,
required input data. Dependencies can exist between different tasks in
the same cycle (e.g.\ a sea state forecast might depend on wind fields
generated by a cotemporal weather forecast), between instances of the
same task in different cycles (e.g.\ a forecast model generally writes
out a ``background model state'' that is used in initializing its next
...
here depend on upstream tasks {\em finishing} rather than on any
...
operational delay of almost one whole forecast cycle on a sequentially
cycling system that normally has little downtime between cycles: it
takes many cycles to catch up again. Above the time axis is the optimal
schedule that is possible, in principle, when intercycle dependencies
are taken into account: the second cycle after the delay is hardly
affected, and subsequent cycles all run on time.  Note that simply
overlapping the single cycle schedules of Figure \ref{fig-time-one} from
the same start point would have resulted in a dependency violation in
task {\em c}.
