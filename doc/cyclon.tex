...
\begin{abstract}
    ...
    The control system for a forecasting operation is a form of {\em
    metascheduler} - it determines when members of a set of tasks with
    dependencies are {\em ready to be queued for execution}.\footnote{The
    term can also refer to {\it a single aggregate view of multiple
    Distributed Resource Managers}, which is not the topic of this
    paper.} Environmental forecasting systems consist of a set of tasks
    that repeat indefinitely in the presence of dependencies within and
    between cycles, and the cycle period may be task dependent.
    Intercycle dependencies greatly increase a system's apparent
    complexity, and existing forecast systems controllers ignore them
    because they are automatically satisfied in normal real time
    operation. This assumes a linear sequence of distinct forecast
    cycles at all times, however, which is very inefficient when
    catching up from operational delays or running over historical data
    (case studies); the driving data for the next cycle(s) are then
    available in advance and tasks from many different cycles may be
    able to run at once when all dependencies are accounted for.
    %particularly for systems that have little down time between
    %forecast cycles.  
    Our dynamic metascheduler, {\em cyclon}, is able to queue tasks for
    execution as soon as their prerequisites are satisfied, regardless
    of forecast cycle, to achieve optimal throughput in all modes of
    operation. It is also extremely flexible and widely applicable
    because it uses no system-specific scheduling logic: each task
    simply registers its own prerequisites and reports its own outputs,
    and dependencies are negotiated dynamically so that correct
    execution order, with maximum functional parallelism, emerges
    naturally at run time. {\em Cyclon} is easily interfaced to existing
    tasks (models etc.), and control of complex task sets, and failure
    scenarios, can be tested quickly via a simulation mode that is
    indistinguishable, to the controller, from real operation. Forecast
    models can register their own restart dependencies; this allows the
    system to continue running over operational failures that involve
    omitted cycles with very little human intervention. Cyclon is
