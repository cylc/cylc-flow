...
This document describes a new control system for forecasting operations,
{\em cyclon}, which implements a novel algorithm that avoids the
apparent complexities of multi-cycle metascheduling and automatically
provides optimal throughput, with maxim functional parallelism, in all
modes of operation. Each system task is configured simply, as if it were
to run alone, but registers its prerequisites with the controller and
reports outputs as they are generated. Dependencies are then negotiated
dynamically so that correct execution order emerges naturally at run
time and tasks are presented to the job scheduler(s) at the earliest
possible moment, regardless of forecast cycle.  The development of
cyclon was largely motivated by the need to run tasks from several
forecast cycles at once during catchup operation, for complex systems
with little downtime between cycles. Cyclon's lack of system-specific
scheduling logic, however, also makes it extremely flexible, easy to
use, and widely applicable. {\em Cyclon} can control tasks across a
heterogenous network is easily interfaced to existing tasks (models
etc.). Control of specific complex systems can be simulated in a dummy
mode that is indistinguishable, to the controller, from real operation.
Forecast model `restart' dependencies are handled explicity, which
allows the system to continue operating with very little human
intervention over failures that involve omitted cycles in the driving
models. Cyclon is written in Python and uses the Python Remote Object
Protocol ({\em Pyro}).
