...
\begin{abstract} This document describes a new control system for
forecasting operations, {\em cyclon}, which uses a novel simulation-like
algorithm to dramatically reduce the complexity of the general
metascheduling problem. It is able to present tasks to the job
scheduler(s) as soon as their prerequisites are satisfied regardless of
forecast cycle, to achieve optimal throughput in all modes of operation.
The development of cyclon was largely motivated by the need, for a
complex system with little downtime between cycles, to run tasks from
several forecast cycles at once during catchup operation. As a
consequence of its lack of system-specific scheduling logic, however, it
is also extremely flexible and widely applicable. Each task simply
registers its own prerequisites and reports its own outputs as they are
generated, and dependencies are negotiated dynamically so that correct
execution order emerges naturally at run time.
    %, with maximum functional parallelism,
{\em Cyclon} is easily interfaced to existing tasks (models etc.), and
control of complex systems, failure and recovery scenarios, etc., can be
tested quickly via a simulation mode that is indistinguishable, to the
controller, from real operation. Forecast model `restart' dependencies
are also handled explicity, which allows the system to continue
operating, with very little human intervention, over failures that
...
control tasks on multiple platforms at once.  \end{abstract}


\section{Environmental Forecasting Systems}

Environmental forecasting operations generate forecast products at
regular intervals using a set of linked scientific models and associated
data processing tasks\footnote{A {\em task} is any group of processes
treated as a single entity for scheduling purposes.} driven by real time
external data (observations and/or forecast fields from another
operation).  From a job scheduling perspective, this is a large set of
cycling (repeating) interdependent tasks whose execution order must be
carefully controlled to avoid dependency violations.  

Normal real time operation necessarily takes the form of a series of
distinct {\em forecast cycles} initiated, after a gap in processing, by
new external driving data (typically real time observational data and/or
forecast data from an external system). Each forecast cycle consists of
a set of tasks all with the same {\em forecast reference time}, which is
the nominal forecast start time for a forecast model, or that of the
associated forecast model for data processing tasks. Some tasks may run
more frequently than others (i.e.\ not all tasks necessarily run in
every cycle). 
...
from the Met Office (UK), and uses them to drive global sea state and
...
A control system that automates this can be termed a {\em metascheduler}
because it must determine, taking account of dependencies, when each
task is ready to be sent to the batch queue scheduler to be queued for
execution.\footnote{The term can also refer to {\it a single aggregate
view of multiple Distributed Resource Managers}, which is not the topic
of this paper.}  Ideally each task should be queued as soon as it is
ready, but not before; this is the best that can be done whether or not
the queued tasks can begin executing immediately, which is determined by
the batch queue scheduler(s). A task's state of readiness is determined
by its dependencies, i.e.\ whether or not its prerequisites have been
satisfied yet by other tasks in the system,\footnote{A task that depends
on external data can be treated as depending on the output of another
task that waits on arrival of the external data.}. Usually this means
other tasks must have generated, or otherwise made available, all
required input data. 
...
range; and a forecast product generation task clearly can't run before
the forecast itself is finished. Perhaps less obviously, because they
