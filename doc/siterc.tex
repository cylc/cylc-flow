...
The command template (with \%(job_id)s substitution) for testing the run time
stderr viewer functionality for this batch system.

\subparagraph[out viewer]{[test battery] $\rightarrow$ [batch systems] $\rightarrow$ [\_\_NAME\_\_] $\rightarrow$ out viewer}

The command template (with \%(job_id)s substitution) for testing the run time
stdout viewer functionality for this batch system.
