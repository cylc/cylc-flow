...
\subsection[{[[}cylc suite settings{]]}]{[cylc]}

Default values for entries in the suite.rc.

\subsubsection[{[[}cylc suite settings utc{]]}]{[cylc] $\rightarrow$ [UTC mode]}
\label{SiteUTCMode}

Allows you to set a default value for UTC mode in a suite at the site level.
See ~\ref{UTC-mode} for details.

\subsubsection[{[[}cylc suite settings event hooks{]]}]{[cylc] $\rightarrow$ [event hooks]}
\label{SiteCylcHooks}
