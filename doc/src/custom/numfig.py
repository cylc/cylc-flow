#
# This is a Sphinx extension 'sphinx-numfig' created by 'arjones6'., taken from
# https://bitbucket.org/arjones6/sphinx-numfig, with a few adaptions.
#
...
# Fix to source code as of 21/12/18. See:
# 'https://bitbucket.org/arjones6/sphinx-numfig/issues/6/ ...
    ...
    if app.builder.name != 'latex':
        for ref_info in doctree.traverse(num_ref):
            ...
            if app.builder.name == 'html':
                ...
                link = "%s#%s" % (
                    app.builder.get_relative_uri(docname, target_doc), target)
                html = '<a class="pageref" href="%s">%s</a>' % (
                    link, labelfmt %(figids[target]))
