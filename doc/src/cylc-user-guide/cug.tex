...
At restart (see \lstinline=cylc restart --help=) a suite daemon initializes its
task pool from a previously recorded checkpoint state. By default the latest
automatic checkpoint - which is updated with every task state change - is
loaded so that the suite can carry on exactly as it was just before being shut
down or killed.
...
Suite daemons automatically update the ``latest'' checkpoint every time a task
changes state, and at every suite restart, but you can also take checkpoints
at other times. To tell a suite daemon to checkpoint its current state:
\lstset{language=transcript}
\begin{lstlisting}
...
[runtime]
   ...
   [[checkpointer]]
      script = """
sleep 5  # important! (see below)
CHECKPOINT_NAME=cp-${CYLC_TASK_CYCLE_POINT}
cylc checkpoint ${CYLC_SUITE_NAME} ${CHECKPOINT_NAME}"""
\end{lstlisting}

Note that the checkpointer task is running at the time, and it will be recorded
as such in the resulting checkpoint. This doesn't matter, because on restart
the suite daemon will poll that task to determine that it finished successfully.

However {\em the \lstinline=sleep 5= above is required to ensure that the
checkpointer task is correctly recorded as running} - this is because the
``task started'' message that informs the suite daemon of running status is
sent back via a background process (so that network outages don't delay the
job's real work from starting). Without this short delay the checkpoint may
be taken just before the task hits the running state in the daemon, and the
checkpointer task will be recorded incorrectly in the waiting state. If this
happens {\em and} checkpointer's upstream dependencies have already been
removed from the suite (so they won't be recorded in the checkpoint) the task
will need to be manually reset from waiting to succeeded after the restart.
...
their true state conforms to what is currently recorded by the suite daemon.
See \lstinline=cylc poll --help= for how to poll one or more tasks manually, or
...
suite daemon itself is down when tasks finish execution.

To poll a task job the suite daemon interrogates the batch system, and the
...
daemon was down or unreachable on the network.
...
Suite daemons maintain two \lstinline=sqlite= databases to record restart
checkpoints and various other aspects of run history:
...
The private DB is for use only by the suite daemon. The identical public DB is
provided for use by external commands such as \lstinline=cylc suite-state=,
\lstinline=cylc ls-checkpoints=, and \lstinline=cylc report-timings=. If the
public DB gets locked for too long by an external reader, the suite daemon
will eventually delete it and replace it with a new copy of the private DB, to
ensure that both correctly reflect the suite state.
...
The public DB is not strictly required for a restart - the suite daemon will
recreate it if need be - but it is required by \lstinline=cylc ls-checkpoints=
if you need to identify the right restart checkpoint.
