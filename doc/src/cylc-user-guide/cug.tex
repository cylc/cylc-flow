...
separately in Section~\ref{External Triggering}.
...
\lstinline=wall_clock= external trigger function, which is more efficient
(trigger functions can be shared by many tasks) and is conveniently exposed in
the graph).}
...
\paragraph{External Event Triggers}

This is a substantial topic in its own right, documented in
Section~\ref{External Event Triggers}.
...
is often preferable to implementing long-running tasks in the workflow that
poll for external events. The triggering mechanism described in this section
replaces an older and less powerful one documented in~\ref{Old-Style
External Triggers}.

{\em WARNING - the suite configuration interface for this new capability
may change somewhat in future releases - see {\em Current Limitations} below
in~\ref{Current Trigger Function Limitations}.}
...
such as delivery of a new dataset, creation of new entries in some database
table, or appearance of new data availability notifications in a message
broker.

Cylc has built-in trigger functions for suite state polling (i.e.\ for
inter-suite triggering, see~\ref{Built-in Suite State Triggers}) and for clock
triggering (see~\ref{Built-in Clock Triggers}).

Trigger functions are just normal Python functions with the following
...
\begin{myitemize}
  ...
  \item they can be located in:
    ...
    \end{myitemize}
  \item they can take both positional and keyword arguments
  \item they should return quickly
  \item on failure they must return a tuple \lstinline=(False, {})=
  \item on success they must return a tuple \lstinline=(True, results)= where
    \lstinline=results= is a dictionary of event data to be passed to dependent
    tasks (how this appears to task jobs is described below)
\end{myitemize}

At runtime, the suite server program manages trigger function execution as
follows:
\begin{myitemize}
  \item trigger functions are called asynchronously in the process pool, so if
    one takes some time to execute it will not tie up the server program
    \begin{myitemize}
  ...
  \item they are subject to the normal process pool command time out - if they
    take too long to return, the process will be killed to prevent it from
    holding up Cylc resources 
  \item they are shared for efficiency: a single call will be made for all
    ...
    name and arguments
  \item return status and results persist across suite restarts
  \item trigger function stdout is redirected to stderr (because stdout
    is needed by the function wrapper to communicate returned data) and visible
    in the suite log in debug mode
\end{myitemize}

\subsection{Built-in Clock Triggers}
\label{Built-in Clock Triggers}

These replace the older deprecated clock triggers described
in~\ref{ClockTriggerTasks}.

The clock trigger function signature looks like this:
\begin{lstlisting}
wall_clock(offset=None, point_as_seconds=None)
\end{lstlisting}

Clock triggers are treated differently to other trigger functions in that they
are executed synchronously in the main process, and the cycle point of
dependent tasks is automatically passed to them (via the second function
argument above).
...
Notice that the label \lstinline=clock_1= is used to represent the trigger in
the graph. This is because trigger function signatures can be quite large (many
arguments).

Argument keywords can be omitted if called in the right order, so the
\lstinline=clock_1= trigger can also be defined like this: 
\lstset{language=suiterc}
\begin{lstlisting}
    [[xtriggers]]
...
in~\ref{SuiteStatePolling} for inter-suite triggering - i.e.\ to trigger tasks
off of events in other suites.
...
The first three arguments single out the target suite name
(\lstinline=suite=) task name (\lstinline=task=) and cycle point
(\lstinline=point=). The others are optional - they correspond to the
various arguments and options of the \lstinline=cylc suite-state= command.

The data returned by the function when the suite state trigger is satisfied
looks like this:
\begin{lstlisting}
    results = {
        'suite': suite,
...
\end{lstlisting}
i.e.\ the names and values of all of the target suite state parameters are
returned, in case they are needed by dependent tasks. Exactly how this is
passed to task job environments is described below. 
...
Note the time interval \lstinline=:PT10S= (10 seconds, which is also
the default) appended with a colon after the call signature. This is how often
the suite server program will call the trigger function to check if the
specified upstream suite state has been achieved yet.
...
waits on the task \lstinline=foo= in the upstream suite to achieve the default
\lstinline=succeeded= state. Note that the trigger function, like the
\lstinline=cylc suite-state= command, must have read-access to the upstream
suite's public run database. 

When each trigger succeeds, each item in the function return dictionary (above)
is qualified with the unique trigger label and passed to the environment of
dependent task jobs (in this case, to the members of the \lstinline=FAM=
family). To see this, take a look at the job script for one the downstream tasks:
...
(So the triggered task has to know the name of the external trigger that it
depends on - although the name could also be passed to the task job enviroment
from the suite configuration).

\subsection{Custom Trigger Functions}

{\em Note that trigger functions cannot store data Pythonically between
invocations because each call is executed independently in the process pool.
If necessary the filesystem can be used for this purpose.}
...
%Note auto debug arg.
(\lstinline=<cylc-dir>/lib/cylc/xtriggers/suite_state.py=) 

{\bf TODO}:
\begin{myitemize}
  \item explain the various metadata available to function args as string
    templates
...
\begin{myitemize}
  \item external trigger labels cannot be used in conditional (OR) expressions
  ...
  \item aside from the predefined zero-offset \lstinline=wall_clock= trigger, all
    unique external trigger function calls must be defined {\em with all of
    their arguments} under the \lstinline=[scheduling][xtriggers]= section.
    It would be more convenient to allow some arguments to be supplied in the
    graph, e.g.\ to define a \lstinline=wall_clock= call once and give
    different offsets as needed in the graph
    \begin{myitemize}
      \item the qualification of function result item names the label string,
        as passed to task environments, may need to change if this is
        implemented
    \end{myitemize}
  \item if a trigger function depends on files or directories (for example)
    ...
    unsatisified until everything that's required does exist.
\end{myitemize}
...
this, and in any case filesystem events are not very useful in an HPC cluster
environment where events can only be detected at the specific node on which
they were generated.
...
{\em NOTE: This external triggering mechanism is now deprecated, please use
    external trigger functions instead (see~\ref{External Trigger Functions}).}

In this method, external triggers are defined as task runtime properties that
must be satisfied by passing an associated message to the
\lstinline=cylc ext-trigger= command, which must be given the suite name, the
event message, and some ID that distinguishes one instance of the event from
another (the name of the target task and its current cycle point are not
required). The event ID is just an arbitrary string to Cylc, but it can be used
to identify something associated with the event to the suite - such as the
filename of a new externally-generated dataset. When the suite server program
...
\input{job-host-1.tex}
\input{job-host-2.tex}
