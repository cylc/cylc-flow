...
separately in Section~\ref{External Event Triggers}.
...
{\em NOTE: this older clock-triggering mechanism is now deprecated in favour of
built-in \lstinline=wall_clock= external triggers, which are more transparent
(visible in the graph) and efficient (triggeris are shared among dependent
tasks).}
...
change somewhat in future releases - see {\em Current Limitations} below
...
dependent tasks) than the older deprecated clock triggers described
...
tasks off of remote task statuses or messages in other suites.
...
\begin{myitemize}
  ...
  \item they must take a boolean \lstinline=debug= argument, because this will
    be automatically supplied by the suite when running in debug mode
...
{\em WARNING: This mechanism is now deprecated, please use external trigger
functions instead, see~\ref{External Event Triggers}.}
