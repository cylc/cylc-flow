...
Cylc has the ability to automatically stop suites and restart them on a
different host. This is useful if a host needs to be taken off-line e.g.\ for
...
When a host is added to the \lstinline=condemned hosts= list in a site
configuration, any suites will automatically shutdown, select another host from
\lstinline=run hosts= and restart.
