...
above listing.  The job script sections are:
\begin{myenumerate}
    \item  cylc location, suite location, suite identity
    \item  task identity
    \item  set error trapping - send ``task failed message to the suite
        in case of error
    \item  access to cylc
    \item  send a ``task started'' message to the suite
    \item  configure access to cylc, for the task
    \item  global variables
    \item  local (task-specific) variables
    \item  (global pre-command scripting - not shown)
    \item  task invocation (command line or verbatim scripting - or a list thereof for retry in case of failure)
    \item  (global post-command scripting - not shown)
    \item  send a ``task succeeded'' message to the suite\footnote{This may 
        be omitted in favour of messaging in the task implemention, for 
        certain tasks - see Section~\ref{TaskImplementation}.
\end{myenumerate}
...
This results in a ``task failed'' message to the suite if any command in 
the job script fails. Task invocation usually involves calling an external
script, which should \lstinline=exit 1= if any error occurs, in order to
invoke the job script trap. See Section~\ref{TaskImplementation}, {\em
Task Implementation}, for more on this).
...
If you want the suite to log an explanatory message for the task
failure, send a task message before exiting:
