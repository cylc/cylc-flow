...
because no task command lines have been defined. If you do try to run
...
processing can continue.\footnote{Actually this isn't entirely true 
because downstream tasks can also trigger off individual family members
as if they weren't family members - however, the family members themselves
will not be able to start running until the family itself triggers.} As
an extreme example, imagine grouping all
...
cycles the task can run in and how it relates to other tasks, the
command line that initiates processing when the task is ready to run,
the execution environment in which the command
...
use of hardwired task names is required outside of the suite.rc file.
...
supplied by cylc (Section~\ref{TaskExecutionEnvironment}) rather than
hardwired into the task implementation, so that the suite is flexible
and portable (see {\em Suite and Task Identity Variables},
Section~\ref{TaskAndSuiteIdentityVariables}).


\subsubsection{Task Job Scripts: How Tasks Are Invoked By Cylc}
\label{JobScripts}

When a task is ready to run, cylc writes, and then executes via the
task's job submission method (see
...
The actual command line used to submit the job script is also written to
stdout by cylc. You can test this yourself by running a suite or using
the \lstinline=cylc submit= command, which submits a single task to 
...
above; it should be registered as {\em examples:CUG74} in your central
suite database.
...
DRY RUN: create the task job script and show how it would be executed.
 ...
 > JOB SUBMISSION METHOD: /tmp/cylc-A%2011052400-jfHtqy </dev/null 1>
...
executing the task command line or scripting, as can be seen in the
above listing.  The job script sections perform the following functions:
\begin{myenumerate}
    \item  identify the parent suite and the cylc installation that is
        running it
    \item  identify the task
    \item  set error trapping to inform the suite if the task fails
    \item  inform the suite that the task has started running
    \item  configure access to cylc, for the task
    \item  export global variables
    \item  export task-specific variables
    \item  (global pre-command scripting - not shown)
    \item  invoke task processing
    \item  (global post-command scripting - not shown)
    \item  inform the suite that the task has finished 
        successfully\footnote{This may omitted, for 
        certain tasks, in favour of messaging from the task
        implementation - see Section~\ref{TaskImplementation}}.
\end{myenumerate}

Note that the task identity variables are defined prior to the
...
The first sections of the job script define variables that identify the
...
This results in the suite being informed of task failure if any command 
in the job script fails. If task invocation involves calling an external
script (which it usually does) such scripts must abort with non-zero exit
status if any internal error occurs (See
Section~\ref{TaskImplementation}, {\em Task Implementation}, for more on this).
...
The global {\em pre-command scripting} and {\em post-command scripting}
suite.rc items (not shown in the example job script above, because they
were not defined for that task) allow you to write arbitrary scripting
in a multiline string in the suite.rc task sections, that will be
written verbatim immediately before, and after, the task command line 
{\em of every task}.
...
{\em Most existing processing jobs do not require any modification to
function as cylc tasks.}

If a task {\em has no internal outputs} (i.e. downstream processing only
triggers off the task {\em finishing}) and {\em its initiating script 
does not detach and exit before all processing is finished}, then the 
task implementation does not need to be ``cylc-aware'' because all the
necessary task messaging (wherein a task informs its parent suite of
starting, finishing successfully, or failing) is handled by the job
script generated by cylc for the task, as illustrated in
Section~\ref{JobScripts}.

Just call your existing command, script, or scripting, via the task {\em
command} string:
...
Note that simple tasks can be entirely scripted within the suite.rc file
because the task ``command'' can be a single command line or verbatim
scripting enclosed in a multiline string (or a list thereof, to retry
on task failure).

\subsection{Exit 1 On Error}

The only requirement for such unmodified task scripts is that they
abort on error with non-zero exit status, in order to trigger the
job script's error trap, which will in turn call 
\lstinline=cylc task failed=
to alert the suite of the problem.

You can avoid writing explicit error checks for every 
operation in a task script by using \lstinline=set -e=:

\begin{lstlisting}
#!/bin/bash
set -e  # abort on error
mkdir /illegal/dir  # this will abort with non-zero exit status
\end{lstlisting}

To log an explanatory message, send a message before exiting:
...
\subsection{Tasks Requiring Minor Modifications For Cylc}
\label{TaskModsForCylc}

{\em Tasks with initiating scripts that spawn jobs internally and then
detach and exit without seeing all task processing through to the
finish} must send their own ``cylc task succeeded'' message (and
``cylc task failed'' in case of error) instead of the default 
messaging at the end of the task job script generated by cylc. 

{\em Tasks with internal outputs allowing downstream processing to
trigger before they are finished} must call ``cylc task message'' 
at the appropriate time to report their internal outputs complete.

\subsubsection{Task Messaging}
\label{TaskMessaging}

Note the lack of any apparent sender identification or target addressing
in the cylc messaging commands. This information is automatically
deduced from the execution environment written by the suite to the job
script used to invoke the task. 
...
This section explains how tasks are executed once they are ready to
...
via the task command line and/or execution environment, in the
suite.rc file.
...
messaging manually. As explained in Section~\ref{TaskModsForCylc}, {\em 
