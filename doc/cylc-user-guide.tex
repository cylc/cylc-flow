...
forecasting suite. A task proxy object can run the real task that it
...
suite, they just know their own prerequisites and outputs. Intercycle
...
Figure~\ref{fig-task-pool}. Now, whenever any task proxy changes state
(as a result of an output completion message, for example) cylc gets the
entire pool to interact indiscriminately ({\em regardless of cycle
time}) in an attempt to match unsatisfied prerequisites with completed
outputs.\footnote{In fact this dependency negotiation goes through a
middleman or broker object, which reduces the interaction scaling from
$n^2$ to $n$, where $n$ is the number of task proxies in the suite.} 
...
reporting the task started, and then succeeded or failed according to
the exit status of the wrapped process (see {\em Task
...
This is in fact the {\em examples:CUGQS3} suite from the Quick Start Guide,
...
suite is flexible and portable (see {\em Suite and Task
Identity Variables}, Section~\ref{TaskAndSuiteIdentityVariables}).
