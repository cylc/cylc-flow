...
is configured not to use the cylc lockserver (see {\em Starting Your Cylc
Lockserver Daemon, Section~\ref{StartingYourCylcLockserverDaemon}).
...
the task implementation. Another way to add additional scripting each 
side of the main task script, and potentially to script very simple
tasks entirely within the suite.rc file, is to wrap multiple commands on
the task command line using the cylc task wrapper (see {\em Task
Wrapping}, Section~\ref{TaskWrapping}).
...
Specifically, any script, command, or program that sees through, from 
start to finish, all of the processes that it initiates, 
...
To avoid writing explicit error checks for every mundane operation 
in a task script, wrapped tasks can use \lstinline=set -e= to abort on
any error, and then rely on the cylc wrapper to report the failure to
cylc:
\begin{lstlisting}
#!/bin/bash
...
needed for tasks that cannot be wrapped, or those with
internal outputs that have to be reported before the task is finished.} 
...
to B's input directory and so on). This is not ideal because
...
input parameters, and their cross-task configuration can
