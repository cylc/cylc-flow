...
Cylc task definition is very simple: where a task appears in the suite
dependency graph determines its relationship to other tasks and its
valid cylce times, and its other properties are defined under 
\lstinline=[tasks] [[NAME]]= - principally execution environment and
the command line or scripting used to invoke task processing.
...
Task names can contain letters, digits, and underscores. They should not
be hard wired into task implementations (scripts, models, etc.) - task
and suite identity can be extracted portably from the task execution
environment supplied by the suite
(Section~\ref{TaskExecutionEnvironment}) - then to rename a task you can
just change its name in the suite.rc file.
...
task's job submission method (see Section~\ref{JobSubmission}), a 
{\em job script} that configures the execution environment and then
invokes the external processing defined for the task.
       
The actual command line used to submit the job script is also written to
stdout by cylc. You can test this using \lstinline=cylc submit=, which
...
following listing illustrates using the two task example suite just
above (registered as {\em examples:CUG74} in
the central suite database):
...
As can be seen in the job script listing above, the task execution
environment is explicitly defined prior to executing the task command
line or scripting.  
...
These variables are also used automatically by cylc task messaging
commands, to target the right task proxy in the right suite.

\paragraph{Task Access To Cylc}

The cylc environment script, in \lstinline=$CYLC_DIR=, is sourced so
that the executing task has access to cylc commands and utilities. 
The fact that this is done prior to exporting user-defined variables 
means that cylc utility commands can be used in variable assignment
expressions, as shown in the listing above.
...
suite.rc items (not shown in the example job script above) are multiline
strings containing scripting to be executed verbatim immediately before,
and after, the task command line or scripting, {\em for every task}.
...
in the job script fails. External scripts invoked for task processing
must abort with non-zero exit status if an error occurs (See
Section~\ref{TaskImplementation}, {\em Task Implementation}) in order
to trigger the task failure message.
...
Most scripts can function as cylc tasks without any modification: if a
task {\em has no internal outputs} (i.e.\ downstream processing only
triggers off it {\em finishing}) and {\em its initiating script 
...
its progress) is handled by the job script generated by cylc for the
task, as illustrated in Section~\ref{JobScripts}.

Just call your existing command, script, or inlined scripting, via the
task {\em command} string. For example:
...
Simple tasks can be entirely scripted within the suite.rc file
because the task {\em command} string can contain multiple lines
of scripting (or a list thereof, to retry the task, with variations, on
failure).
