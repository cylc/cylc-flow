...
Cylc task definition is very simple: a task's position in the suite
dependency graph determines its relationship to other tasks and its
valid cycle times, then any other properties are defined under 
\lstinline=[tasks] [[NAME]]= - principally execution environment and
the command scripting used to invoke task processing.
...
The cylc environment script, in \lstinline=$CYLC_DIR=, is sourced in
the job script so that the executing task has access to cylc commands
and utilities. The fact that this is done prior to exporting
user-defined variables means that cylc utility commands can be used in
variable assignment expressions, as shown in the listing above.
...
An existing script (etc.) can function as a cylc task without 
modification so long as the following conditions are met:
\begin{myitemize}
    ...
    \item the task's initiating script or control process does not
        ...
        otherwise the job script cannot automatically detect and report
        task success or failure.  % (as shown in Section~\ref{JobScripts}).
    \item all task scripting returns non-zero exit status on error to
        enable failure detection by the job script.
\end{myitemize}

If these conditions are met you can just invoke your existing processes
via task {\em command} scripting in the suite.rc file, supplying input
parameters as required via command lines and/or execution environment.
For example:
...
of scripting (or a list of such strings, to retry on failure).

\subsection{Exit Status On Error}

The requirement to abort with non-zero exit status in case of error
allows the job script to trap errors and send a 
\lstinline=cylc task failed= message to alert the suite.  You can avoid
writing explicit error checks for every operation with \lstinline=set -e=:
...
What makes this an ``output'', as opposed to some general informative
message, is that it matches an output registered for the task in the 
suite.rc file:
...
detach and exit without seeing the resulting processing through to the
finish must arrange for the spawned processing to send its own ``cylc
task succeeded'' or ``cylc task failed'' messages on completion -
because the cylc-generated job script that can otherwise provide
automatic messaging will also finish early, just after the initiating
process exits.

Reporting success or failure is just a matter of calling the right messaging
command:
...
if $SUCCESS; then
    ...
    cylc task failed "oops"
    exit 1
fi
\end{lstlisting}

If your task's spawned sub-job(s) execute in the same environment as its
initial scripting, this may be all you need to do. But if the initial
execution environment is not propagated through to the sub-job, you 
will need to arrange for this this to happen in order for the messaging
commands to target the right task proxy object and suite, and the
environment must be configured to give access to cylc commands. Other
user-specified variables may be needed too, depending on the task. 

One way to do this is to call a {\em wrapper} that modifies the native
task scripts on the fly to insert task finished messaging in the
appropriate places, and anything else required, before invoking the (now
modified) native process. 
...
environment before calling the task command scripting.  Cylc then 
