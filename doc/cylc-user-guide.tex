...
\subsection{Run The Automated Database Test} 
...
\subsection{Run The Automated Scheduler Test} 
...
\begin{myitemize}
    \item Figure~\ref{fig-command-help} - the cylc command line interface, top level.
    \item Figure~\ref{fig-db} - a private database in gcylc.
    \item Figure~\ref{fig-cdb} - a central suite database in gcylc.
    ...
    \item Figure~\ref{fig-gcylc-original} - suite control GUI, treeview interface.
...
\begin{figure}
    ...
    \end{center}
\caption{\small The cylc command line interface, top level}
...
Find the suites registered under the `admin:examples' group in the
...
Now right click on the examples group and choose `Import'; this will
...
\subsection{View The examples:CUG\_QS1 Suite Definition}
...
constituent include-files when you save and exit the editor}. 
...
(Section~\ref{SuiteDefinition}); but here's how to interpreset this
suite.rc file means: 
...
Most tasks should be capable of this (see {\em Suite Design Principles 
and Suggestions}, Section~\ref{SuiteDesignPrinciples}) but if necessary
you can force particular tasks to run sequentially without using an 
explicit trigger on their own previous instance, e.g.: 
\begin{lstlisting}
# SUITE.RC
[special tasks]
    sequential = GetData
...
\subsection{Plot The examples:CUG\_QS1 Dependency Graph}

Right-click on the examples:CUG\_QS1 suite in gcylc and choose Graph; or on
the command line,
\begin{lstlisting}
$ cylc graph examples:CUG_QS1 2011052300 2011042318 &
\end{lstlisting}
...
\begin{figure}
    ...
    \end{center}
\caption{\small The examples:CUG\_QS1 suite dependency graph.}
\label{fig-QuickStartA-graph18}
\end{figure}

\subsection{Run The examples:CUG\_QS1 Suite}
...
been defined in our example suite so the tasks all default to a  
{\em dummy} command line like this:
\begin{lstlisting}
# SUITE.RC
[tasks]
    [[GetData]]
        command = cylc wrap -m "echo DUMMY $TASK_ID; sleep $CYLC_SIMULATION_SLEEP"
\end{lstlisting}
where \lstinline=$TASK_ID= is \lstinline=NAME%YYYYMMDDHH=) and 
\lstinline=$CYLC_SIMULATION_SLEEP= defaults to 10 seconds - it writes
an identifying
...
existing commands, scripts, or programs by automatically
reports the task started, and succeeded or failed according to the 
exit status of the wrapped process (see {\em Task
Wrapping: Using Existing Scripts As Cylc Tasks}, 
Section~\ref{TaskWrapping}).

Now start a suite control GUI by right-clicking on the suite in gcylc
and choosing `Control (graph)'.  You can also open an instance of
original text treeview control GUI on the same suite, if you like.

Multiple GUIs running at the same time will automatically connect to
the same running suite (they won't try to run separate instances). 
Note also that if you start a suite running via a control GUI
and then shut down the GUI, the suite will keep running and you 
can reconnect to it by simply opening another control GUI instance.
{\em Read {\em Using The Graph-Based Suite Control GUI}, 
Section~\ref{UsingTheGraphBasedSuiteControlGUI}.}
...
startup'' - the suite will start up in the held state wherein tasks
are not be submitted even if they are ready to run. 
...
\begin{figure}
    ...
    \end{center}
    \caption{\small Suite examples:CUG\_QS1 started with an initial
    cycle time ending in 06 or 18 hours.}
...
\begin{figure}
    ...
    \end{center}
    \caption{\small Suite examples:CUG\_QS1 started with an initial
    cycle time ending in 00 or 12 hours.}
...
\begin{figure}
    ...
    \end{center}
    \caption{\small Suite examples:CUG\_QS1 running, showing multiple
    instances of the clock-triggered task GetData running at once.}
...
\begin{figure}
    ...
    \end{center}
    \caption{\small Suite examples:CUG\_QS1 running, but stalled, after 
    the clock-triggered GetData tasks have finished, because of Model's
    previous-cycle dependence (see main text).}
...
range of cycle times present in the suite.} because the clock triggered
tasks are then constrained by the wall clock, and the other tasks have
to wait on them.
...
\begin{figure}
    ...
    \end{center}
    \caption{\small Viewing current task state after right-clicking on a task in gcylc. The
...
the real forecast model that the Model task represents would presumably
...
Having watched the examples:CUG\_QS1 suite run for a while, choose Stop from
...
Now take a look at the examples:CUG\_QS2 suite, which is a minor modification 
of examples:CUG\_QS1. The suite.rc file has a new {\em cold start} task called
ColdModel,
...
\begin{figure}
    ...
    \end{center}
\caption{\small The examples:CUG\_QS2 dependency graph showing a model cold start task.}
...
to restart the rest of the suite.
...
If you run examples:CUG\_QS2 you'll see that no manually triggering is required
to get the suite started this time.


\subsection{examples:CUG\_QS3 - With Task Implementation}

The suite examples:CUG\_QS3 is the same as examples:CUG\_QS2 except that
...
output files and consume input files in way that requires them to 
...
be accessed again later if reconnect to the suite with a new control GUI
instance.
...
the examples:CUG1 suite, because the QuickStart suites are a little
too minimal to bother with in this context:
\begin{lstlisting}
$ cylc grep OUTPUT_DIR examples:CUG1

FILE: /home/oliverh/suites/examples/CUG1/suite.rc
...
FILE: /home/oliverh/suites/examples/Intro/bin/E.sh
   [7]: cylc checkvars -c OUTPUT_DIR
   [21]: touch $OUTPUT_DIR/sea-state.products

FILE: /home/oliverh/suites/examples/Intro/bin/D.sh
   [7]: cylc checkvars -c OUTPUT_DIR
...
examples:CUG73 (unfortunately the suite was registered under 
a different name when the screenshots were taken!). Note the warnings
...
\begin{figure}
    ...
    \end{center}
\caption{Example Suite CUG73 Validation.}
...
cylc lockserver, so this will not be difficult. In the meantime, copying
a suite manually is simply a matter of copying the suite definition
directory and registering the new copy in your private suite database.
...
\begin{myitemize}
    ...
    \item and for each task,
        \begin{myitemize}
            ...
            \item the command line to execute when the task is ready to run
        \end{myitemize}
...
This is in fact the examples:CUGQS3 suite from the Quick Start Guide,
...
\lstset{language=bash}
\begin{lstlisting}
$ cylc graph dep:example1 2011052200 2011052206
...
\subsubsection{The Cycle Times At Which Each Task Can Run}

The set of hours at which a particular task can run is defined by
...
\subsubsection{Partitioning The Suite Graph}
...
side has an offset. Summarizing:
\begin{myitemize}
...
\end{myitemize}
This make graphs look clean and concise, because the majority of tasks
typically only depend on others with the same cycle time.

Now, having explained that graphs are interpreted in a
pairwise manner, you can optionally chain pairs together intuitively to
``follow a path'' through the dependency graph. So this,
\begin{lstlisting}
# SUITE.RC
    graph = """
...
\paragraph{Complex Conditional Triggers (Both AND and OR)}
...
\paragraph{Parenthesized Conditional Triggers}

{\em Parenthesized conditional expressions are not allowed as yet.}
...
doesn't know the details of the previous cycle (otherwise you could
just restart from the previous state rather than warm start) so it still
has to solve the bootstrapping problem to get the suite started. It does
...
Cylc task definition is very simple: the dependency graph defines which
cycles the task can run in and how it relates to other tasks, then we 
specify the command line used to initiate processing when the task
is ready to run, the execution environment in which the command
line should be invoked, and a few lesser details such as remote hosting
and task ownership.
...
(etc.) with varying inputs supplied by their respective command lines and
execution environments.

\subsubsection{Task Names}
\label{TaskName}

A task's name is defined by its entry(s) in the suite.rc dependency
graph, then a matching subsection heading under the suite.rc [tasks]
section is used to define other parameters (the command line, etc.)
for the task. To rename a task, just change its name in the suite.rc
file.

Cylc's messaging commands, which report task progress back to the parent
suite, do need to know the name and suite of the calling task, but they
automatically extract this information from the task execution
environment (which is configured by the suite; below) so that no
explicit use of task names is required outside of the suite.rc file 
(it may sometimes be useful, but it isn't strictly required).
Similarly, if a task needs to make use of its own name, or that of its
parent suite, (to make I/O paths task and suite-specific, for instance)
these should be extracted portably from the execution environment
(below) rather than hardwired into the task implementation, so that the
suite is flexible and portable (see
...
the environment for the task and then calls its command line.
       
Cylc writes the command used to submit the job script to stdout. You can
test this yourself by running a suite or using the 
\lstinline@ cylc submit@ command, which submits a single task to 
run just as it would be submitted by the suite, and with the
\lstinline=--dry-run= option just generates the task script for 
inspection and says how it would be executed according to the chosen
job submission method:

The following illustrates this using the suite listed just
above; it should be registered as examples:CUG7 in your central suite
database.

\begin{lstlisting}
 cylc submit --dry-run examples:CUG7 A%2010080806
 DRY RUN: create the task job script and show how it would be executed.
 TASK JOB SCRIPT: /tmp/cylc-A%2010080812-b9w0V5
 JOB SUBMISSION METHOD: /tmp/cylc-A%2010080812-b9w0V5 </dev/null 1>
   /home/oliverh/CylcLogs/examples/CUG7/A%2010080812-raV_3z.out 2>
   /home/oliverh/CylcLogs/examples/CUG7/A%2010080812-raV_3z.err &
\end{lstlisting}

And here is the resulting job script:

\begin{lstlisting}
% cat /tmp/cylc-A%2010080812-b9w0V5
...
export CYLC_DIR="/home/oliverh/cylc/cylc-3"
export CYLC_MODE="submit"
export CYLC_SUITE_HOST="oliverh-33586DL.greta.niwa.co.nz"
export CYLC_SUITE_PORT="NONE"
export CYLC_SUITE_DIR="/home/oliverh/cylc/cylc-3/examples/CUG7"
export CYLC_SUITE="examples:CUG7"
export CYLC_SUITE_GROUP="examples"
export CYLC_SUITE_NAME="CUG7"
export CYLC_SUITE_OWNER="oliverh"
export CYLC_USE_LOCKSERVER="True"
export CYLC_SIMULATION_SLEEP="10"

# TASK IDENTITY:
export TASK_ID=A%2010080812
export TASK_NAME=A
export CYCLE_TIME=2010080812

# ACCESS TO CYLC:
. $CYLC_DIR/environment.sh

# GLOBAL VARIABLES:
export NEXT_CYCLE="$( cylc util cycletime -a 12 )"
export PREV_CYCLE="$( cylc util cycletime -p 12 )"

# LOCAL VARIABLES:
export INPUT="foo-${NEXT_CYCLE}.txt"
...
The task execution environment is explicitly defined in the job script prior to 
executing the task command line, as can be seen in the above listing.
...
An executing task will almost certainly need to make use of its
own cycle time:
\begin{lstlisting}
$CYCLE_TIME    
\end{lstlisting}
...
$CYCLE_SUITE_DIR
\end{lstlisting}
And it may need to use the task and suite identity variables (to 
...
\paragraph{Cylc Access Environment}

Next the cylc environment is configured according to the value of
\lstinline=$CYLC_DIR=. This is done prior to exporting user-defined
variables so that cylc utility commands can be used in the following
variable assignment expressions, as shown above.
...
and after, the task command line. This can be used to 
...
suite.rc file is to wrap multiple commands on the task command line using
...
\begin{lstlisting}
$ cylc wrap -m "date; sleep 30; date; echo Goodbye cruel world; /bin/false"
...
processes, you may need to write a custom wrapper that takes input
parameters from the environment and its command line, and then modifies
...
native process. The task command line would then just invoke the custom
wrapper.
...
{\em Note that you can easily run the cylc example suites host by simply
setting the global remote host suite.rc items (and ensuring that cylc
is installed on the remote host)}. For the example suites with real task
implementations that work with ``real'' files, you'll have to run {\em
all} tasks on the same remote host so that they can all access their 
required input and output files. To distribute a suite across several
hosts you must arrange (using additional tasks) to transfer files
between the hosts as required to satisfy the real I/O dependencies.
...
environment before calling the task command line.  Cylc then 
...
{\bf This section of the user guide not complete yet.}
For the moment, please refer to command line help (included here in
...
\begin{myitemize}
    \item cold starting, warm starting, raw starting, restarting, and stopping suites
        {\em (see \lstinline=cylc run help=)}
    \item running single tasks outside or inside suites
        {\em (see \lstinline=cylc insert help= and \lstinline=cylc submit help=)}
    \item intervening in suites, e.g.\ stopping, removing, and inserting tasks, \dots
        {\em (see \lstinline=cylc control help=)}
    \item getting information about running suites, and the tasks in them 
        {\em (see \lstinline=cylc info help= and \lstinline=cylc discovery help=)}
    \item where to look for output - suite log, suite stdout/stderr, task stdout/stderr
        {\em (this is documented in the Quick Start Guide)}
    \item understanding cylc suite evolution, particularly in catchup operation
        {\em (the Quick Start Guide will also help here, along 
        with the cylc example suites, and running your real suites in simulation mode)}

    \item how to recover from certain kinds of task failure (TBD)

    \item the cylc lockserver 
        {\em (lockserver startup is documented in the Quick Start Guide; see also 
        \lstinline=cylc lockserver help= and \lstinline=cylc lockclient help=})

    \item suite security - use of secure passphrases {\em (this is
        almost trivial to configure, see documentation of ``use secure passphrase'' 
        in the Suite.rc Reference, Section~\ref{SuiteRCReference})}
    \item automatic state dump backups, named pre-intervention state dumps
        {\em (mentioned in the Quick Start Guide; watch the suite log
        after intervening in a suite)}
    \item centralized alerting and timeouts
        {\em (see documentation of ``task event hooks'' in the Suite.rc
        Reference, Section~\ref{SuiteRCReference})}
\end{myitemize}

\subsection{Using The Graph-Based Suite Control GUI}
\label{UsingTheGraphBasedSuiteControlGUI}
...
\subsubsection{Can I Switch A Suite Between Simulation And Live Modes?}
...
via the command line and/or execution environment (both defined in the
suite.rc file.
...
that, in essence, defines the suite dependency graph and the command line
