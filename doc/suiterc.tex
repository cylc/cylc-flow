...
\begin{myitemize}
    \item {\em type:} string (a message containing
        \lstinline=<CYLC_TASK_CYCLE_POINT>= with an optional offset as shown
        below. Note that you cannot use the corresponding shell variable
        \lstinline=$CYLC_TASK_CYCLE_POINT= here. The string substitution
...
\item{ \em examples:}
 \begin{lstlisting}
 foo = "sea state products ready for <CYLC_TASK_CYCLE_POINT>"
 bar = "nwp restart files ready for <CYLC_TASK_CYCLE_POINT+6>"
 \end{lstlisting}
