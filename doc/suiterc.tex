...
Initial scripting appears at the top of the task job script after the 
task execution environment is configured but before configuring access
to cylc itself. The original intention was to allow remote tasks
to source login scripts before calling the first cylc command (i.e.\ the
task started message) to set \lstinline=$PYTHONPATH= if Pyro has been
...
Finally, {\em job script error trapping is configured after any initial
scripting completes} so errors in initial scripting will not be detected
and will not cause a task to fail (unless downstream consequences cause
failure indirectly).
