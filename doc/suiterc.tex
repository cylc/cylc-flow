...
(on the suite host) will be adapted for the remote host. 

\begin{myitemize}
\item {\em type:} string (a valid hostname on the network)
\item {\em default:} (none) - i.e.\ run on the suite host
\item {\em examples:}
    \begin{myitemize}
...
The username of the task host account.  If you use dynamic host
selection and have different usernames on the different selectable
hosts \lstinline=$HOME/.ssh/config= can be used to handle username
translation.

\begin{myitemize}
\item {\em type:} string (a valid username on the remote host)
\item {\em default:} (none) - i.e.\ use the suite account username
\end{myitemize}
