...
script. This may be useful for remote tasks in particular - e.g.\ if
there is a need to source the task owner's login scripts, because ssh
(by which means cylc submits remote tasks) does not do this. It
is written to the top of the task job script, prior to configuring
access to cylc itself, because it may be employed to define
variables needed by cylc - e.g.\ if Pyro has been installed locally on
the remote host rather than at system level.
