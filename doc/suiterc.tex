...
\begin{myitemize}
 ...
 \lstinline=CCYYMMDDThhmm=, 19951231T0630), or ``now'', or ``UTCnow''.
    \item {\em default:} (none)
\end{myitemize}

The string ``now'' converts to the current date-time on the suite host, to
minute resolution - but minutes (or hours, etc.) may be ignored depending
on your cycle point format (\ref{cycle-point-format}).  Use ``UTCnow'' if the
suite runs in UTC mode (\ref{UTC-mode}) but the suite host does not.  
...
Clock-trigger tasks (see~\ref{ClockTriggeredTasks}) wait on a wall clock time
...
\paragraph[external-triggered]{[scheduling] $\rightarrow$ [[special tasks]] $\rightarrow$ external-triggered}
...
\begin{lstlisting}
    external-triggered = get-satx("new sat-X data ready"), \
