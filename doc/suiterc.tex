...
Cylc suite.rc files are automatically validated against a spec,
\lstinline=$CYLC_DIR/conf/suiterc.spec=, that defines all legal
entries, values, options, and defaults. Some values, however, are
complex strings that require further parsing by cylc to determine
their correctness.
...
\begin{myitemize}
    ...
    \item {\bf [[Subsection Nesting]]} is indicated by the number of square brackets.
    %% This is reasonably obvious:
    %%\item Sections are closed by the next section heading, so top level items within a section must be defined
    %%    before any further subsections.
    \item {\bf Duplicate Items} are illegal, except in
        \lstinline=environment= and \lstinline=directives=
        sections.\footnote{This was
        originally intended to allow 
        individual tasks to override specific environment variables set
        in an include-file used in multiple tasks.  However, namespace 
        inheritance now provides a better way to do this in most cases.}
...
\begin{myitemize}
    \item {\bf [cylc] } - how cylc itself behaves when running this suite.
    \item {\bf [scheduling] } - configures when tasks are ready to run.
    \item {\bf [runtime] } - how, where, and what to execute when a
        task is ready to run.
    \item {\bf [visualization] } - suite graphing and the graph-based
        control GUI.
\end{myitemize}

\subsubsection{Namespaces - Inheritance Of Runtime Properties}
\label{NIORP}

The [runtime] section configures what to execute, and where and how,
when tasks are ready to run. The sections immediately below [runtime]
are, for want of a better term, {\em namespaces}\footnote{We use the
...
for individual tasks or for families of tasks.  A namespace represents a
family if other namespaces inherit from it (overriding properties as
required). This allows configuration of related tasks without
repetition. 

Any namespace that does not explicitly inherit from another
automatically inherits from the {\em root} namespace (below).
Namespaces thus form a tree-like hierarchy of nested task
families in which the leaves are the individual tasks of the suite. 

\paragraph{The Root Namespace - Runtime Defaults}

All namespaces inherit first from the root namespace, which provides
defaults for some configuration items. The defaults are sufficient to
to allow simple test suites to be defined by dependency graph alone
(command scripting, for example, defaults to printing a
simple message, sleeping for ten seconds, and then exiting). 
Suite default values can be changed by explicitly configuring root.

Default values are documented along with each suite.rc item below. See
also Section~\ref{SuiteDefaults}, which shows default values in context. 

\paragraph{Triggering, And Triggering Off, Families}

Family names can be used in the suite graph as shorthand for the
effective dependencies on member tasks. Member dependencies are
substituted in such a way that downstream tasks will not
trigger until all family members have either succeeded or failed.

\paragraph{An Example}

The following suite.rc file defines a default job
submission method and one environment variable; and 
a family called {\em obs} that contains the tasks {\em land} and {\em
ship}, each of which add to or override some of their inherited
properties.

\lstinputlisting{../examples/appendix/suite.rc}
...
Also, \lstinline=cylc jobscript= generates the job scripts used by cylc
to run tasks; and the namespace hierarchy itself can be printed with
\lstinline=cylc list --tree=:
...
\paragraph{Collapsible Task Families In Suite Graphing And GUIs}

Nested families from the namespace inheritance hierarchy 
can be expanded or collapsed in suite graphs 
([visualization] $\rightarrow$ collapsed families) and by menu options
in the graph-based suite control GUI (this will also be added to the
text treeview control GUI in a future release).
...
This section configures various aspects of cylc's behaviour.
...
\begin{myitemize}
\item {\em type:} string
\item {\em default:} \lstinline=echo SIMULATION MODE $TASK_ID; sleep 10; echo BYE=
\end{myitemize}
...
\item {\em root default:} \lstinline=echo DUMMY $TASK_ID; sleep 10; echo BYE=
\end{myitemize}
...
\begin{myitemize}
    \item {\em type:} string (a message containing \lstinline=$(CYCLE_TIME)=
    with an optional offset as shown below. Note the round parentheses
    on $(CYCLE_TIME) - this is not a shell variable, although without an
    offset it does correspond to \lstinline=$CYLC_TASK_CYCLE_TIME= in
