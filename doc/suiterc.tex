...
\subsubsection[force run mode]{ [cylc] $\rightarrow$ force run mode}

If this item is set, cylc will refuse to run the suite in anything but
the specified run mode. This can be used for demo suites taken out of
their normal context (which will likely have to be run in simulation
mode) or to prevent accidental submission of expensive real tasks during
suite development. 
\begin{myitemize}
    \item {\em type:} string
    \item {\em legal valuse:} live, simulation, dummy
...
\paragraph[clock rate]{[cylc] $\rightarrow$ [[accelerated clock]] $\rightarrow$ clock rate}

The speed at which the accelerated clock runs in real seconds per
simulated hour.
...
\paragraph[clock offset]{[cylc] $\rightarrow$ [[accelerated clock]] $\rightarrow$ clock offset}
...
error status if cylc fails, or if any task fails, or if the suite times
out, or if a shutdown event handler than compares the test run with a
reference run fails. To use this feature first generate a reference
suite log with task triggering information in it by running a suite with
the \lstinline=--reference-log= option and manually verify the
correctness of the reference run; then later, to do a reference test run
just run the suite (in dummy mode for the most comprehensive test
without running real tasks) with the \lstinline=--reference-test= option.

\paragraph[suite shutdown event handler]{[cylc] $\rightarrow$ [[reference test]] $\rightarrow$ suite shutdown event handler}

The shutdown event handler that will compare the test run with the reference run.

\begin{myitemize}
    \item {\em type:} event handler command name or path
...
if it has not finished, for live mode runs. 
Test runs cannot be done in live mode unless you define a value for this
item, because it is not possible to arrive at a sensible default for all
suites.
...
The timeout value in minutes after which the test run should be aborted
if it has not finished, for simulation mode runs. 
Test runs cannot be done in simulation mode unless you define a value for this
item, because it is not possible to arrive at a sensible default for
all suites.
...
The timeout value in minutes after which the test run should be aborted
if it has not finished, for dummy mode runs. 
Test runs cannot be done in dummy mode unless you define a value for this
item, because it is not possible to arrive at a sensible default for all
suites.
...
\paragraph[dummy mode command scripting]{[runtime] $\rightarrow$ [[\_\_NAME\_\_]] $\rightarrow$ dummy command scripting}
...
\begin{myitemize}
\item {\em type:} string
\item {\em root default:} \lstinline=echo "Dummy command scripting"; sleep 10=
\end{myitemize}

\paragraph[simulation mode run length]{[runtime] $\rightarrow$ [[\_\_NAME\_\_]] $\rightarrow$ simulation mode run length}

The time interval in seconds that a task proxy remains in the `running'
state in simulation mode.

\begin{myitemize}
\item {\em type:} float
\item {\em root default:} \lstinline=10=
\end{myitemize}

\paragraph[fail in simulation mode]{[runtime] $\rightarrow$ [[\_\_NAME\_\_]] $\rightarrow$ fail in simulation mode}

This determines whether or not tasks succeed or fail in simulation mode.
Note that to experiment with task failure in simulation mode you can 
alter this setting and then reload the suite definition at run time
before re-triggering an affected task. 

\begin{myitemize}
\item {\em type:} boolean
\item {\em root default:} \lstinline=False=
\end{myitemize}
