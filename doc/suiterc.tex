...
\subparagraph[initial scripting]{[runtime] $\rightarrow$ [[\_\_NAME\_\_]] $\rightarrow$ initial scripting}

Any scripting defined here is inserted at the very top of the task job
script, before configuring access to cylc etc. This may be useful for 
remote tasks in particular, if there is a need to source the task
owner's login scripts - because ssh (by which means cylc submits remote
tasks) does not do this. If defined, this scripting is written to the
top of the task job script because it may be define variables needed by
cylc itself - if Pyro has been installed locally instead of at system
level, for example.

\begin{myitemize}
\item {\em type:} string
\item {\em default:} (none)
\item {\em example:} \lstinline@scripting = ". $HOME/.profile"@
\end{myitemize}
