...
directly by cylc}. Any variables read by task event hook scripts must be
defined here, for instance, because event hook scripts are executed
directly by cylc, not by running tasks. And similarly the {command
lines} issued by cylc to invoke event hook scripts or to submit task 
job scripts could, in principle, make use of environment variables
defined here. 
{\em Note that cylc local variables are not available to executing tasks
unless you happen to choose a local direct job submission method -
always use task runtime environments to pass variables into the task
...
shell in which cylc is running can be used.

\begin{myitemize}
    \item {\em type:} string
    \item {\em legal values:} depends on the shell in which you run
        cylc. White space around the `$=$' is allowed. 
    \item {\em default:} (none)
    \item {\em examples:} 
        \begin{myitemize}
            \item \lstinline@FOO = $HOME/foo@
            \item \lstinline@BAR = ${USER%_oper}@
            \item \lstinline@BAZ= "${USER#*_oper}" # <-- QUOTED to escape the suite.rc comment character@
        \end{myitemize}
\end{myitemize}
...
The task owner (i.e.\ the username under which to access the remote host
via passwordless ssh). May be defined by local environment variables 
(i.e.\ [cylc] $\rightarrow$ [[environment]]). 
...
\item {\em type:} string
\item {\em default:} (none)
\item {\em legal values:} depends on the task job submission shell
    (Section~\ref{JobSubShell}). White space around the `$=$' is allowed. 
