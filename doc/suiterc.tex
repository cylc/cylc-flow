...
A list of environment variables that if given, will be used to filter
the environment inherited by a task: only variables in the list will be
passed to the task job script. {\em This is not intended as an alternative 
to a well-designed inheritance hierarchy in which each task gets only what 
it needs.}  It can, however, improve poorly designed suites with tasks
that inherit a lot of environment they don't need, by making it clear
which tasks use which variables.
...
namespaces only have an effect if they are not overridden in descendants.
