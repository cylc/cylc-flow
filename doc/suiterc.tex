...
\subsubsection[{[[}shell command execution{]]}]{[cylc] $\rightarrow$ [[shell command execution]]}
\label{ProcessPool}

Cylc job submission, event handler, and job poll and kill commands are
executed by a pool of threads or processes.

\paragraph[pool type]{[cylc] $\rightarrow$ [[shell command execution]] $\rightarrow$ pool type}

\begin{myitemize}
    \item {\em type:} string
    \item {\em legal values:} `process' or `thread'
    \item {\em default:} `process'
\end{myitemize}

\paragraph[process pool size]{[cylc] $\rightarrow$ [[shell command execution]] $\rightarrow$ process pool size}
Number of process pool worker processes, if a process pool is chosen.
\begin{myitemize}
    \item {\em type:} integer
    \item {\em default:} None (number of processor cores on the suite host)
\end{myitemize}

\paragraph[thread pool size]{[cylc] $\rightarrow$ [[shell command execution]] $\rightarrow$ thread pool size}
Number of thread pool workers, if a thread pool is chosen.
\begin{myitemize}
    \item {\em type:} integer
    \item {\em default:} 4
\end{myitemize}
...
\begin{myitemize}
    \item These variables are not passed to tasks - use task runtime
        ...
        available to event handlers - which are executed by cylc, not by
...
\item {\em examples}, for the bash shell:
   \begin{myitemize}
       ...
       \item \lstinline@NEXT_CYCLE = $( cylc cycletime --offset=PT6H )@
       \item \lstinline@PREV_CYCLE = `cylc cycletime --offset=-PT6H`@
