...
\subparagraph[method]{[cylc] $\rightarrow$ [[simulation mode]] $\rightarrow$ [[[event hooks]]] $\rightarrow$ enable } 

Currently event hooks are ignored by default in simulation mode. If you 
enable them, be aware that timeouts are relative to the accelerated
simulation mode clock (which normally runs very quickly) and may need to
be redefined accordingly.

\begin{myitemize}
    \item {\em type:} boolean
    \item {\em default:} Fals=
\end{myitemize}
