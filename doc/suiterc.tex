...
\begin{myitemize}
    ...
    \item {\bf submit-failed}  - the job submit command failed, or the
                                   submitted job was killed before it started executing
    \item {\bf submit-retry}   - job submit failed, but cylc will resubmit it
                                   after a configured delay
    \item {\bf submit-timeout} - the submitted job timed out without commencing execution 
    ...
    \item {\bf retry}          - the task failed, but cylc will resubmit it
                                  after a configured delay
    \item {\bf timeout}        - the task timed out after execution commenced
...
\subparagraph[submit-timeout]{[runtime] $\rightarrow$ [[\_\_NAME\_\_]] $\rightarrow$ [[[event hooks]]] $\rightarrow$ submit-timeout}
...
{\em submit-timeout} as the EVENT argument:
\begin{myitemize}
    \item {\em type:} float (minutes)
    \item {\em default:} (none)
\end{myitemize}

\subparagraph[timeout]{[runtime] $\rightarrow$ [[\_\_NAME\_\_]] $\rightarrow$ [[[event hooks]]] $\rightarrow$ timeout}
