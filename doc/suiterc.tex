...
\subsubsection[{[[}max simultaneous job submissions{]]} ]{[cylc] $\rightarrow$ [[max simultaneous job submissions]]}
...
Initial scripting goes at the top of the task job script, before the 
task execution environment is configured, and before configuring access
to cylc. The original intention for this item was to allow remote tasks
to source login scripts before calling the first cylc command (i.e. the task
started message), to set \lstinline=$PYTHONPATH= if Pyro has been
installed locally rather than at system level for example. Note however
that the remote task invocation mechanism now automatically sources both 
\lstinline=/etc/profile= and \lstinline=$HOME/.profile=, if they exist. 
Note also that pre-command scripting, command scripting, post-command
scripting should be used for most other purposes, because these can
access variables exported to the task execution environment.
