...
\subparagraph[retrieve job log]{[runtime] $\rightarrow$ [[\_\_NAME\_\_]] $\rightarrow$ [[[remote]]] $\rightarrow$ retrieve job log}
...
automatically, you can set this setting to \lstinline=True=. The suite will
...
\subparagraph[retrieve job log max size]{[runtime] $\rightarrow$ [[\_\_NAME\_\_]] $\rightarrow$ [[[remote]]] $\rightarrow$ retrieve job log max size}

If the disk space of the suite host is limited, you may want to restrict the
maximum sizes of the job log files to retrieve by setting a maximum size. The
value can be anything that is acceptable by the \lstinline@--max-size=SIZE@
option of the \lstinline=rsync= command.
...
\subparagraph[retrieve job log delays]{[runtime] $\rightarrow$ [[\_\_NAME\_\_]] $\rightarrow$ [[[remote]]] $\rightarrow$ retrieve job log delays}
...
very little resource to run and should return instantaneously.

Each task event handler can be specified as a list of shell scripts or a shell
script templates.

A shell script template may have any or all of these patterns which will be
...
\begin{myitemize}
    ...
    \item %(submit_num)s: submit number
    \item %(id)s: task ID (i.e. %(name)s.%(point)s)
    \item %(message)s: event message, if any
\end{myitemize}

Otherwise, the shell script will be appended with the following arguments:
\begin{lstlisting}
...
\subparagraph[handler]{[runtime] $\rightarrow$ [[\_\_NAME\_\_]] $\rightarrow$ [[[events]]] $\rightarrow$ handler}

Specify a set of script or script templates as task event handlers.

Item details:
\begin{myitemize}
    \item {\em type:} Comma-separated list of strings (event handler scripts or script templates).
    \item {\em default:} []
...
Specify the events when the general task event handlers should be invoked.
...
\subparagraph[handler delays]{[runtime] $\rightarrow$ [[\_\_NAME\_\_]] $\rightarrow$ [[[events]]] $\rightarrow$ handler delays}

Specify any delays in case an event handler does not return 0 (zero).
...
Specify the events when notification emails should be sent.
...
\subparagraph[mail delays]{[runtime] $\rightarrow$ [[\_\_NAME\_\_]] $\rightarrow$ [[[events]]] $\rightarrow$ mail delays}

Specify any delays in case the mail notification command fails.
...
anything acceptable by the \lstinline=mail= command.
