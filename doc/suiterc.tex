...
\ref{cycle-point-timezone}. To just alter the number of expanded year digits
(for years below 0 or above 9999), see
\ref{cycle-point-num-expanded-year-digits}.

Cylc usually uses a $CCYYMMDDThhmmZ$ ($Z$ in the special case of UTC) or $CCYYMMDDThhmm±hhmm$ format for writing down date/time cycle points, which
follows one of the basic formats outlined in the ISO 8601 standard. For
example, a cycle point on the 3st of February 2001 at 4:50 in the morning,
UTC (+0000 timezone), would be written $20010203T0450Z$. Similarly, for the
the 3st of February 2001 at 4:50 in the morning, +1300 timezone, Cylc would
write $20010203T0450+1300$.

You may use the isodatetime library's syntax to write dates and
times in ISO 8601 formats - $CC$ for century, $YY$ for decade and decadal
year, $±X$ for expanded year digits, thereafter following the ISO 8601
standard example notation. For example, to write date/times as week dates
with fractional hours, set cycle point format to $CCYYWwwDThh,iiZ$ e.g.
$1987W041T08,5Z$ for 08:30 UTC on Monday on the fourth ISO week of 1987.

You can also use a subset of the strptime/strftime POSIX standard -
supported tokens are $%F$, $%H$, $%M$, $%S$, $%Y$, $%d$, $%j$, $%m$, $%s$,
$%z$.

To use the old Cylc cycle time format (e.g. $2014020106$ for 06:00 on the
1st of February 2014), set cycle point format to $%Y%m%d%H$.

Please note that using characters like "/" will break task output files ("/"
is a reserved character in POSIX file and directory naming). Using ":" is
likely to interfere with usage of commands like "rsync" when applied to task
output files and is not recommended.
...
submitted, the {\em submission timeout} event handler(s) will be called.
\begin{myitemize}
    \item {\em type:} float (minutes)
    \item {\em default:} (none)
\end{myitemize}
...
started running, the {\em execution timeout} event handler(s) will be
called.
\begin{myitemize}
    \item {\em type:} float (minutes)
    \item {\em default:} (none)
