...
\subparagraph[scripting]{[runtime] $\rightarrow$ [[\_\_NAME\_\_]] $\rightarrow$ [[[remote]]] $\rightarrow$ scripting}

A scripting section for remote
tasks. This intended use for this environment configuration for remote
jobs, typically sourcing the task owner's login scripts - because ssh
(by which means cylc submits remote tasks) does not do this.
If defined, it is written to the top of the task job script because
it may be define variables needed by cylc itself - if Pyro has been
installed locally by a user instead of at system level, for example.
