...
This section determines when tasks are ready to run.
...
catch up sufficiently. Failed tasks (which are not automatically removed
from a suite) do not activate the runahead limit, but downstream tasks
that can't run because of them will. In real time operation the runahead
limit is of little consequence because the suite will be constrained by
its clock-triggered tasks (the limit must be long enough to cover the
range of tasks present in the suite, however; a task that only runs once
per day, for instance, needs to spawn 24 hours ahead). The runahead
limit is intended to stop quick-running unconstrained tasks in
historical trials (i.e.\ sustained catch up operation) from running off
into the future and potentially swamping the system. 
...
A namespace represents a family if several other namespaces inherit from
it; it represents an individual task if it is a leaf on the inheritance
tree (i.e.\ no other namespaces inherit from it).
...
Specify another namepace here that this one should inherit all of its
runtime configuration from; specific items can then be overridden as
required. Note that many of the available items are left undefined even
