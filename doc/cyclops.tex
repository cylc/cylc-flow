...
\begin{abstract}
    ...
    %forecast cycles.  
    The metascheduler presented in this paper, {\em cyclops}, is able to
    ...
    correct execution order emerges naturally at run time. {\em cyclops}
...
input data. These dependencies exist between different tasks in the same
forecast cycle (e.g.\ a sea state forecast might depend on wind
fields generated by a cotemporal weather forecast), between the same
task in different cycles (e.g.\ a forecast model generally
writes out a ``background model state'' that used in initializing its
next forecast), and potentially between different tasks in different
cycles (e.g.\ the EcoConnect catchment models run on an hourly cycle as
they assimilate real time streamflow observations, but they depend on
...
well finished or the system would suffer increasing delays with each
...
is difficult to be sure of this as they are invariably unpublished
highly system-specific in-house designs that have not been published.
...
An obvious control system design that is capable of achieving the job
schedule of Figure \ref{fig-time-one} is a Finite State Machine that
enforces a predetermined non-linear order of events using hard coded
scheduling logic: {\em if task A has just finished, then begin executing
tasks B and C}, and so on. As far as the author is aware existing
control systems are of this type, or an even simpler sub-optimal design
in which the user must dictate the task execution order and guarantee
that no dependencies will be violated in the process, perhaps with some
crude means of specifying tasks that can run in parallel.\footnote{By ``even
simpler'' I refer only to the underlying scheduling algorithm. Specific
implementations of these designs can often be horrendously complicated.} 
Unfortunately the hard coded scheduling logic of the Finite State design
is highly system-specific and it rapidly becomes convoluted as system
complexity increases, which inevitably leads to inflexibility and
fragility. 

\subsection{Optimal Metascheduling}

Catching up from operational delays, and in historical case studies, the
external driving data for the next cycle(s) is available in advance.  It
should then be possible to get greatly increased system throughput by
running tasks from multiple cycles at once.  But this can only be
...
cyclops, however, is specialized toward cycling forecast systems in that
...
The simplicity of the dynamic scheduling implementation in {\em cyclops}
...
\subsection{Interfacing Tasks To cyclops}
blah.
