...
\begin{myitemize}
    ...
    \item Figure~\ref{fig-db} - the gcylc GUI, local (user-specific) database view.
    \item Figure~\ref{fig-cdb} - the gcylc GUI, central (multi user) database view.
...
\begin{figure}
    ...
    \end{center}
\caption[gcylc: Local Suite Database]{\small
The gcylc GUI, local (user-specific) suite database view.}
\label{fig-db}
\end{figure} 
...
This chapter is a quick introduction to basic cylc functionality using
the ``simple dependency graph example'' suite from Chapter 6, which
...
Now use the `View' menu to switch to the central suite database, which 
contains suites accessible to all users. Suites are displayed in a
...
\begin{figure}
    ...
    \end{center}
\caption[gcylc: Central Suite Database]{\small
The central suite database, viewed in gcylc, showing some example suites.}
\label{fig-cdb-new}
\end{figure} 
...
Use the View menu to switch back to your private database, and check
...
A private suite database, viewed in gcylc, showing a first example suite
...
\begin{myenumerate}
    ...
    \item retrieve the suite description,
    \item list tasks in the suite,
    \item edit the suite defintion in your editor,
    \item plot the suite dependency graph,
    \item search the suite definition and bin scripts,
    \item validate the suite defintion,
...
suite.rc file ({\em the} suite definition) and a \lstinline=bin=
sub-directory for scripts and programs that implement, or are used by,
suite tasks.
...
Strictly speaking the bin directory is optional because every task
...
{\em In this chapter we describe the format and general structure
of a suite.rc file. The definitive reference for legal suite.rc entries,
values, options, and defaults, is Appendix~\ref{SuiteRCReference}; for
the ins and outs  of suite.rc dependency graph definition, see
Chapter~\ref{DependencyGraphs}; task implementation is described in 
Chapter~\ref{TaskImplementation}; and task job submission in 
Chapter~\ref{JobSubmission}.}
...
[dependencies]
    ...
    [[ 0,6,12,18 ]]  # four times daily
        graph  =  """
            ...
            GetData => Model => PostA
            ColdModel | Model(T-6) => Model  # cold start or restart
                  """
...
cross-entry consistency checking is done, and finally cylc attempts to
create all of the suite's task proxy objects according to information
parsed from the suite.rc file.
...
\subsection{Principles And Suggestions}

Matters pertaining to simplicity, flexibility,
efficiency, and portability of cylc suites.

\subsubsection{Use The Example Suites to Learn About Cylc}
...
\subsubsection{Remember: There Is No Suite Cycle Time}
...
\subsubsection{Make Fine-Grained Suites} 
\label{Granularity}
...
\subsubsection{Don't Overuse Task Families}
...
\subsubsection{Use Include-Files For Groups Of Related Tasks}
...
\subsubsection{Make Tasks Rerunnable}
...
\subsubsection{Limit Previous-Instance Dependence} 
...
\subsubsection{Put Cycle Time In All Output File Paths}
...
\subsubsection{How To Manage Input/Output File Dependencies}
...
\subsubsection{Use Generic Task Scripts}
...
\subsubsection{Make Suites Portable}
...
\subsubsection{Make Tasks As Self-Contained As Possible}
...
\subsubsection{Make Suites As Self-Contained As Possible}
...
\subsubsection{Orderly Product Generation?}
...
\paragraph{Use Of Artificial Dependencies}
...
\subsubsection{Clock-triggered Tasks Should Wait On External Data}
...
A task family is a named group of tasks that functions as a single task
in the suite dependency graph. The entire family triggers as a group,
and downstream tasks can trigger off the entire family finishing or
failing as a group. 
...
real dependencies amongst the family members on the right (plotted with
