...
In this section we work through some basic cylc functionality using the
``QuickStart'' example suites, which should have been registered in the
central suite database during cylc installation. 
...
and name, as shown in Figure~\ref{fig-cdb-new}.
...
Find the suite registered as `admin:examples:QuickStart1' in the central
...
Now right click on QuickStart1 and choose `Import' to copy the suite to
your private database. A dialog box will pop up allowing you to register
your copy under a different group and name (just leave these as they are)
and a destination directory for the suite definition (e.g.\
\lstinline=$HOME/suites/QuickStart1=). 

{\em Note that suite groups do not need to be created explicitly, they
are automatically created and deleted as required.} 

Now use the Database menu to switch back to your private database, and
confirm that you now have a copy of the example suite; it should look
like Figure~\ref{fig-pdb-new}. Note that you can also import the whole
...
prompt> cylc db print --central | grep QuickStart1
admin:examples:QuickStart1  "Dependency Graph Example (Ch.6)" \
    /home/admin/cylc/examples/QuickStart1

# Import the suite,
prompt> cylc db import admin:examples:QuickStart1 $HOME/suites/examples/QuickStart1
Locking database /home/oliverh/.cylc/LDB/db
REGISTERED examples:QuickStart1  "Dependency Graph Example (Ch.6)" \
    /home/oliverh/suites/examples/QuickStart1
Copying suite definition
Unlocking database /home/oliverh/.cylc/LDB/db
DONE

# Check the import was successful,
prompt> cylc db print | grep QuickStart1
examples:QuickStart1 "Dependency Graph Example (Ch.6)" \
    /home/oliverh/suites/examples/QuickStart1
\end{lstlisting}
...
\begin{lstlisting}
prompt> cylc edit examples:QuickStart1
\end{lstlisting}
