...
Cylc employs a highly structured file format for suite definition.
Suite.rc files parse directly into nested data structures inside cylc,
making it very easy to add and use new configuration items. They are
also automatically validated against a specification file that defines all
...
\begin{myitemize}
    ...
    \item {\bf Single Line Strings} can be quoted, but the quotes can be 
        omitted if the string contains no commas (these indicate list values).
    \item {\bf Multiline Strings} must be triple-quoted.
    \item {\bf List Values} must be comma separated (space may follow the comma)
    \item {\bf White Space} is ignored but you can indent for clarity.
    \item {\bf Section Nesting} is indicated by number of square
        brackets on the heading.
    ...
    \item {\em Items in the environment and directives sections can be
        redefined} with the latest definition taking precedence.
    \item {\bf Include-files:} the file path should be specified,
        portably, relative to the suite definition directory; include
        files can span section boundaries, and may be multiply-included
        and nested. 
\end{myitemize}

The following pseudo-listing
illustrates most aspects of the file format:
...
This is the suite.rc file for cylc's ``simple'' example suite; a few
...
Cylc suite.rc dependency graph notation is intended to provide an
intuitive textual representation of an actual graph, but is more concise
...
which sleep for a few seconds and emit an identifying messages,
...
\begin{myitemize}
    ...
    \item {\bf task output labels} - \lstinline=foo:fail=, \lstinline=foo:out1=
        (the default output is ``task finished'')
    \item {\bf cycle time offsets} - \lstinline=T-6=
\end{myitemize}

\subsubsection{Chained Triggers}

Dependencies may be chained together, but they don't have to be. 
{\em Task names can appear multiple times}, but  {\em triggers 
must be unique} (i.e.\ the connections between tasks must be unique).
This means you can split a graph up however you like, for clarity's sake:
\begin{lstlisting}
# B triggers when A is finished, and then C triggers when B is finished:
    graph = "A => B => C"
...
are a convenient shortcut to defining multiple triggers separately:
\begin{lstlisting}
# 1/ this:
    graph = "A & B => C"
# is equivalent to:
...
OR operators (\lstinline=|=) can only appear on the left,\footnote{An OR
operator on the right doesn't make much sense in the context of a
forecasting suite:
\begin{lstlisting}
    graph = "A => B | C"  #? should cylc run B or C when A finishes?
\end{lstlisting}
}
Triggers involving OR operators are true ``conditional triggers''.
\begin{lstlisting}
# C triggers when either A or B finishes:
    graph = "A | B => C"
\end{lstlisting}

% TO DO: CURRENT GRAPHING BUG FOR A | B & C EXPRESSIONS!
%Forecasting suites tend to have quite simple requirements for
%conditional triggers, but if you need it cylc can do it:
% The AND operator takes precedence over the OR operator,
%\begin{lstlisting}
%# C triggers when either A or (B and C) finish:
%    graph = "A | B & C => C"
%\end{lstlisting}
%
%, but if you want to use both AND and OR in the same expression,
%read the following very carefully.
%
%{\bf Parenthesized expressions are illegal.}  Internally cylc can handle arbitrary 
%conditional expressions on task prerequisites because it transforms them into 
%equivalent python code and evaluates the resulting expression, BUT the
%suite.rc graph parser cannot currently decode such expressions

In a list of OR'd tasks, all members of the group are used to create 
the conditional expression that determines run time behaviour, but 
only the rightmost member will be plotted, by default. To change this,
change the order or attach and asterisk to the task:
\begin{lstlisting}
...
The default behavior, as shown above, is to trigger off the upstream
task {\em finishing}.  However, you can also trigger off a task 
failing, or off ``internal outputs'' that are emitted before a task
finishes, 
...
{\em This is only required if you need to trigger downstream processing 
before a task finishes, off some output it generates in mid stream.}
...
By default tasks trigger off other tasks with the same cycle time. But
some tasks necessarily depend on others with earlier cycle times. Most
notably this applies to warm cycled forecast models which write out
``restart files'' required in initializing the next forecast (or the
next next, if a cycle has to be skipped, and so on according to the
number of restart files generated throughout the forecast). 
Here's how to express this kind of relationship:
\begin{lstlisting}
# SUITE.RC
[dependencies]
    [[0,6,12,18]]
...
the next forecast. However, restart files may also be generated for a
number of subsequent cycles with the expectation that they may be used
in an emergency (if one or more cycles has to be skipped because of
missing data, for example). 
These restart dependencies are therefore unusual in that they can
potentially be used by any one of a number of subsequent cycles (of the
same model) {\em but} we only want to use the most recent ones (i.e.\
those generated by the most recent previous cycle) unless they are not
available, in which case we may have to use the older ones. 

Here's a forecast model that generates restart files for the next four cycles,
\begin{lstlisting}
# SUITE.RC
[special tasks]
    sequential = A
[dependencies]
    [[0,6,12,18]]
        # Model A restart dependencies:
        graph = """
A(T-6) | A(T-12) | A(T-18) | A(T-24) => A
                """
\end{lstlisting}
...
worse, while A(T) is still running.  If A(T) fails though, it can be
force-spawned and then removed from the suite by the operator. Then 
if A(T+6) is inserted into the suite it will automatically have its
restart dependencies satisfied by the older finished task. 
...
early triggering problem will not arise. if the restart dependency 
relies on an explicit output trigger, instead of task finished,
successive instances of the forecast model can overlap (running in
parallel) if the opportunity arises.

Finally, if you put a warm cycled model in the list of `models with explicit
restart outputs', instead of the list of `sequential' tasks, you
can use explicit labeled restart outputs, which must contain the 
word `restart', then the task will not spawn until its last restart
output is completed. This will allow successives instances of the 
task to overlap, in spite of having multiple restart outputs, if the
opportunity arises (whether this is worth the effort is up to you).
...
[dependencies]
    [[0,6,12,18]]
        graph = """
A(T-6):res6 | A(T-12):res12 | A(T-18):res18 | A(T-24):res24 => A
                """
TBD: SHOW OUTPUTS (c.f. dev:usg)
\end{lstlisting
...
\paragraph{\bf Recovering From A Family Failure} 
...
member then completes successfully this time, so will the family,
...
\begin{myitemize}
    \item spin up processes via temporary tasks and on-the-fly dependencies.
