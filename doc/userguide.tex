...
easy to add new configuration items to cylc. The following
pseudo-listing illustrates the file format:
...
the suite) so that no explicit use of task names is needed outside of
the suite.rc file. Similarly, if a task wants to make use of its name,
or that of its parent suite, (to make I/O paths task and suite-specific,
for instance) the names should be extracted from the environment (below)
rather than hardwired, so that the suite is flexible and portable.
...
it executes, is specified in the suite.rc file.

\subsubsection{Generic Task Scripts}

If several tasks in a suite all perform a similar function, e.g.\ to
move files around, they may all be able to invoke the same command,
script, or program, with task-specific input parameters provided via the
environment and/or commandline. The cylc file transfer and file
housekeeping utility programs were designed with this in mind
(see \lstinline=cylc util help=).

\subsubsection{Global And Task-Specific Environment}
...
Cylc also configures the task execution environment to provide access 
(for the executing task) to cylc itself, to the suite bin directory
(through \lstinline=$PATH=), and to the suite definition directory.
Variables are also defined to identify the task, cycle time, suite,
suite host and port, and several other parameters (see the example task
job script below). 

Executing tasks will almost certainly need to run scripts in the suite
bin directory, and make use of their own cycle time,
\begin{myitemize}
    \item \lstinline=$CYCLE_TIME= - 2011051118
\end{myitemize}

They may need to know the suite definition direction of their parent
suite, to access files stored there,
\begin{myitemize}
...
\begin{myitemize}
    \item \lstinline=$CYLC_SUITE=  - foo:bar
...
As shown in the example {\em task job script} below, the order of 
environment definition is,
\begin{myitemize}
    \item 1. suite identity
    \item 2. access to cylc
    \item 3. task identity
    \item 4. global variables
    \item 5. local variables
\end{myitemize}
...
can be made task-specific by referencing to these parameters. 

\subsubsection{Delaying Evaluation Of Variables Till Run Time}

E.g. for a task, having \lstinline=$HOME= evaluate to the remote home
directory: {\em escape the dollar sign}. TBD: EXAMPLE.
...
configures the environment as specified for the task, and then calls the
commandline defined for it.
...
file. However, use of these should be restricted to simple,
fast, and reliable scripting, because it is executed outside of the task
itself (i.e.\ prior to the task started message, and after the task
finished message).
