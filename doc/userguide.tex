...
failing as a group.  Task families can have internal dependencies. 
Family members can also have their own triggers, independent of the family,
and downstream tasks can trigger off family members as if they weren't part 
of a family. 

Task families are implemented in cylc as a special abstract task type
that just enters the 'running' state when its prerequisites are
satisfied; members automatically trigger off their family starting;
and the family's final state (finished, if all members finish; or
failed, if one or more members fail) is not determined until all of its
members have either finished or failed. 
...
using a conditional trigger on the family:

\begin{lstlisting}
    graph = """familyX | familyX:fail => PostProc"""
\end{lstlisting}

Task families should really only be used as a (very) convenient
...
they otherwise artificially constrain cylc's ability to achieve maximum
