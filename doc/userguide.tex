...
treeview by suite owner, group, and name, as shown in
...
\subsection{Import An Example Suite To Your Private Database} 
...
database, use View $\rightarrow$ Filter to restrict the view (you
can leave
the dialog open and refilter as often as you like).

Now right click on the DepGraphCh6 suite and choose `Import' to copy the
suite to your private database. A dialog box will pop up allowing you 
to register your copy under a different group and name (just leave these
as `examples' and `DepGraphCh6') and a location to copy the
suite definition to (e.g.\ \lstinline=$HOME/suites/DepGraphCh6=).
{\em Note that suite groups do not need to be created explicitly, they are 
automatically created and deleted as required.} 

Use the View menu to switch back to your private database, and 
check that you now have a copy of the example suite; it should look like
...
{\em In real mode operation (as opposed to {\em dummy mode},
Section~\ref{DummyMode}) do not choose an initial cycle time in the
future or nothing will happen until that time.}\footnote{For any 
suite processing initiated by clock-triggered tasks, at least.}
