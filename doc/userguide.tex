...
Of the software listed above, Python and Pyro are essential; graphviz
and pygraphviz are required for dependency graphing and the graph-based
suite control interface, but you can run cylc without them.
...
This section of the userguide contains a few screenshots of cylc in use,
...
The gcylc GUI, Private (user-specific) suite database.}
\label{fig-db}
\end{figure} 
...
The gcylc GUI, central (all users) suite database view.}
\label{fig-cdb}
\end{figure} 
...
\begin{figure}
    \begin{center}
        \includegraphics[width=0.8\textwidth]{../images/screenshots/ecox-1.png}
    \end{center}
...
This section is a quick introduction to basic cylc functionality using
the ``simple dependency graph example'' suite from Section 6, which
should have been registered in the central suite database during cylc
installation as ``examples:DepGraphCh6''. 
...
Now use the `Database' menu to switch to the central suite database,
which contains suites accessible to all users. Suites are displayed in a
treeview by owner, group, and name, as shown in
Figure~\ref{fig-cdb-new}.
...
\begin{figure}
    ...
    \end{center}
\caption[gcylc: Central Suite Database]{\small
The central suite database showing some example suites.}
\label{fig-cdb-new}
\end{figure} 
...
\begin{myenumerate}
    ...
    \item unreregister the suite or group (if you own it),
    \item reregister the suite or group (if you own it).
\end{myenumerate}

However, you can only {\em run} suites that are registered in your
private database, which requires either registering them directly or
importing them from the central database.

\subsection{Import A Suite To Your Private Database} 

Find the suite registered as `admin:examples:DepGraphCh6' in the central
database (replace `admin' with the name of the user account under which
cylc was installed). If there are a lot of suites in the central
database, use View $\rightarrow$ Filter to restrict the view (you can
leave the dialog open and refilter as often as you like).

Now right click on DepGraphCh6 and choose `Import' to copy the suite to
your private database. A dialog box will pop up allowing you to register
your copy under a different group and name (just leave these as
`examples' and `DepGraphCh6') and a location to copy the suite
definition to (e.g.\ \lstinline=$HOME/suites/DepGraphCh6=).  {\em Note
that suite groups do not need to be created explicitly, they are
automatically created and deleted as required.} 

Use the Database menu to switch back to your private database, and check
that you now have a copy of the example suite; it should look like
Figure~\ref{fig-pdb-new}. Note that you can also import the whole
`examples' group at once if you like (right-click on the group instead
of the suite name).

\begin{figure}
    \begin{center}
        \includegraphics[width=0.8\textwidth]{../images/screenshots/pdb-new.png}
    \end{center}
\caption[gcylc: Private Suite Database]{\small
A private suite database showing a first example suite
imported from the central database.}
\label{fig-pdb-new}
\end{figure} 

\subsubsection{Commandline}

If you like, you can now delete your new suite and re-import it via the
cylc commandline. To delete a suite, right-click $\rightarrow$
Unregister and choose ``delete suite definition directory'' (ignore the
dire warnings - you still have the master copy in the central database). 
The following transcript shows how to reimport the suite using the cylc
commandline:

\lstset{ basicstyle=\color{basic}\scriptsize\ttfamily }
\begin{lstlisting}
# Check that the example suite is registered in the central database,
prompt> cylc db print --central | grep DepGraphCh6
admin:examples:DepGraphCh6  "Dependency Graph Example (Ch.6)" \
    /home/admin/cylc/examples/DepGraphCh6

# Import the suite,
prompt> cylc db import admin:examples:DepGraphCh6 $HOME/suites/examples/DepGraphCh6
Locking database /home/oliverh/.cylc/LDB/db
REGISTERED examples:DepGraphCh6  "Dependency Graph Example (Ch.6)" \
    /home/oliverh/suites/examples/DepGraphCh6
Copying suite definition
Unlocking database /home/oliverh/.cylc/LDB/db
DONE

# Check the import was successful,
prompt> cylc db print | grep DepGraphCh6
examples:DepGraphCh6 "Dependency Graph Example (Ch.6)" \
    /home/oliverh/suites/examples/DepGraphCh6
\end{lstlisting}
...
Here is the suite.rc file for the DepGraphCh6 example suite:
\begin{lstlisting}
# SUITE.RC
title = Dependency Graph Example (Ch.6)
# This is a complete suite definition - run it!
[dependencies]
    [[0,6,12,18]]
        graph = """
A => B & C   # B and C trigger off A
A(T-6) => A  # A triggers off previous instance
                """
    [[6,18]]
        graph = "C => X"  # X triggers off C just twice daily
[visualization]
    [[node attributes]]
        X = "color=red"
\end{lstlisting}

This is a complete, valid, runnable suite definition. Commandlines have
not been defined for each task so cylc defaults to running {\em dummy
tasks} that write an identifying message to stdout and sleep for a few
seconds before exiting. If defined explicitly in the suite.rc file
the dummy task commandlines would look like this:
\begin{lstlisting}
# SUITE.RC
[tasks]
    [[X]]
...
\lstinline=$CYLC_DUMMY_SLEEP= defaults to 10 seconds.

The task wrapper \lstinline=cylc wrap=, which allows cylc to run
unmodified existing commands, scripts, or programs (see {\em Task
Wrapping: Using Existing Scripts As Cylc Tasks},
Section~\ref{TaskWrapping}), automatically reports the task started, and
finished or failed, to the suite.

\subsection{Graphing A Suite}

To plot the dependency graph from the suite.rc file, use
the gcylc Graph right-click menu item, or the commandline,
\begin{lstlisting}
prompt> cylc graph examples:DepGraphCh6 2011042500 2011042518 &
\end{lstlisting}
This will pop up a zoomable, pannable, graph viewer application showing
Figure~\ref{fig-dep-graph-ch6}. If you edit the suite.rc file the graph
viewer will update whenever you save the file.

\begin{figure}
    \begin{center}
        \includegraphics[width=0.5\textwidth]{../images/screenshots/dep-example.png}
    \end{center}
\caption{Example Suite DepGraphCh6 Plotted.}
\label{fig-dep-graph-ch6}
\end{figure}
...
the examples:userguide suite, because the DepGraphCh6 suite is too 
minimal to be of much use a demonstration here:
\begin{lstlisting}
...
Section~\ref{SuiteValidation}.

\subsection{Running A Suite}

Right-click on examples:DepGraphCh6 and bring up a graph-based 
suite controller, choose Run from the control menu, 
enter an intial cycle time for a {\em cold start}, and hit the Run
button. The suite should start up with a set of tasks in the waiting
state, like either one of Figures~\ref{fig-graph-control-1} or
Figures~\ref{fig-graph-control-2}.

\begin{figure}
\begin{minipage}[t]{0.5\textwidth}
    \begin{center}
        \includegraphics[width=\textwidth]{../images/screenshots/graph-control-1.png}
    \end{center}
    \caption{\small foo}
    \label{fig-graph-control-1}
\end{minipage}
\hfill
\begin{minipage}[t]{0.5\textwidth}
    \begin{center}
        \includegraphics[width=\textwidth]{../images/screenshots/graph-control-2.png}
    \end{center}
    \caption{\small foo}
    \label{fig-graph-control-2}
\end{minipage}
\end{figure} 

When cylc starts up, every task is inserted into the suite {\em at the
initial cycle time, or at the next valid cycle time for the task}. This
explains the difference between the two figures. If the intial cycle 
time is at 00Z, the next valid cylc for task X is at 06Z, and the 
control GUI fills in the dependency graph between with blank tasks
(the off-white colour).  Now, the reason why nothing is happening
in the suite is that all the tasks are waiting for their prerequisites
to be satisfied. In particular, task A is waiting on its own previous
instance, which does not exist because this is the first cycle. 
Right-click on task A and choose `View Prequisites and
Outputs' to see this (Figure~ref{fig-view-prereq-eg}.

\begin{figure}
\begin{minipage}[t]{0.3\textwidth}
    \begin{center}
        \includegraphics[width=\textwidth]{../images/screenshots/view-prereq-eg.png}
    \end{center}
    \caption{\small foo}
    \label{fig-view-prereq-eg}
\end{minipage}
\hfill
\begin{minipage}[t]{0.6\textwidth}
    \begin{center}
        \includegraphics[width=\textwidth]{../images/screenshots/graph-con-3.png}
    \end{center}
    \caption{\small foo}
    \label{fig-graph-control-3}
\end{minipage}
\end{figure} 



Normally a suite like this would have a special {\em cold start task}
to satisfy task A's previous-cycle dependencies at startup, but for
demonstration purposes is is convenient to have a suite that requires
manual triggering to get it going, so that things don't happen too
quickly.

To run a suite from the commandline,
\begin{lstlisting}
prompt> cylc run examples:DepGraphCh6 2011042506
\end{lstlisting}

Note that {\em if} the suite contained any {\em clock-triggered} tasks
(any real suite would) you would not choose an initial cycle time in
the future, or nothing would happen until that time. As this suite
does not contain any clock-triggered tasks, however, it's tasks are 
not constained by the wall clock time at all.

Then start up a suite control interface to follow progress:
\begin{lstlisting}
prompt> gcylc examples:userguide &
# or:
prompt> gcylc &
# then right-click on the userguide suite and choose 'Control'
\end{lstlisting}

You can also start, stop, and control suites directly from the gcylc
suite control interface (see options available under the `Control' 
menu, and by right-clicking on tasks in the suite).
...
suite.rc file allows that (the suite GROUP and NAME is used in all 
suite I/O paths so that different suite instances will not interfere
with each other).

\subsection{Shutting A Suite Down}

To shut a suite down from the commandline, 
\begin{lstlisting}
prompt> cylc stop examples:userguide
\end{lstlisting}

This will cause the suite to shut down once any currently running tasks
have finished. There are various other ways of stopping a suite too; see
\lstinline=cylc stop help=.

\subsection{Restarting A Suite}

To restart a suite from its most recent previous state, on the
commandline,
\begin{lstlisting}
prompt> cylc restart examples:userguide
\end{lstlisting}

You can also restart a suite from other saved prior states; 
see \lstinline=cylc restart help=. 

\pagebreak

\section{Suite Definition Overview} 

This section provides an overview of what a cylc suite looks look:
what should be kept in the {\em suite definition directory}, and the 
the format and structure of the {\em suite.rc} suite definition file. 

For detailed information on what's legal in a suite.rc file see
the {\em Suite.rc Reference} (Appendix~\ref{SuiteRCReference});
...
\begin{myitemize}
    ...
    \item any include-files included in the suite.rc file (see Section~\ref{IncludeFiles})
        \begin{myitemize}
            \item use of include-files is optional
            \item these may be kept in the sub-directories
            \item paths are specified relative to the suite definition directory
        \end{myitemize}
    \item {\bf a bin directory} for scripts and programs that implement,
        or are used by, suite tasks
        \begin{myitemize}
            \item technically this is optional as tasks can call external
                commands, scripts, or programs 
            \item tasks get automatic access to their own suite bin directory
        \end{myitemize}
    \item any other suite-related files and directories
        \begin{myitemize}
            \item documentation, ancillary files, control files, \dots
            \item will be copied if you copy the suite
            \item can be accessed by tasks at run time - see
                {\em Task Execution Environment} (Section~\ref{TaskExecutionEnvironment})
            \item holding everything in one place makes revision control easy
        \end{myitemize}
\end{myitemize}
...
Entire suite.rc files parse directly into a nested data structure inside
cylc, which makes it very easy to add and use new configuration items,
and they are automatically validated against a specification file that
defines all legal entries, values, options, and defaults
...
The following pseudo-listing illustrates the general suite.rc file structure:
...
[dependencies]
    ...
    [[ 0,6,12,18 ]]  # four times daily
        graph  =  """
            Prep => GetData & ColdA
            GetData => ModelA => PostA
            ColdA | ModelA(T-6) => ModelA  # cold start or restart
                  """
...
Dependency graphs plotted from this (by gcylc or 
\lstinline=cylc graph=) are shown in Figures~\ref{fig-simple-graph-1} 
and~\ref{fig-simple-graph-2}.
How to define suite dependency graphs is described in
...
\subsubsection{Suite.rc Syntax Highlighting}
...
\clearpage
\pagebreak
\section{Suite Database Registration}
\label{SuiteRegistration}

\subsection{Private Suite Database}

Before you can run a suite (or in fact do anything with a suite using
cylc commands) you must register it in your private suite database. 
Registration simply associates a GROUP and NAME for a suite with its
actual suite definition directory. Cylc commands can then reference
particular suites ``by name'' (i.e.\ by the registered GROUP:NAME).
The database also provides a convenient one-stop location from which to
view, work on, and run your suites. 

\subsection{Public Suite Database}

You can {\em export} suites from your private database to the public suite
database, where they can be viewed, and {\em imported} for use, by others.

\subsection{Suite Database Commands}

Cylc's {\em database } or {\em db} command category contains a number
commands to work with suite registrations:
\begin{lstlisting}
prompt> cylc db help
...
COMMANDS:
  ...
  get ................. Print a suite definition directory path
...
\subsection{Task Finished Triggers}
...
\subsection{Task Failed Triggers}
...
\subsubsection{Which Task Is Plotted?}
...
\subsubsection{Complex Conditional Triggers}
...
\subsection{Intercycle Triggers}
...
\subsection{Satisfying Intercycle Dependencies At Startup}
...
\subsubsection{Cold Start Tasks}
...
\subsubsection{Warm Starting A Suite}
...
the finished state. In other words, the cold start tasks stand for the
...
finished (by default, tasks spawn as soon as they start running in
...
in the suite dependency graph. The family, instead of each individual
member, can trigger off upstream tasks; and downstream tasks can trigger
off the family, instead of off each member.

Cylc task families are implemented as special pseudo-tasks that do not
actually submit anything to run when they enters the `running' state.
A task proxy representing the family will be seen in the suite control
GUI when a suite. 
...
\begin{myitemize}
    ...
    \item a family's final state is not determined until all of its
        members have finished or failed:
        \begin{myitemize}
            \item finished, if all members finished
...
\subsubsection{Family Failure Recovery} 
...
\subsubsection{Use Families Sparingly}
...
\subsubsection{Suite And Task Identity Variables}
...
When a task is ready to run cylc writes a special {\em job script} to
...
Section~\ref{SuiteRCFile}) and override it, if necessary, for specific
tasks:
...
\subsection{Remember: There Is No {\em Suite-Wide} Cycle Time}

In cylc, {\em each task has its own private cycle time} and can run when
its prerequisites are satisfied regardless of what other tasks (with, 
potentially, different cycle times) are running in the suite at the time.
There is no global loop over of successive forecast cycles; instead  
the suite advances by means of each task spawning its own successor, at 
the next valid cycle time for the task (this information is gleaned from 
the suite.rc dependency graph).
