...
\pagebreak
\section{Suite Registration Databases}
...
\pagebreak
\section{Cylc Suite Definition} 
\label{SuiteDefinition}

A cylc suite is defined by a single {\em suite.rc file} that concisely
specifies the properties of the various {\em tasks} managed by
the suite, and the relationships between them.

This section of the user guide shows how a define a cylc suite \dots

A ``task'' is a somewhat abstract entity in that its role in the 
suite is distinct from its implementation by real command, script, or
program 

representing in the sense of a named entity that represents a specific
function in the suite, is definitely part of suite definition, per se.


task represents processing initiated by a (but note that the same command, script, or program, can lie
behind multiple tasks in the same suite, with differing task-specific
input parameters). 
...
\subsection{Defining Suite Dependency Graphs}
...
\subsection{Job Submission Methods: How Cylc Submits Tasks To Run}
\label{JobSubmission}
...
\subsubsection{Task Job Scripts}
