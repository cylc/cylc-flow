...
(but you can control and monitor cylc suites from the commandline). 
...
use \lstinline=cylc db print --central= on the commandline.
...
Each cylc task has a commandline that is called to invoke the real 
task processing when the task is ready to run. No task commandlines have
been defined in the QuickStart:foo suite so they all default to a  
{\em dummy} commandline like this:
\begin{lstlisting}
# SUITE.RC
[tasks]
    [[GetData]]
...
\begin{lstlisting}
prompt> cylc grep OUTPUT_DIR examples:intro
...
commandlines to replace the default ``dummy'' one.
...
\begin{figure}
    \begin{center}
        \includegraphics[height=0.35\textheight]{../images/screenshots/simple-18-cold.png}
    \end{center}
...
finishes successfully. Therefore, to make graphs as concise
...
A cylc {\em task definition} entails task name, 

For each task we need to define:
    - how it relates to other tasks in the suite, and what hours of the
    day are its valid cycle times (dependency graph)
    - its a name (dependency graph and/or task section)
    - a commandline, to invoke the processing it represents
        (task section, defaults to dummy task)
    - the execution environment, in which the commandline is 
        executed (global and/or task environment section)

Although ``tasks'' stand for real external processes, or groups of
processes, they are somewhat abstract in that they acquire a
...
\begin{lstlisting}
prompt> cylc submit --dry-run examples:intro A%2010080806 
...
refer to commandline help (included here in Section~\ref{CommandReference}) 
...
very clearly the true connections between tasks in a suite. It even 
...
every time the graph changes, so the layout is not stable as the suite
evolves. This may or may not be a solvable problem - it seems likely
that incremental changes in an evolving graph would eventually result in
some kind of highly non-optimal layout.

However, the following features of the GUI make it quite usable in spite
...
\begin{myitemize}
    \item The disconnect button can be used to temporarily prevent the
        ...
        in real time operation suites tend to evolve quite slowing in
        any case)
    \item Right-click on a task and choose the "Focus" option to restrict
        ...
        those to focus on their cycles).
    \item Task filtering is the ultimate quick route to focusing on just
        the tasks you want after taking a look at the global layout.
    \item In future releases we plan to keep the graph centered, after
        the layout changes, on the task most recently clicked on.
\end{myitemize}
...
dump file. {\em Cylc will not let you restart a dummy suite in live
mode, or a live mode suite in dummy mode} because an attempt at the 
former must certainly be a mistake, and an attempt at the latter,
while feasible, would corrupt your live suite state dump file and 
turn it over to simulation mode. Note, however, if you really want
to run the current state of a live suite forward in simulation mode,
...
\begin{myenumerate}
    ...
    \item later, restart the live suite from the pre-simulation model
        state dump or backup 
\end{myenumerate}

{\em Cylc also has the ability to create an instant dummy mode clone 
of the current state of any running or stopped live suite, but this 
``practice mode'' has been temporarily disabled because we haven't
had time to test that it is still working.}
