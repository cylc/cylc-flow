...
proxy object in the suite task pool.  Once submitted, a task must,
\begin{myitemize}
...
The real task scripts don't need to know the name by which cylc refers
to them, although they can extract it from their execution environment
if they wish (actually the cylc task messaging commands do need to know
the name of the calling task, in order to target the right task proxy
inside the parent suite, but they automatically extract this information
from the task execution environment - so no explicit use of tasks names 
is required outside of the suite.rc file).


\subsection{How Tasks Are Invoked}

When a task is ready to run cylc writes a temporary {\em job script}
that configures the execution environment for the task and then
executes the commandline specified for the task. Finally, cylc submits
the new job script to run by the chosen job submission method for the
task.  For remote tasks, the job script is copied to the remote host 
and executed there.

\subsection{Task Wrapping: Using Existing Scripts As Cylc Tasks}
\label{TaskWrapping}

{\em Most existing pre- and post-processing jobs, models, etc., can
be used as cylc tasks without modification.}
...
conscripted as a cylc task by simply executing it through the cylc task
wrapper:
\begin{lstlisting}
# SUITE.RC
[tasks]
...
of any specific outputs registered for the task in the suite.rc file
(usually there are none, if other tasks just trigger off it finishing). 
...
\subsubsection{Exceptions: Scripts That Require Modification}
\label{Unwrapping}

The only tasks that can't be wrapped are:

\begin{myitemize}

    \item those with initiating scripts that do not see subsequent
        processing through until the end - e.g.\ scripts that spawn
        background sub-processes or submit internal batch jobs and then
        detach and exit before those jobs are complete. These cannot be
        wrapped because the wrapper has no way of knowing when the task
        is actually finished.
    \item those with internal outputs that have to be reported complete
        {\em before the task is finished} so that others can trigger off
        them early (tasks with internal outputs can be wrapped, but the
        internal outputs will only be reported complete by the cylc
        wrapper when the task is finished).

\end{myitemize}

Scripts that can't be wrapped have to be modified slightly for cylc, 
by inserting a few calls to the cylc task messaging interface in 
appropriate places. For details, refer to the next section, {\em Task
Messaging}.

\subsection{Task Messaging}

As noted previously, {\em explicit use of task messaging is only 
required for the minority of tasks that cannot be wrapped, or those
with internal outputs that have to be reported before the 
task is finished.} 
...
Note the lack of any sender identification or targetting address for the
message. The task messaging commands automatically deduce the calling
...
The task messaging commands have commandline help, see 
\lstinline=cylc task help= and \lstinline=cylc task message help=, etc.;
and Appendix~\ref{AnnotatedTaskScript} illustrates the kinds of 
scripting required, including automatic error trapping, in more 
detail.
...
Otherwise, save yourself the trouble and just use the cylc task wrapper 
