...
This is a complete, valid, cylc suite definition that defines a suite
you can run: each task will execute the default cylc dummy task
commandline, which if defined explicitly in the suite.rc file would look
like this:
\begin{lstlisting}
# SUITE.RC
[tasks]
    [[X]] # define properties of task X
        command = cylc wrap -m "echo DUMMY $TASK_ID; sleep $CYLC_DUMMY_SLEEP"
\end{lstlisting}
In other words, each task will emit an identifying message (e.g.\
\lstinline=DUMMY X%2011052218=), sleep for a short period (10 seconds),
and then exit. The wrapper, \lstinline=cylc wrap=, automatically reports the
task started, and finished (or failed), to the suite.

\subsection{Graphing A Suite}

To plot the dependency graph defined in the suite.rc file, use
right-click $\rightarrow$ `Graph', or the commandline,
\begin{lstlisting}
prompt> cylc graph examples:DepGraphCh6 2011042506 2011042606 &
\end{lstlisting}
...
To use the suite search tool, which reports matches in the suite.rc file
...
commands in the ``suite preparation'' category.
...
\subsection{Starting A Suite}

To run a suite from the commandline,
\begin{lstlisting}
prompt> cylc run examples:userguide 2011042506
\end{lstlisting}

{\em do not choose an initial cycle time in the future or nothing will
happen until that time.}\footnote{For any suite processing initiated by
clock-triggered tasks, at least.} In {\em dummy mode} suites run on an
accelerated clock, and you can simulate catchup to real time,
(Section~\ref{DummyMode}), 
...
analysis suites parts of the the observation processing and data
...
cycle-specific by referencing the the task identity.} 
