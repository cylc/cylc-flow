...
Start up a suite control interface to follow progress.
\begin{lstlisting}
prompt> gcylc examples:userguide &
\end{lstlisting}

Alternatively you can also start, stop, and control suites directly 
from the gcylc GUI. Start the suite control interface as just described,
or by right-clicking on the suite in the gcylc local database viewer,
and choose an item from the `Control' menu.
...
Internally cylc can handle parenthesized conditional expressions on task
prerequisites because it transforms them directly into Python code and
evaluates the resulting expression, {\em but} the suite.rc graph parser cannot
handle general conditionals yet.  This restriction should not be much of
an impediment because forecasting suites tend to have very simple
conditional triggers, and you can rely on operator precedence (AND
before OR), and split large expressions into multiple small expressions
on separate lines.
...
Considering the kind of intercycle dependence shown just above (tasks A
...
cycle there will be no previous instance of A to satisfies B's
prerequisites. 
...
the cold start task masquerades as its counterpart's normal
previous-cycle upstream trigger.

A cold start task may invoke real processing (presumably to generate the
files that are normally generated by the cycling task that it stands in
for) or it may just be a dummy task that stands for some external spin
up process which has to be completed before the suite is started (in this
case the cold start task in cylc will just report itself successfully
completed, thereby satisfying the aforementioned dependencies).

We can express this kind of relationship with a conditional trigger:
\begin{lstlisting}
# SUITE.RC
[special tasks]
    coldstart = ColdFoo
...
to cold start just the task it is associated with, in case of problems
that prevent continued normal cycling.
...
\subsection{Intercycle Restart Dependencies (Models)}
...
cycles in case the next (or more) cycle has to be omitted due to
...
finished (tasks spawn as soon as they start running, by default, in
...
to the point where the new cold start input data are available). After
...
job script, illustrated above, and how they are defined in the suite.rc file.
...
The ordering of the job script section is,
\begin{myitemize}
...
{\em Note also that task identity, which includes the task's private cycle
