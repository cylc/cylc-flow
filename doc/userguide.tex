...
{\em \small Released Under the GNU GPLv3.0 Software License} \linebreak
...
%admin:examples:intro  "user guide example suite"  /home/admin/cylc/examples/Intro
%\end{lstlisting}
...
The following few screenshots provide a glimpse of what the cylc user
...
same time. In other words, each task may pass through the ``current cycle''
(etc.) at a different time as the suite evolves, particularly in delayed
(catchup) operation.
...
$ cylc db print --central | grep QuickStart
...
$ cylc db import admin:QuickStart: $HOME/suites/QuickStart
# TBD!

# Check the import was successful,
$ cylc db print | grep QuickStart
...
Note that all the tasks in this suite can run in parallel with their own
previous instances if the opportunity arises (i.e.\ if their prerequisites
...
the command line,
\begin{lstlisting}
$ cylc graph QuickStart:foo 2011052300 2011042318 &
\end{lstlisting}
...
been defined in the QuickStart:foo suite so they all default to a  
...
of QuickStart:foo. The suite.rc file has a new {\em cold start} task called
ColdModel,
...
the examples:intro suite, because the QuickStart suites are a little
too minimal to bother with in this context:
\begin{lstlisting}
$ cylc grep OUTPUT_DIR examples:intro

FILE: /home/oliverh/suites/examples/Intro/suite.rc
...
%The ``QuickStart:baz'' and ``intro'' examples have actual external task
...
\subsubsection{Can Suite's Be Shared Across The Network}

{\em The central suite database is not currently accessible on the
network, it is local to the cylc host} 
It briefly locks users out during operations that change the minimal
...
In the meantime, do it copying a suite manually is simply a matter of
copying the suite definition directory and registering the new copy 
in your private suite database.
...
This is in fact the examples:little suite, which you can copy, study, 
and run. Dependency graphs plotted from this (by gcylc or
\lstinline=cylc graph=) are shown in Figures~\ref{fig-simple-graph-1}
and~\ref{fig-simple-graph-2}.
...
To get a task to trigger off another that succeeds (i.e.\ when it
reports that it has {\em finished successfully}), use the default notation:
\begin{lstlisting}
# SUITE.RC
# B triggers when A SUCCEEDS:
    graph = A => B
\end{lstlisting}
...
rely on operator precedence - AND before OR - and split large expressions
into multiple lines). If there is call for more complicated triggers we
will certainly address the problem.
...
above; it should be registered as examples:UG7 in your central suite
database.

\begin{lstlisting}
 cylc submit --dry-run examples:UG7 A%2010080806
 ...
 JOB SUBMISSION METHOD: /tmp/cylc-A%2010080812-b9w0V5 </dev/null 1>
   /home/oliverh/CylcLogs/examples/UG7/A%2010080812-raV_3z.out 2>
   /home/oliverh/CylcLogs/examples/UG7/A%2010080812-raV_3z.err &
\end{lstlisting}
...
export CYLC_SUITE_DIR="/home/oliverh/cylc/cylc-3/examples/UG7"
export CYLC_SUITE="examples:UG7"
export CYLC_SUITE_GROUP="examples"
export CYLC_SUITE_NAME="UG7"
...
{\bf This section of the user guide is, unfortunately, yet to be completed.}

For the moment, for reasonably complete documentation of available functionality,
refer to command line help (included here in Section~\ref{CommandReference}) 
the suite.rc reference (Section~\ref{SuiteRCReference}), the gcylc help 
menus, and examples elsewhere in this User Guide. 
...
\begin{myitemize}
    ...
    \item where to look for output - suite log, suite stdout/stderr, task stdout/stderr
        {\em (this is documented in the Quick Start Guide, Section~\ref{QuickStartGuide})}
...
the structure of a suite very clearly as it evolves. It even 
works well for large suites, on the order of at least one or two
hundred tasks, because it zooms and pans easily and responsively.

On the downside, the graphviz engine does a new global optimization
every time the graph changes, so the layout may not remain stable. This
may or may not be a solvable problem - it seems likely that making
continuing incremental changes to an existing graph would
inevitably result in some kind of mess.
...
changes, on the most recently clicked task.
...
%
%\begin{lstlisting}
%cylc restart --practice examples:intro
%\end{lstlisting}
%
