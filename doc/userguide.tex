...
mind that the members of such a group may not all exist in the suite 
at the same point in real time. In other words, each task may pass
through the ``current cycle'' (etc.) at a different time as the suite
...
\subsection{Using Example Suites To Learn About Cylc}

Before we begin the specific example it's important to note, more
generally, that cylc has been designed from the ground up to make
testing and prototyping very easy.  Simply ``drawing'' (textually) a
dependency graph in a new suite.rc file creates a valid suite that runs
dummy tasks (which just emit an identifying message, sleep for a few
seconds, and then exit). You can then arrange for a particular task to
fail, for example, by supplying an appropriate task commandline to
replace the default ``dummy'' one.

Cylc's example suites run quickly and are completely portable: you 
can import a copy of one and run it without changing a thing; in fact
you can run multiple copies of them at once, under different names,
without any interference between them. Some example suites employ dummy
tasks or just run simple task commands (as just described above) but
some have actual external task implementations, in the form of scripts
in the suite bin directory, that behave like real tasks in
that they generate specific output files (using touch) that are
``required'' by other tasks in the suite, and if the suite does not
run them in the proper order they will abort will an error message 
(``input file not found'' or similar).

To test something quickly you can copy an example suite (or any other 
suite) in an instant, using the cylc commands or GUI, make a few changes
according to your needs, and then run it to see what happens.

You can also run real suites in dummy mode: each task is replaced by the
default dummy task (above) and the suite runs quickly on an accelerated
clock. As far as cylc is concerned this is identical to real operation,
so dummy mode can be used to test recovery strategies for certain kinds
