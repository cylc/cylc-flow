...
%Cylc {\em task definition} basically entails defining the following:
%\begin{myitemize}
%    \item the task name (below),
%    \item what cycles it can run in, and how it relates to other
%        tasks (the dependency graph, Section~\ref{DependencyGraphs}),
%    \item the command line that should be invoked to initiate 
%        processing when cylc determines that the task is ready to run,
%    \item the execution environment in which the task command
%        line should be executed, and,
%    \item how it should be submitted to run (Section~\ref{JobSubmissionMethod)})
%    \item and a few other lesser details, such as batch queue
%\end{myitemize}
...
(etc.) with varying inputs supplied via the task command line and
execution environment.

\subsection{Task Names}
\label{TaskNames}

A task name is defined by its entry(s) in the suite.rc dependency graph
and its subsection heading under the suite.rc [tasks] section.
To rename a task, just change its name in the suite.rc file.
...
explicit use of task names is needed outside of the suite.rc file.
...
these should be extracted from the execution environment (below) rather
than hardwired, so that the suite is flexible and portable (see
Section~\ref{TaskAndSuiteIdentityVariables}, {\em Task And Suite
Identity Variables}).

\subsection{Task Command Line and Environment}
\label{TaskExecutionEnvironment}

Every task has an associated command line to initiate the processing
that it represents. The task command line, and the environment in which
...
%        \end{myitemize}
%    \item scripting to call the command line defined for the task.
%\end{myitemize}
...
SUBMITTING TASK: /tmp/cylc-ColdB%2011101300-5gRx61 </dev/null \
    1> /home/oliverh/CylcLogs/examples/intro/ColdB%2011101300-N3pBYv.out \
    2> /home/oliverh/CylcLogs/examples/intro/ColdB%2011101300-N3pBYv.err &
\end{lstlisting}
...
\begin{lstlisting}
$ cylc submit --dry-run examples:intro A%2010080806 
DRY RUN: create the job script and show how it would be executed.
  ...
  * JOB SUBMISSION METHOD: /tmp/cylc-A%2010080806-VXQusn </dev/null \
    1> /home/oliverh/CylcLogs/examples/intro/A%2010080806-CWYM68.out \
    2> /home/oliverh/CylcLogs/examples/intro/A%2010080806-CWYM68.err &
\end{lstlisting}

And here is the resulting job script:

\begin{lstlisting}
$ cat /tmp/cylc-A%2010080806-VXQusn
#!/bin/bash
...
export CYLC_SUITE_DIR="/home/oliverh/suites/examples/intro"
export CYLC_SUITE="examples:intro"
export CYLC_SUITE_GROUP="examples"
export CYLC_SUITE_NAME="intro"
...
\label{TaskAccessToCylcCommands}

Executing tasks need access to the cylc task messaging commands, and
possibly to cylc utility commands, so task job scripts source the cylc
environment script.

%\paragraph{On Remote Hosts}
%
%If you specify a remote host for task, you must also specify the 
%locations of the remote cylc installation and the remote suite
%definition directory (which should be installed on the remote
%host so that tasks can access suite files and the suite bin 
...
%        remote host = fitzroy.niwa.co.nz
%        remote cylc directory = /path/to/remote/cylc/installation
%        remote suite directory = /path/to/remote/suite/definition
%\end{lstlisting}
%This results in the \lstinline=$CYLC_DIR= and
%\lstinline=$CYLC_SUITE_DIR= variable definitions being set to 
%their remote values in the job script.
...
%\paragraph{On Remote Hosts}
%
%See {\em Task Access To Cylc Commands} (Section~\ref{TaskAccessToCylcCommands}).
%
...
For the moment, for quite complete documentation of all available functionality,
refer to command line help (included here in Section~\ref{CommandReference}) 
and the suite.rc reference (Section~\ref{SuiteRCReference}). The gcylc help 
menus are also reasonably complete.
...
\begin{myitemize}
    ...
    \item where to look for output - suite log, suite stdout/stderr, task stdout/stderr
        {\em (this is documented in the Quick Start Guide, Section~\ref{QuickStart})}
    \item understanding cylc suite evolution, particular in catchup operation
        {\em (the Quick Start Guide, Section~\ref{QuickStart}, will help here, along 
    ...
    \item automatic state dump backups, named pre-intervention state dumps
        {\em (mentioned in the Quick Start Guide,
        Section~\ref{QuickStart}; watch the suite log after intervening
        in a suite)}
