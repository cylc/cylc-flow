...
\section{Suite.rc Dependency Graphs}
...
\subsection{Time Offset Notation}
...
\subsubsection{Which Task Will Be Plotted?}
...
The variable \lstinline=$CYCLE_TIME=, which tasks can read from the
execution environment configured for them by cylc, is {\em
task-specific}. There is no global loop over of successive forecast
cycles; the suite advances by means of each task spawning its own
successor at its next valid cycle time (which is known gleaned from 
