...
Dependency graphs define the relationships between tasks in a suite. In
cylc suite.rc files, dependency graphs are specified within freefrom
multiline strings that closely mirror the actual graph, but are more
concise because sections of the graph that repeat at different hours
of the day only have to be defined once.
...
# SUITE.RC
[dependencies]
    [[0,6,12,18]] # hours for graph section
        graph = """
            A => B & C   # B and C trigger off A
            C => D       # D triggers off C
            A(T-6) => A  # A has restart files
                """
    [[6,18]]
        graph = "C => X" # X triggers off C
\end{lstlisting}
\end{minipage}
\hfill
\begin{minipage}[t]{0.5\textwidth}
...
(this listing, just as it is, is actually a valid suite that you can
run; with no task properties specified cylc will just run dummy tasks
with the relationships defined in the graph).
 
Here are the suite.rc graph syntax rules:
\begin{myitemize}
    \item blank lines, and arbitrary white space, are allowed
    \item comments are allowed inside the graph string (\# character)
    \item dependencies (triggers) are indicated by arrows (\lstinline@=>@)
    \item conditional expressions:
        \begin{myitemize}
            \item operators \lstinline=&= (AND) and \lstinline=|= (OR)
                are allowed on the left side of arrows.
            \item only operator \lstinline=&= (AND) is allowed on the
                right side of arrows.
            \item each line within the graph string has an AND relationship
...
# SUITE.RC
    ...
    graph = """A => B
               B => C"""
\end{lstlisting}
            \item {\em parenthesized expressions are not allowed} (but 
                this should not be a problem; see below) 
        \end{myitemize}
    \item in a list of OR'd tasks, the left-most task will be plotted 
        by default, but you can change this by appending an asterix 
        to another task in the list.

    \item triggering,
        \begin{myitemize}
            \item the default is to trigger off an upstream task finishing
            \item to trigger of an upstream task failing,
\begin{lstlisting}
# SUITE.RC
    graph = "A:fail => B"  # B triggers if A fails
\end{lstlisting}
            \item to trigger of a specific internal output:
\begin{lstlisting}
# SUITE.RC
    graph = "A:out1 => B"  # B triggers output 'out1' of task A
[tasks]
    [[A]]
        [[[outputs]]]
            # task A must emit this message
            out1 = "NWP products uploaded for $(CYCLE_TIME)"
\end{lstlisting}

    \item to trigger off tasks in previous cycles, use:
\begin{lstlisting}
# SUITE.RC
[dependencies]
    [[12,18]]
        graph = "A(T-6) => B"  # B(12) triggers off A(6); B(18) triggers of A(12)
\end{lstlisting}
    \item all of the above can be combined:
\begin{lstlisting}
# SUITE.RC
    graph = "A(T-6):fail => B"  # ...
\end{lstlisting}
...
format: free form multiline strings
containing task names with arrows between them, arbitrary blank space 
and comments. In a typical suite most
tasks run in every cycle (e.g.\ 6 hourly), and a few tasks run more or
less frequently so that the full dependency tree does not repeat for a
full 24 hours. Rather than define everything that happens in a full 24
hour period, which could be a very large graph, the graph can be split
into sections headed by a list of hours for which each graph section is
valid.  For example, here is the graph for a suite with four tasks that
run 6 hourly, and a fifth task that triggers off one of the others only
twice daily:
