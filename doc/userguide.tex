...
contains suites accessible to all users. You can view suites in the 
central database (just right-click on them) but to run them you need
to import them to your private suite database.

\subsection{Import The Example Suite} 

Right-click on the suite registered as `admin:examples:DepGraphCh6'
and choose the `Import' option to copy the suite to your private
suite database (replace `admin' with the name of the user account under
which cylc was installed; and note that the suite owner, group, and
name, are displayed in a tree view (admin $->$  examples $->$
DepGraphCh6). A dialog box will pop up asking for the group 
and name under which to register the suite in your private database,
and a destination directory into which to copy the suite definition. 
Leave the suite GROUP:NAME as examples:DepGraph6, and choose a 
destination directory that you have permission to write to. Now
use the `View' menu to switch back to your private database, and 
check that you now have a copy of the example suite.

The following subsections desribe how to import a copy of the example
suite from the central database to a local directory,
\begin{lstlisting}
...
\subsection{Editing A Suite}

To edit the suite definition via gcylc right-click on the suite and
choose `Edit'; right-click again and choose `Graph' to plot the suite
dependency graph that is defined in the suite.rc file. If you edit the
suite.rc file the graph viewer will update whenever you save the file.
...
do this entirely manually, but using the edit command (or right-click Edit 
suite in gcylc) has some advantages: it is very quick and convenient,
you don't have to remember the suite definition directory
...
split back into its constituent include-files {\em when you save and
exit the editor}. 
...
\end{lstlisting}
(this results in the graph of Figure~\ref{fig-simple-graph-2}, in the 
Suite Design Overview chapter)
