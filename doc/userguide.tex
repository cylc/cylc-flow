...
tasks will default to emitting an identifying message, sleeping for a
few seconds, and then exiting; but you can then arrange for particular
tasks to do more complex things (to fail, for example) by supplying
appropriate task commandlines to replace the default one.

Cylc's example suites run quickly and are portable: you can copy and run
them immediately without modification. You can even run multiple
copies of the same example suite at once, under different registrations,
because the suite group and name are used in all I/O paths.

The ``QuickStart2'' and ``intro'' examples have actual external task
