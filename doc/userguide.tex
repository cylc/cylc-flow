...
This chapter contains a few screenshots to show what cylc looks like.

\begin{myitemize}
    \item Figure~\ref{fig-command-help} - the top level of cylc's commandline interface.
    ...
    \item Figure~\ref{fig-simple-graph-2} - a graph of the simple suite shown in the vim editor.
    ...
    \item Figure~\ref{fig-ecox-1} - a large operational suite graphed by cylc (a).
    \item Figure~\ref{fig-ecox-2} - a large operational suite graphed by cylc (b).
\end{myitemize}

\begin{figure}
    \begin{center}
        \includegraphics[width=0.6\textwidth]{../images/screenshots/commandhelp.png}
    \end{center}
\caption[The top level of cylc commandline help]{\small
The top level of cylc commandline help}
...
The local suite database as seen by gcylc.}
\label{fig-db}
\end{figure} 
...
The central suite database as seen by gcylc.}
\label{fig-cdb}
\end{figure} 

\begin{figure}
    \begin{center}
        \includegraphics[width=0.6\textwidth]{../images/screenshots/simple-suiterc.png}]
    \end{center}
    \caption{A simple cylc suite definition in the vim editor}
    \label{fig-cylc-vim} 
\end{figure} 

\begin{figure}
    \begin{center}
        \includegraphics[width=0.6\textwidth]{../images/screenshots/simple-18-cold.png}
    \end{center}
\caption{The simple suite definition (shown in the vim editor) graphed by cylc.}
...
\begin{figure}
    ...
    \end{center}
\caption[gcylc: Suite Control GUI, original interface]{\small
The original gcylc treeview suite control interface.}
...
\begin{figure}
    ...
    \end{center}
\caption[gcylc: Suite Control GUI, graph interface]{\small
The gcylc graph suite control interface.}
...
\begin{figure}
    ...
    \end{center}
\caption{\small A large operational suite graphed by cylc (a).}
\label{fig-ecox-1}
\end{figure} 
...
\begin{figure}
    ...
    \end{center}
\caption{\small A large operational suite graphed by cylc (b).}
\label{fig-ecox-2}
\end{figure} 
...
future or nothing will happen until that time.}
...
\subsection{Shutting Down And Restarting A Suite}

\lstset{language=bash}

The default shutdown mode causes the suite to stop once any currently
running tasks have finished,
\begin{lstlisting}
prompt> cylc stop examples:userguide
\end{lstlisting}

To stop the suite immediately use the \lstinline=--now= option, but be
aware that this will orphan any tasks that are still running.  

The suite can be restarted from its most recent previous state,
\begin{lstlisting}
prompt> cylc restart examples:userguide
\end{lstlisting}
or from other saved prior states. % (see Section~\ref{StateDumpFiles}).
...
The definitive reference for legal suite.rc items, values, options,
and defaults, is Appendix~\ref{SuiteRCReference}.
...
rightmost member is plotted by default; to change which task is
plotted, change their order or attach an asterisk to the chosen one:
\begin{lstlisting}
# SUITE.RC
...
the cold start task masquerades as the previous-cycle trigger for its
counterpart.
...
from the suite, and thereafter Bar(T-6) will do it.
...
{\em Warm starting} a suite that has previously been shut down is an
alternative to {\em restarting} it from a previous state. Warm starts
...
cycles in case the next (or more) cycle has to be omitted: 
\begin{lstlisting}
# SUITE.RC
[special tasks]
    sequential = A
...
restart files for the very next cycle does not need to be declared
sequential because this early triggering problem does not arise.}
...
the task will not spawn until its last restart output is completed, and 
successives instances of the task will be able to overlap (i.e.\ run in
parallel) if the opportunity arises. Whether or not this is worth the
effort depends on the situation, of course.
...
\begin{myitemize}
    ...
    \item 4. task-specific variables
    \item 5. access to cylc
    \item 6. global and task-specific pre-command scripting (not shown)
    \item 7. task commandline
    \item 8. global and task-specific post-command scripting (not shown)
\end{myitemize}

Consequently, task-specific variables can reference global variables.
...
Both global and task-specific environment variables are defined in
exactly the same order as they are listed in the suite.rc file, so you
can safely refer to previously defined variables.
