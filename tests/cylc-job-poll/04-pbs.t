...
if ! ${IS_AT_T_HOST:-false}; then
    T_HOST=$(cylc get-global-config -i '[test battery][directives]pbs host')
    if [[ -z $T_HOST ]]; then
        skip_all '"[test battery][directives]pbs host" not defined'
    fi
...
if ! ${HAS_READ_T_DIRECTIVES_MORE:-false}; then
    export T_DIRECTIVES_MORE=$(cylc get-global-config -i \
        '[test battery][directives][pbs directives]')
