...
run_ok "${TEST_NAME_BASE}-validate" \
    cylc validate -s "PLATFORM=${CYLC_TEST_PLATFORM}" -s 'GLOBALCFG=True' "${SUITE_NAME}"
suite_run_ok "${TEST_NAME_BASE}-run" \
    cylc run --reference-test --debug --no-detach -s "PLATFORM=${CYLC_TEST_PLATFORM}" -s 'GLOBALCFG=True' \
    "${SUITE_NAME}"
