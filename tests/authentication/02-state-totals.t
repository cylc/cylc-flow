...
grep_ok "client DENIED (privilege 'state-totals' < 'full-read') ${USER}@.*:cylc-show" suite.log1

# Commands should be denied.
TEST_NAME="${TEST_NAME_BASE}-stop"
run_fail "${TEST_NAME}" cylc stop "${SUITE_NAME}"
cylc log "${SUITE_NAME}" > suite.log2
grep_ok "client DENIED (privilege 'state-totals' < 'shutdown') ${USER}@.*:cylc-stop" suite.log2
