...
async def _test_config_change(
    ...
    config: dict = yield
    id_ = flow(config)
    schd: 'Scheduler' = scheduler(id_, paused_start=paused_start)
    ...
    schd = scheduler(id_, paused_start=paused_start)
    async with start_or_run(schd):
        yield schd
...
async def test_graph_change_handling(
    ...
    * If tasks are removed by graph change, they should be logged, killed and
    ...
    # remove "waiting", "running", "failed"; add "bar":
    ...
    assert re.search('Removed tasks', record.message)
    ...
    if change_type == 'reload':
        ...
        assert re.search('Added task definitions', record.message)
        assert re.search(r'\* bar', record.message)


@pytest.mark.parametrize('change_type', ('reload', 'restart'))
@pytest.mark.parametrize('psx_type', ('xtrigger', 'parentless'))
async def test_psx_removal(
    flow,
    scheduler,
    start,
    db_select,
    caplog,
    change_type: Literal['reload'] | Literal['restart'],
    psx_type: Literal['xtrigger'] | Literal['parentless'],
):
    """Orphaned PSX tasks should be removed.

    Parentless/sequential-xtriggered tasks (PSX) should be removed if
    orphaned.

    This is testing the same logic as covered by test_graph_change_handling,
    however, but making sure the PSX removal (which works differently) still
    works.
    """
    prefix = ''
    if psx_type == 'xtrigger':
        prefix = '@a =>'

    test_generator = _test_config_change(
        flow,
        scheduler,
        start,
        change_type,
    ...
    config = {
        'scheduling': {
            'xtriggers': {'a': 'xrandom(0, sequential=True)'},
            'graph': {'R1': f'''
                {prefix} x
                {prefix} y
            '''},
    ...
    # ensure they are written to the database
    await schd._main_loop()
    assert set(
        db_select(schd, True, 'task_pool', 'cycle', 'name', 'status')
    ) == {
        ('1', 'x', TASK_STATUS_WAITING),
        ('1', 'y', TASK_STATUS_WAITING),
    }

    # remove "x" add "z"
    config['scheduling']['graph']['R1'] = f'''
        # {prefix} x
        {prefix} y
        {prefix} z
    '''
    ...
    assert re.search('Removed tasks', record.message)
    assert re.search(r'\* 1\/x.*:waiting', record.message)

    if change_type == 'reload':
        # NOTE: we cannot easily determine added tasks when restarting
        assert re.search('Added task definitions', record.message)
        assert re.search(r'\* z', record.message)
