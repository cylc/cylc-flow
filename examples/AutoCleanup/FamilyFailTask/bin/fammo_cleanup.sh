...
FAILED_TASKS=$(cylc dump $CYLC_SUITE_REGNAME_REGNAME | grep $CYCLE_TIME | grep failed | sed -e 's/,.*$//')
...
for T in $FAILED_TASKS; do
    ...
    if [[ $T == m_* ]]; then
        ...
        cylc control remove --force $CYLC_SUITE_REGNAME_REGNAME ${T}%$CYCLE_TIME
    else
