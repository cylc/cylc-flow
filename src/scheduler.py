...
class scheduler:
    ...
    def initialize_graphfile( self ):
        ...
        default_node_color = self.config.get( 'default_node_color' )
        default_edge_color = self.config.get( 'default_edge_color' )
        if self.config.get( 'use_node_color_for_edges' ):
            default_edge_color = self.config.get( 'default_node_color' )
 ...
 * "foo" cylc suite, and can be visualized with the graphviz
 * package (http://www.graphviz.org). Minimal processing e.g.:"
 *  $ dot -Tps foo.dot -o foo.eps # ps output"
 *  $ dot -Tsvg foo.dot -o foo.svg # svg output"
 * See graphviz documentation, e.g. "Drawing graphs with dot".
 * 
 * Note that nodes in a subgraph with no internal edges all have
 * the same "rank", which determines horizontal placement. To
 * split the subgraph into several rows you can manually add
 * some invisible edges: node1 -> node2 [color=invis];
 */\n\n''' )
        ...
        self.graphfile.write( '    node [ style=filled, fillcolor=' + default_node_color + ', color=invis ];\n' )
        self.graphfile.write( '    edge [ color=' + default_edge_color + ', style=bold ];\n' )
